...
| 検索順位の改善（`/metrics/seo-watch`） | `.claude/config/seo-watchwords.json`・`.claude/state/experiments.json`・GSC rank-watch履歴 | 固定クエリの7日比較、改善内容、本番反映待ち/観察中/達成/再検討、レビュー期限。同じ判定関数をCLIと共有し、読み取り専用。 |
