...
| 戦略・収益化 ＞ 資格一覧（`/strategy/qualifications`） | `.claude/config/qualification-registry.json`（資格の一覧・分類・展開状態 active/candidate/declined）＋ `exam-calendar.json`（日程）＋ `exam-stats.json`（受験者数） | 展開中・候補・見送りの全資格を分類ごとに並べ、次の日程（申込・試験・発表の種類つき、未発表は期間の文言）と最新年度の受験者数・合格率（採用試験は倍率）と出典を出す**読み取り専用**画面。`?p=candidate` などで展開状態を絞り込む。公式で確認できていない値は埋めずに「未確認」と理由を出す。三者の整合は `scripts/lib/qualification-registry.mjs`（`check-exam-calendar` と同じ実装）で判定し、不整合は赤いカードで出す |
