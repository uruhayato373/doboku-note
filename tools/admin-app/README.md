...
| 戦略・収益化 ＞ 商品ラインナップ（`/content/lineup`） | `.claude/config/product-lineup.json`（資格×試験区分の定義・id→マスの分類ルール・アプリ/PWA の手動台帳）＋ `note-magazines.ts`／`coconala-services.ts`／`kindle-published/catalog.json`／`brain-products.ts` | 商品を「資格 × 試験区分 × チャネル」の表に並べ、表紙サムネ・価格・状態だけを出す**読み取り専用**画面。販売中 0 件のマスは「未展開」。試験日・受験者数は資格一覧（`/strategy/qualifications`）が扱う。分類は `scripts/lib/product-lineup.mjs`（`tests/product-lineup.test.mjs`）、未分類は赤表示、読めなかったチャネルは「未検査」。停止中は既定で隠し `?retired=1` で表示 |
