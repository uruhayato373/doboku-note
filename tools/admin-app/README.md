...
| アフィリエイト ＞ 成果（`/affiliate`） | `.claude/state/metrics/affiliate/a8-report-log.json`・`career-funnel-latest.json`・`experiments.json` | 1画面の最小表示: サイト内クリックとクリック率（GA4・配置別、`npm run report-career-funnel`）、A8 の発生・確定（対象期間の行だけ）、実行中の実験の次の判定日、未写像の件数。口座横断の月別・日別と検算は週次レビュー（`check-a8-report-due`）が見る。データ供給は `/a8-report` |
| アフィリエイト ＞ 掲載先（`/affiliate/placements`）・提携・案件（`/affiliate/programs`） | 原稿（src・content/site・content/note・content/sns）の a8mat、`src/config/affiliate-mats.json`、`.claude/state/ads/affiliate-catalog.json` | 掲載先（サイト／note／SNS）ごとのリンクのある原稿と案件、note 記事の一覧（`scripts/lib/affiliate-placements.mjs`）。案件ごとの配置・ASP の提携状態と報酬・リンク期限 |
...
| 計画 ＞ 月間・週間・バックログ（`/todo?f=monthly|weekly`・`/todo`） | `.claude/todo/*.md`, `.claude/state/todo-claims.json`, `.claude/plans/**` | **月間＝`[時期:]` が今月を含むカード（導出・バックログと同じ表）＋monthly.md の成果目標、週間＝weekly.md の表、年間は `/plan/roadmap` へ転送**（2026-09-26〜。サイドバーの件数も同じ導出）。**絞り込み（優先度/種類）は右ペイン**＝属性、という分担。カテゴリ軸は 2026-08-18 に廃止。カード本文は `lib/markdown.ts` の共有レンダラ（ナレッジタブと同一）で HTML 表示。**backlog カードは `DN-####` を持ち、`?id=DN-0001` で 1 枚を強調＋アンカー**。weekly は本文を複製せず ID 参照ビューで、backlog と `backlogIndex()` join してタイトル・優先度・期日を表示（台帳に無い ID は「台帳なし」赤バッジ・コミット時は check-backlog-schema の `weekly-ref` が止める）。backlog カードには**そのタスクを参照している Project 文書**へのリンクも出る。**状態列**（IN_PROGRESS/THIS_WEEK/THIS_MONTH/PLANNED/BACKLOG＝`deriveStatus()` の導出のみ・新台帳を持たない）・**claim 表示**（`[進行中]` カードの owner と経過時間）・**実装計画リンク**（`.claude/plans/` の unit があれば `/plans/...` へ）・**Claude Code 向け prompt 生成**（カードID・実装契約・検証コマンドを組み立ててコピーのみ、実行はしない）を DN-0093 順5 で追加。read-only・編集はカード見出しの VS Code リンク |
...
| 戦略・収益化 ＞ 展開の判断（`/strategy/market`） | `exam-formats.json`（出題形式）＋ `exam-stats.json`（受験者数）＋ `exam-calendar.json`・`annual-roadmap.json`（買われる時期）＋ `sales-log.json`・`product-lineup.json`（売上）＋ `market-scan.json`・`.claude/state/market/history/`・`coconala/market-research.json`（混み具合）＋ `*-competitors.json`（追跡数） | 展開する資格を決めるための**読み取り専用**画面。資格ごとに「自分で書く区分（経験記述・論文）・その受験者数・買われる時期・売上・YouTube / note / ココナラの混み具合（無/少/中/多と強い売り手の数）・X / IG の追跡数」を 1 行に出す。並べ替えは受験者（既定・展開中→候補→見送りの順）／売上／資格名（`?sort=compose|sales|name`）。組み立ては `scripts/lib/qualification-market.mjs`（`npm run qualification-market` と同じ）。ココナラの * は資格専用の検索語が未取得で汎用の検索結果から数えた下限。正本の不整合（`check-qualification-market`）と要対応は件数だけ出す |
