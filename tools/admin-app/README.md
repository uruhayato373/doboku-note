...
| アフィリ | `.claude/state/metrics/affiliate/a8-report-log.json`・`career-funnel-latest.json`・`experiments.json` | 1画面の最小表示: サイト内クリックとクリック率（GA4・配置別、`npm run report-career-funnel`）、A8 の発生・確定（対象期間の行だけ）、実行中の実験の次の判定日、未写像の件数。口座横断の月別・日別と検算は週次レビュー（`check-a8-report-due`）が見る。データ供給は `/a8-report` |
