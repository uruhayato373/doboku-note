...
| 戦略・収益化 ＞ 資格一覧（`/strategy/qualifications`） | `.claude/config/qualification-registry.json`（資格の一覧・分類・展開状態 active/candidate/declined）＋ `exam-calendar.json`（日程）＋ `exam-stats.json`（受験者数） | 人が見る画面として、分類ごとの見出し行つきの 1 表で「資格・状態・試験日・合格発表・受験者数・合格率」だけを出す**読み取り専用**画面。試験日・合格発表は次の予定（無ければ今年度で最後のものを薄く）。技術士第二次の総監以外の部門は日程が共通なので 1 行にまとめ、受験者数・合格率は部門別表の 20 部門合計を出す（正本は部門ごとのまま）。出典・照合記録・未確認の理由は画面に出さず、正本と `npm run exam-ssot-status`（月次レビュー）が持つ。正本の不整合（`scripts/lib/qualification-registry.mjs`・`check-exam-calendar` と同じ判定）があるときだけ警告を出す |
| 戦略・収益化 ＞ 商品ラインナップ（`/content/lineup`） | `.claude/config/product-lineup.json`（資格×試験区分の定義・id→マスの分類ルール・アプリ/PWA の手動台帳）＋ `note-magazines.ts`／`coconala-services.ts`／`kindle-published/catalog.json`／`brain-products.ts` | 商品を「資格 × 試験区分（一次/二次/口頭）× チャネル（note/ココナラ/Kindle/Brain/アプリ・PWA）」の表に並べ、表紙サムネ・価格・状態を出す**読み取り専用**画面。区分列には `.claude/config/exam-calendar.json` の今年度の試験日と残り日数（JST）を出し、合格発表日も同ファイルの events、日付未発表の口頭試験・官報公告などは `periods`（公式の期間の文言）を出す。受験者数・合格率は `.claude/config/exam-stats.json` の最新年度を出し、公式未確認・未掲載の値は埋めずにそう表示する（ラインナップ config には日付・期間・人数を書かず、参照 id だけを持つ）。販売中 0 件のマスを「未展開」と表示して打ち手の空白を見せる。分類は `scripts/lib/product-lineup.mjs` が唯一の実装（`tests/product-lineup.test.mjs`）、状態語彙は `content-lifecycle.mjs` を共有。どのルールにも当たらない商品は落とさず「未分類」で赤表示、読めなかったチャネルは「未検査」と出す。note 表紙は `content/note/*/magazines/*/` の `_cover.png`・`img/cover.png` を utmCampaign・noteUrl・ラベル・掲載文タイトルで結び付ける（repo に実体の無い表紙は「表紙なし」）。停止中は既定で隠し `?retired=1` で表示 |
