...
| 領域の概要（`/domains/<id>`・サイドバーのグループ名から） | `.claude/config/domains.json`・backlog の `[領域:]`・frontmatter `domain`・`documents` | 領域ごとの次の予定・タスク・スキルとエージェント・文書。サイドバーのグループ名と並びも domains.json（`lib/domains.ts`） |
