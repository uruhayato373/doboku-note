...
| 年間ロードマップ（`/plan/roadmap`） | `.claude/config/annual-roadmap.json`・`exam-calendar.json`・`qualification-registry.json` | 月の横軸に、展開中の資格の申込・試験・合格発表と買い場（翌年の未公表分は昨年度から推定・破線）、領域ごとの重点の帯（バックログのカードへリンク）。計算は `scripts/lib/annual-roadmap.mjs` |
