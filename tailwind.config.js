...
module.exports = {
  // 走査対象。**ここに無いファイルに書いたクラスは 1 つも生成されない**（JIT）。
  // 2026-08-20 実測: './src/pages' は App Router 移行で存在しない死んだ glob、
  // src/lib・hooks・utils・config・types は className を持ちうるのに未走査だった。
  // 「変種が効かない」の正体は大半がこれ（@layer 順や PostCSS 設定ではない）。
