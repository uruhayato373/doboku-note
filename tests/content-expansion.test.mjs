...
function fixture(t) {
  ...
  const registry = { sources:[{id:'source-1',title:'試験教材',shortTitle:'試験教材',shelf:'テスト',class:'commercial-book'}] };
...
test('サイドバー用の短い名前と棚が無い教材は台帳の確認事項に出す', t => {
  const f=fixture(t);delete f.registry.sources[0].shelf;
  const r=expansionReport(f.root,f.data,f.registry);
  assert.ok(r.issues.some(x=>x.includes('shortTitle / shelf')));assert.equal(r.productionComplete,false);
});
