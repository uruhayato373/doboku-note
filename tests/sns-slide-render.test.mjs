...
test('renderSlide: definition 型で 1080×1920 の PNG を生成', async () => {
  const png = await renderSlide({
    width: 1080,
    height: 1920,
    slide: {
      ...
      data: {
        title: 'フォロワーシップ',
        body: '部下が独自のクリティカルシンキングと積極的関与で組織に貢献するアプローチ',
      },
    },
  });
  assert.ok(Buffer.isBuffer(png));
...
test('renderSlide: examPoint 型で 1080×1920 の PNG を生成（番号付き）', async () => {
  const png = await renderSlide({
    width: 1080,
    height: 1920,
    slide: {
...
test('renderSlide: examPoint 型で index 省略時もデフォルト 1 で動作', async () => {
  const png = await renderSlide({
    width: 1080,
    height: 1920,
    slide: { type: 'examPoint', data: { body: 'デフォルト 1' } },
  });
  assert.ok(Buffer.isBuffer(png));
});

test('renderSlide: cta 型で 1080×1920 の PNG を生成', async () => {
  const png = await renderSlide({
    width: 1080,
    height: 1920,
    slide: {
...
test('renderSlide: cta 型でデフォルト値（headline/subline/url 省略）でも動作', async () => {
  const png = await renderSlide({
    width: 1080,
    height: 1920,
    slide: { type: 'cta', data: {} },
