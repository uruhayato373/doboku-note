...
> **多資格運用の真実源は [.claude/knowledge/reference/x-post-policy.md](../../../.claude/knowledge/reference/x-post-policy.md)**（試験別テンプレ/タグ・投稿型・5 軸ルーブリック・偽成功検証）。本ファイルは文字数ルール等の現場規約。執筆は `x-post-writer`（Generator）、採点は `x-post-qa`（Evaluator）が担う。
...
# tweets.md 単発検証（汎用）
node .tmp/count-x-chars.mjs content/sns/x/draft/<NNN>-<name>/tweets.md
```
