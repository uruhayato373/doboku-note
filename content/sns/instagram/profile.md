...
personaQualifications:
  ...
  - 技術士（総合技術監理部門）
relatedDocs:
  - content/sns/x/draft/033-pinned-profile/tweets.md  # X プロフィール + 固定ポスト草案
  - content/note/プロフィール.md  # note プロフィール (140 字)
...
詳細: `docs/marketing/01_SNS集客戦略.md` v6 §2
...
- 採用版を変更した場合、X 版 (`content/sns/x/draft/033-pinned-profile/tweets.md`) と note 版 (`content/note/プロフィール.md`) の経歴・資格表記の整合を確認する
...
```bash
node -e "const bio = require('fs').readFileSync('content/sns/instagram/profile.md','utf8').match(/```\n保存して.*?\n```/s)[0]; console.log('chars:', [...bio.replace(/```\n|\n```/g,'')].length, '/ 150');"
```
