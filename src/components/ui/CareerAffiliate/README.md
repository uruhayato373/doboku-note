...
建設・施工管理の**転職サービス**のアフィリエイトリンクを統一カードで表示するコンポーネント。資格講座用 `CourseAffiliate` の姉妹コンポーネント。

## CourseAffiliate との違い

| 観点 | CourseAffiliate | CareerAffiliate |
|---|---|---|
| 対象 | 資格講座（SAT・アガルート等） | 転職サービス（RSG・セコカン等） |
| 計測ピクセル | `trackingPixel`（mat 値、`www19` 固定） | `trackingPixelUrl`（完全 URL、ASP 横断） |
| 訴求ポイント | なし | `points`（年収UP率・求人数等の箇条書き） |
| 画像 | 必須 | 任意（テキスト主体カード可） |
