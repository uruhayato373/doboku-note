...
const MAGAZINES_RAW = {
  ...
  // P1 15工種を先行生成済(keiken-charcount --strict=0 / note-lint OK / 答案重複0)。
  // マガジンは note 公開済(2026-07-01・m8554e87ca6ec)。ただし15記事の収録完了まで published:false 据え置き
  //（空/部分マガジンを広告しない公開ゲート＝civil-1-keiken-complete-pack と同方針）。収録完了後に true 化。
  // 価格は P1 先行の暫定 ¥3,480（フル36で ¥5,480〜 改定）。
  'civil-2-koji-bank': {
    ...
    description:
      '2級土木施工管理技士 第2次検定 問題1（施工経験記述）の想定工事バンク。自分が経験した工事に近い工種を選び、その1工事で品質・安全・工程の3管理をまとめて準備できる工種起点の完成答案集。各工種で現行形式（令和6年度〜・各250字）と令和5年度以前の3項目形式の両方に対応したフル答案＋自分の現場への置換ガイド＋減点NG→合格答案の添削例＋採点者視点＋2テーマ必答の組合せ早見表を収録。必出の3管理に加え、制度改定に備えた保険として施工計画・環境対策も同じ工事で用意（この2つは現状の経験記述では未出題）。現場代理人レベルの独自表現で、改変前提のテンプレート（合格を保証するものではありません）。',
