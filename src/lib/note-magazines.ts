...
const MAGAZINES_RAW = {
  ...
  'pe-construction-road-magazine': {
    ...
    description:
      '技術士第二次試験 建設部門「道路」選択科目の令和3〜7年度を、II-1（全4設問）・II-2（両選択肢）・III（両問題）の全選択肢でフル解答した模範解答集（5年分 × 3区分 ＝ 15記事）。道路科目の合格者かつ元・地方自治体の土木職（発注者）の視点で、各記事に設問全文（出典明記）・設問構成と論述方針・フル模範解答・採点者が見るポイントを収録。さらに令和8年度の出題傾向・国土交通行政の重点施策・改訂コンピテンシーから導出した予想問題＋フル模範解答（II-1・II-2 と、III は脱炭素／4車線化／事前防災／xROAD の4テーマ＝計6記事）を収録した試験直前対策付き（全21記事）。各記事に印刷用PDF付き。',
