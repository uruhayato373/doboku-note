...
**SSOT** — アイデンティティ＝[CHARACTER-SPEC.md](CHARACTER-SPEC.md)（設定書）／ポーズ機械可読＝[`.claude/config/character-poses.json`](../../../../.claude/config/character-poses.json)／運用＝[.claude/knowledge/reference/character-asset-policy.md](../../../reference/character-asset-policy.md)。追加抽出＝`npm run character-extract`。
