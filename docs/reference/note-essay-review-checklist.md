...
着手前にこの判定を通し、異業種は作らない。受注者系は売るが受注者視点の制約を厳守する（発注者権限の施策は書かない。詳細は本表 Step 0 の受注者系各行を参照）。
...
| 自治体（新標準化済・要note再投稿） | 道路 | ✅ **新標準化完了（2026-06-13）**。R08二記事化（R08-yosou-1 気候変動適応・グリーンインフラ／-2 資源循環・サプライチェーン強靭化、各 A案/B案）＋R03-R07 散文化（全ゲートGREEN: blockquote0/答案箇条書き0/600字超0/文体0/note-lint 7記事OK/U+FFFD0/見出し0）。cover 7記事生成済・hashtags テーマ別整備・note-magazines.ts 7記事化済。残=note実投稿（旧 R08-yosou 単一→R08-yosou-1/-2 へ差し替え）＋PDF7本生成（Chrome 環境） |
...
| 受注者系（新標準化済） | ゼネコン / 河川コンサル / 道路橋梁コンサル | ✅ **新標準化完了（2026-06-13 確認）**。3ペルソナとも 7記事・R08二記事化（気候変動／資源循環）・blockquote 0・`note-magazines.ts` published:true。受注者視点厳守で執筆済み。散文性ゲートの全項目最終確認は随時推奨 |
