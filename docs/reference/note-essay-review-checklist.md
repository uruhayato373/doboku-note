...
> [!note]
> **記事の導入・無料部分・販売導線**（論文本文の外枠）は [note-selling-structures.md](note-selling-structures.md)（売れる9型）を観点に組む。模範論文（＝商品本体）の字数・散文性・専門度は上の評価軸が優先で、9型は「読者の悩みをつかみ購入理由を作る」フック〜導線にだけ適用する。総監模範論文 note は専任 writer エージェントがなく親駆動のため、ここで SSOT に橋渡しする。
