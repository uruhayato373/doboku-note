...
`docs/sns/` は一時 1.22 GB / 6,646 ファイルまで肥大し、その大半が再生成可能なバイナリだった。`.gitignore`（135-149 行）は wav/mp4 を「再生成可能なビルド成果物」として既に除外宣言していたが、ルール追加前に commit された 844 件（732 MB）が追跡され続けていた（untrack 漏れ）。これを `git rm --cached` で外し（commit `16d779a0d`）、以後はこの方針で運用する。
