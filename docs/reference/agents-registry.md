...
| 技術士建設部門 模範解答生成（親が起動）           | `pe-secondary-exam-writer`, `pe-secondary-exam-qa`              | 二次模範解答 article.md の生成 → 5軸採点ループ（論述メソッドは `技術士論文の書き方` 由来） |
...
| `pe-secondary-exam-writer`     | 技術士第二次試験 建設部門 note有料マガジン用 模範解答 article.md を生成（必須科目I・選択科目II-1/II-2/III の全科目種別・全11専門分野。元公務員発注者視点を全科目に注入。合格3科目=道路/河川/都市計画は合格者訴求、残8科目は発注者監修訴求。**論述メソッドの真実源＝`docs/textbook/技術士論文の書き方`**：設問の問い分解・論点の絞り込み・あいまい表現排除を執筆原則に組込。**記事単位の完全梱包DoD**＝cover:ブロック＋img/cover.png＋hashtags約90個、選択科目は article*.md 全件、経験記述免責の誤流用禁止）  | Generator    | sonnet  | `docs/note/技術士建設部門/noteコンテンツ計画.md` 参照、`pe-secondary-exam-qa` と対 | ✅ 運用中（2026-06-08 起動、2026-06-09 梱包DoD追加）                      |
| `pe-secondary-exam-qa`         | 技術士第二次試験 建設部門 模範解答 article.md の**6軸**採点（設問適合/論述構成・論点絞り込み/分かりやすさ・あいまい表現排除/発注者視点・専門性/note完成度/**改訂コンピテンシー反映〔令和8〜：データ活用・多角的視点/ステークホルダー・持続可能な成果/経済/文化的価値〕**）＋必須ゲート（U+FFFD・note-lint〔pipe表/太字内全角括弧〕・本文価格/ID・字数上限〔枚数×600字〕・設問1対1・論述式文体・著作権）。論述原則は `技術士論文の書き方`、改訂は `pe-construction-competency-revision-r8` から。**記事単位の完全梱包チェック**（cover:ブロック/img cover.png/hashtags~90/マガジン_meta・SoT）＋経験記述免責の誤流用ゲートを追加 | Evaluator    | sonnet  | `pe-secondary-exam-writer` と対、`docs/textbook/技術士論文の書き方` 参照 | ✅ 運用中（2026-06-09 起動、6軸化＋梱包チェック） |
...
| `ig-carousel-writer`           | Instagram カルーセル `slide-data.json` v2 を1キーワードずつ執筆（枚数可変・figure 判断・findings ログ追記）。色を本文に書かない。**角度モード（angle）**: 既存 note 記事を源に 6 切り口（結論/理由/体験/反論/数字/ハウツー）で展開、Phase 1 は notebook-* 型再利用 | Generator    | sonnet  | `docs/reference/ig-carousel-policy.md` + `docs/design-system/instagram-carousel.md` + **`docs/reference/content-angle-policy.md`** 参照 | 🚧 Phase 1 着手中（2026-05-20 起動、2026-06-09 角度モード草案） |
| `ig-carousel-qa`               | Instagram カルーセル の **6 軸**ルーブリック品質評価（テキスト 5 軸 + デザイン統一性 1 軸）。過去問パックは PNG を Read し tokens.json と照合。**角度型は軸1で角度純度・軸5で Red Line（experience 断片/number 出典/verbatim なし）を確認** | Evaluator    | sonnet  | `docs/reference/ig-carousel-policy.md` + `docs/design-system/instagram-carousel-tokens.json` + **`docs/reference/content-angle-policy.md`** 参照 | 🚧 Phase 1 着手中（2026-05-20 起動、2026-05-27 第6軸追加、2026-05-28 lint E3 はみ出し検知連携、2026-06-09 角度純度） |
...
| `x-post-writer`                | X(旧Twitter)投稿 `tweets.md` を多資格（総監/1級土木/2級土木）横断で執筆。過去問/キーワードからネタ生成、280 weighted 以下・試験別ベースタグ・サイト誘導を遵守。`social-post`/`create-x-card`/`publish-x` と連携。**`experience` 型（合格者の思考・体験の断片）新設**、投稿型↔6 切り口（angle）対応 | Generator    | sonnet  | `docs/reference/x-post-policy.md` + **`docs/reference/content-angle-policy.md`** 参照、`x-post-qa` と対 | ✅ 運用中（2026-06-02 起動、2026-06-09 experience 型/角度対応） |
| `x-post-qa`                    | X 投稿 `tweets.md` の **5 軸**ルーブリック品質評価（文字数 280 weighted・論点的確さ・タグ 1-3 個・導線/UTM・偽成功検証）。`publish-x` の予約完了ログを信用せず予約キュー実査を採点。**軸2 で角度純度（主角度 1 つ・experience 断片・number 出典）を確認** | Evaluator    | sonnet  | `docs/reference/x-post-policy.md` + **`docs/reference/content-angle-policy.md`** 参照、`x-post-writer` と対 | ✅ 運用中（2026-06-02 起動、2026-06-09 角度純度） |
