...
> **件数の SSOT**: エージェント数の真実源は `.claude/agents/*.md` の実数（`find .claude/agents -maxdepth 1 -name '*.md' | wc -l`＝現在 **68**）と下記「エージェント一覧」表。CLAUDE.md など他 doc は件数を重複記載せずここを指す。追加/削除は同一 commit でこの表を更新する。
...
| `/coconala-publish`                       | `coconala-operator`                                             | ココナラ出品サービスを Playwright で新規出品・修正・価格反映（account assert＋draft-first＋--commit gate） |
| `/coconala-order`                         | `coconala-operator`（→ `civil-keiken-tensaku-drafter`）          | ココナラ受注1件のE2E（検証→添削下書き→納品文面→orders-log） |
| `/coconala-status`                        | `coconala-operator`                                             | ココナラ KPI 記録＋カタログ↔state 照合＋撤退ライン判定（read-only） |
...
| `coconala-operator`            | ココナラ（coconala.com）出品サービス（S1 合格診断/S2 添削/S3 答案作成〔ヒアリング→文章化〕/C1-C9 コンテンツPDF〔含 C8・C9 予想模試〕）の運用オーケストレーター。①**出品・修正**（`/coconala-publish`＝`coconala-publish/edit.mjs` でカタログ＋listings を出品フォームへ充填・公開成功でカタログ書き戻し）②受注E2E（`/keiken-tensaku`→納品文面ドラフト→`orders-log.json`）③KPI 手動貼付の正規化→`kpi-log.json`＋撤退ライン判定 ④カタログ status/価格/満枠 flip。**出品・内容修正・価格反映は Playwright で自動化**（account assert=sellerName dobokunote＋draft-first＋`--commit` gate）だが**トークルーム返信送信は運営者**（2026-07-18 改訂）。安全弁＝捏造禁止(Red Line #2 再定義・経験してない工事/事実を創作しない)／外部誘導禁止(規約)／価格はカタログが真実源／公開はエラー時に「公開した」と報告しない／顧客個人情報の非コミット。整合は決定論ガード `check-coconala-wiring`（原則5・catalog↔listings↔画像↔state↔sales カバレッジ） | Generator/オーケストレーター | sonnet | `/coconala-publish`・`/coconala-order`・`/coconala-status` 連携、`civil-keiken-tensaku-drafter`（添削/作成/診断モード）を起動、SoT=`coconala-services.ts`＋`coconala-listings.json`、運用=`docs/reference/coconala-operations.md` | ✅ 運用中（2026-07-16 起動・07-18 出品自動化/S3答案作成/C8-C9予想模試を追加） |
...
| `ig-publish-auditor`           | IG カルーセルパックの**予約投稿 公開可否ゲート**（caption 実在/品質・画像 2〜10 枚・draft 痕跡なし・テーマ整合）＋ `verify-ig-status` 照合結果の**重複投稿/疑わしいドリフトを要人手判断でフラグ**。reconcile JSON ＋ caption.txt ＋ 画像一覧 ＋ status を親から受け取り ready/blocked/flags を返す。audit-only（投稿・予約・SoT 編集はしない・Bash 不可） | Evaluator    | sonnet  | `/ig-reconcile` の未公開予約ゲート。真実源 `docs/reference/ig-publish-reconcile.md`。デザイン6軸の `ig-carousel-qa` とは守備範囲別 | ✅ 新設（2026-06-25 起動） |
...
| `note-membership-operator`     | note メンバーシップ「土木セコカン合格ラボ」（1級・2級土木）専用オーケストレーター。会員特典記事の配信（note-publish で公開→note-magazine-add-articles で特典マガジン収録＝会員へ自動配信）・記事/プラン設定編集（プラン会費/定員/説明の**保存**は note-membership-plan-edit＝非公開のまま可逆・dry-run既定／**「プランの公開」は不可逆ゆえ守備外＝運営者手動**）・公開後 SoT 反映（civil-membership-lab + もくじ URL）。収益アカウント安全弁（account ゲート/draft既定/dry-run）内蔵。ローカル実行限定。買い切り汎用の note-operator とは守備範囲が直交 | Orchestrator | sonnet  | 親が起動 / `/note-membership`、`docs/note/1級・2級土木/noteコンテンツ計画.md`・`メンバーシップ/README.md`・`note-api-verification.md` 参照 | ✅ 運用中（2026-06-23 起動） |
