...
| `/keiken-tensaku`                         | `civil-keiken-tensaku-drafter`                                  | 顧客提出原稿 → 添削返却ドラフト生成（会員/ココナラ共通） |
...
| `coconala-operator`            | ココナラ（coconala.com）出品サービス（S1 合格診断/S2 添削セット/C1 出題分析PDF/C2 完成答案集PDF）の運用オーケストレーター。①**出品・修正**（`/coconala-publish`＝`coconala-publish/edit.mjs` でカタログ＋listings を出品フォームへ充填・公開成功でカタログ書き戻し）②受注E2E（`/keiken-tensaku`→納品文面ドラフト→`orders-log.json`）③KPI 手動貼付の正規化→`kpi-log.json`＋撤退ライン判定 ④カタログ status/価格/満枠 flip。**出品・内容修正・価格反映は Playwright で自動化**（account assert=sellerName dobokunote＋draft-first＋`--commit` gate）だが**トークルーム返信送信は運営者**（2026-07-18 改訂）。安全弁＝代筆禁止(Red Line #2)／外部誘導禁止(規約)／価格はカタログが真実源／公開はエラー時に「公開した」と報告しない／顧客個人情報の非コミット。整合は決定論ガード `check-coconala-wiring`（原則5） | Generator/オーケストレーター | sonnet | `/coconala-publish`・`/coconala-order`・`/coconala-status` 連携、`civil-keiken-tensaku-drafter` を起動、SoT=`coconala-services.ts`＋`coconala-listings.json`、運用=`docs/reference/coconala-operations.md` | ✅ 運用中（2026-07-16 起動・07-18 出品自動化を追加） |
| `civil-keiken-tensaku-drafter` | 1級・2級土木 施工経験記述の**顧客提出原稿**（合格ラボ添削層会員／ココナラ購入者）から添削返却ドラフトを生成（字数判定〔keiken-answer-sheet-limits.json〕・6軸論点抽出・チェックリスト事前判定・NG→OK赤入れ案 最重要2点・採点者視点一言2案。**代筆禁止＝原文の事実の範囲での書き換え提案のみ**・合格保証表現禁止・最終赤入れは運営者）。自社マガジン答案を採点する civil-keiken-essay-qa とは対象が異なる | Generator    | sonnet  | `/keiken-tensaku` 連携、添削テンプレ（2級経験記述-添削テンプレ.md）準拠、ココナラ展開キット参照 | ✅ 運用中（2026-07-16 起動）                      |
