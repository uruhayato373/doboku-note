...
| `note-membership-operator`     | note メンバーシップ「土木セコカン 1発合格ラボ」（1級・2級土木）専用オーケストレーター。会員特典記事の配信（note-publish で公開→note-magazine-add-articles で特典マガジン収録＝会員へ自動配信）・記事/プラン設定編集の起動（note-edit-session・自動保存しない）・公開後 SoT 反映（civil-membership-lab + もくじ URL）。非重複の一線（過去問を会員に入れない）assert・収益アカウント安全弁（account ゲート/draft既定/dry-run）内蔵。ローカル実行限定。買い切り汎用の note-operator とは守備範囲が直交 | Orchestrator | sonnet  | 親が起動 / `/note-membership`、`docs/note/1級・2級土木/noteコンテンツ計画.md`・`メンバーシップ/README.md`・`note-api-verification.md` 参照 | ✅ 運用中（2026-06-23 起動） |
