...
| `coconala-operator`            | ココナラ（coconala.com）出品サービス（S1 合格診断/S2 添削セット）の運用オーケストレーター。①受注E2E（ヒアリングシート検証→`/keiken-tensaku` で添削下書き→納品文面ドラフト→`orders-log.json` 追記）②KPI 手動貼付の正規化→`kpi-log.json`＋撤退ライン判定 ③カタログ（`src/lib/coconala-services.ts`）の status/価格/満枠 flip。**ココナラ UI の実操作（出品・返信送信）はしない**＝文面生成と記録まで・送信はユーザー。安全弁＝代筆禁止(Red Line #2)／外部誘導禁止(規約)／価格はカタログが真実源／AI下書き注記の残存禁止／顧客個人情報の非コミット。整合は決定論ガード `check-coconala-wiring`（Evaluator を作らず機械で担保＝原則5） | Generator/オーケストレーター | sonnet | `/coconala-order`・`/coconala-status` 連携、`civil-keiken-tensaku-drafter` を起動、SoT=`coconala-services.ts`、運用=`docs/reference/coconala-operations.md` | ✅ 運用中（2026-07-16 起動） |
