...
| `guide-qa`                     | ガイド記事（`group: guide`・全資格横断）のガイド軸5軸評価（導入/リード文の質§26・読みやすさ/文体§24・本文ボリューム§25・コンバージョン導線§20・モバイル視認性）。過去問軸 past-exam-qa／キーワード軸 cem-qa と直交。audit-only。**Generator は civil ガイドのみ `civil-textbook-rewriter`（civil-1/2 限定）。pe/総監/コンクリートのガイドには専用 Generator が無く、現状は `general-purpose`（model:opus または sonnet）にルールをプロンプト埋込で代用**（要・専用化検討）。加筆事実の公開前 WebSearch 照合は別途必須（[[factcheck-guide-facts-required]]）。bulk 評価は `model:'sonnet'` 明示 | Evaluator | inherit | lint-mdx-mobile（6-1〜6-4/15-x/12-x）, check-guide-length, check-mdx | ✅ 運用中（2026-06-21 起動）                   |
