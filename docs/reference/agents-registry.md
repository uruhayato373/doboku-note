...
| `pe-secondary-exam-writer`     | 技術士第二次試験 建設部門 note有料マガジン用 模範解答 article.md を生成（必須科目I・選択科目II-1/II-2/III の全科目種別・全11専門分野。元公務員発注者視点を全科目に注入。合格3科目=道路/河川/都市計画は合格者訴求、残8科目は発注者監修訴求。**論述メソッドの真実源＝`docs/textbook/技術士論文の書き方`**：設問の問い分解・論点の絞り込み・あいまい表現排除を執筆原則に組込）  | Generator    | sonnet  | `docs/note/技術士建設部門/noteコンテンツ計画.md` 参照、`pe-secondary-exam-qa` と対 | ✅ 運用中（2026-06-08 起動）                      |
| `pe-secondary-exam-qa`         | 技術士第二次試験 建設部門 模範解答 article.md の5軸採点（設問適合/論述構成・論点絞り込み/分かりやすさ・あいまい表現排除/発注者視点・専門性/note完成度）＋必須ゲート（U+FFFD・note-lint〔pipe表/太字内全角括弧〕・本文価格/ID・字数上限〔枚数×600字〕・設問1対1・論述式文体・著作権）。論述原則は `技術士論文の書き方` から抽出 | Evaluator    | sonnet  | `pe-secondary-exam-writer` と対、`docs/textbook/技術士論文の書き方` 参照 | ✅ 運用中（2026-06-09 起動）                      |
