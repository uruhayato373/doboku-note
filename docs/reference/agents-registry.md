...
| `/keiken-tensaku`                         | `civil-keiken-tensaku-drafter`                                  | ①添削（提出原稿→赤入れ）②作成（`--mode sakusei`・ヒアリング→答案ドラフト・ココナラ S3） |
...
| `civil-keiken-tensaku-drafter` | 1級・2級土木 施工経験記述の**顧客対応ドラフト**を生成する 2モード Generator。①**添削（既定）**＝提出原稿→添削下書き（字数判定〔keiken-answer-sheet-limits.json〕・6軸・チェックリスト・NG→OK 2点・採点者視点2案）②**作成（`--mode sakusei`・ココナラ S3）**＝下書き無しで作成用ヒアリングシートの回答→答案ドラフト（2テーマ×設問1/2・字数内・事実確認チェックリスト）。**捏造禁止（Red Line #2 再定義）＝顧客が回答/記述していない工事・事実・数値を創作しない（欠落は追加質問 or 〇〇）**・合格保証禁止・最終確認は運営者。自社マガジン模範答案を生成/採点する essay-writer/essay-qa とは対象が異なる（顧客答案が対象） | Generator    | sonnet  | `/keiken-tensaku` 連携、添削テンプレ準拠、ヒアリングシート=ココナラ展開キット §4/§4b | ✅ 運用中（2026-07-16 起動・07-18 作成モード追加）  |
