...
| `x-post-writer`                | X(旧Twitter)投稿 `tweets.md` を多資格（総監/1級土木/2級土木）横断で執筆。過去問/キーワード/テーマからネタ生成（切り口分割 angle-slice = 結論/理由/体験/反論/数字/ハウツーの 6 投稿化に対応）、280 weighted 以下・試験別ベースタグ・サイト誘導を遵守。`social-post`/`create-x-card`/`publish-x` と連携 | Generator    | sonnet  | `docs/reference/x-post-policy.md` 参照、`x-post-qa` と対 | ✅ 運用中（2026-06-02 起動） |
