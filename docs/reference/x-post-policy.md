...
同じキーワード・過去問・note記事でも、**切り口を変えることで同一フォロワーに繰り返し届けられる**。
ネタ切れを防ぎ、同一素材から最大6本のツイートを展開できる。

| # | 切り口 | 問いの形 | 例 |
|---|---|---|---|
| 1 | **結論** | 「〇〇が合否を分ける」と断言する | 「品質管理と経済性管理の順番を間違えると不合格になる」 |
| 2 | **理由** | なぜそれが重要か、論拠1つに絞る | 「総監でトレードオフを書かない人が落ちる理由」 |
| 3 | **体験** | つまずき・気づき・学習過程の断片 | 「自分が2回目でやっと気づいた5管理の読み方」 |
| 4 | **反論** | 「〇〇は不要」という通説を覆す | 「暗記は意味ない、と思っていたが違った話」 |
| 5 | **数字** | データ・統計・具体的な量で示す | 「合格率20.7%、上位2割の答案に共通する構造は…」 |
| 6 | **ハウツー** | 手順・方法・再現できる型で示す | 「設問(2)を15分で骨子化する手順」 |

**運用ルール:**
- 同一コアネタで最大6本展開できるが、**連投は避け間隔を開ける**。
- 切り口を明示するか否かは文脈次第（「結論から言うと」等のリード文は任意）。
- `x-post-writer` の `type` パラメータとは直交する概念。`question` 型でも6切り口それぞれの角度で書ける。
