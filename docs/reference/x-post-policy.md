...
> **背景**: 2026-06-12 凍結後、`publish-x`（Playwright 自動投稿）は停止し、@doboku373 で**完全手動・低頻度**の縮退運用に切り替えた（[01_SNS集客戦略 §凍結縮退](../project/03_SNS/01_SNS集客戦略.md)）。「いつ自動投稿を戻してよいか」を勘で判断すると再凍結を招くため、復帰を機械的・観測ベースのゲートにする。**このチェックリストは §11.2/11.5 の運用値を変更しない**（頻度・間隔・機械ゲートの数値はそれぞれの節が真実源）。復帰は「フットプリントを増やす前に安全を実証する」順序で段階化する。
