...
- **問1 経験記述（1級・2級 共通）**: 詳細な「書き方ヒント」「解答例テーブル」は**置かない**（経験記述は希少コンテンツ＝note 過去問模範答案集 `civil-{1,2}-pastexam-essay` が売る価値。サイトで詳細な書き方を配るとカニバる）。`### ポイント`（5要素・失格回避の一般指針）は残し、その直後に `### 経験記述は「自分の答案」が合否を分ける` 導入段落＋記事中CTA `<MagazineCard id="civil-{1,2}-pastexam-essay" utmContent="secondary-r0X-q1" />` を問1直下に配置。メンバーシップ開始後はこのCTA位置を伴走導線に差し替える（`docs/note/1級・2級土木/noteコンテンツ計画.md` §7.3）。**問2以降の択一・穴埋め・用語の客観解は `### 解答例` を維持**（コモディティ＝SEO/AdSense資産、カニバらない）。2026-06-09 方針確立（1級・2級 secondary-r03〜r07 全10ページ適用済）
