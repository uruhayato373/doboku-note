...
616 図: `ok:537 / recrop:28 / recrop-review:28 / rescan:16 / rescan-need-source:7`。うち**公開×掲載（ライブ）= rescan-need-source:7 / recrop:1 / recrop-review:2**（**ライブの rescan は 0＝完結**）。rescan-need-source 7 = bingham-shear-r04（要R4原典）・civil h27-a/h29-b（要別原典）・pe-construction 論文図4（要白書外部）。2026-07-10 に cce 年度別過去問 H26-28 の図12点＋H30/H29差替2点、続けて **H29/H30 の図12点を ok で追加**（+12＝604→616）。
- **recrop-review 28 = concrete-diagnostician 26（`published:false` 著作権凍結ドラフト）＋ライブ2**。ドラフト26 は図クロップ著作権方針の決定待ちで保留。**ライブ2 = concrete-chief-engineer の `pump-longdistance-h27`（下端に軽微な断片・要 tighter recrop）・`xbar-control-chart-h28`（管理図・実は完全クリーンだが text密度でauto誤検知）** ＝ H29/H30 census 再生成時に auto ヒューリスティックが ok→recrop-review へ再判定（答え漏らしはなし・要人手フォロー、`docs/todo/backlog.md`「過去問図の品質」に追記）。（2026-07-09 に手作業＋並列workflow 4本で写り込み除去クロップ→親目視QA。civil-1 94→0、他資格 48図処理。）
