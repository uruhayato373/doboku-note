...
> [!note]
> **真実源の責務分担**
> - 本書 = 導線アーキテクチャ（3層モデル・原則・見直しサイクル）の SSOT
...
6. **ソース→ライブの非同期** — `article.md` 編集は次回 publish／更新まで live note に反映されない。公開済み記事への反映は `publish-note --update`（本文再 paste は 1 エディタ 1 回制約に注意）。
