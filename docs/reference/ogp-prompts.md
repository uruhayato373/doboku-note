...
| 装飾要素 | accentLight 16px 外枠（資格テーマ色）のみ。地は radial グラデ（左上寄り）。※2026-06-29: ①左上の四角は **linear-gradient(135deg) の原点コーナーを satori がブロック状に塗るアーティファクト**が原因→地を **radial-gradient に変更**して解消（grid ではなかった）。②全面グリッドは satori が当該 backgroundImage を描画しない no-op だったためダークから撤去（無影響・dead code 整理。ライト/note カバーは継続）。③旧・左上の浮いたシアンバー（80×4）も同日撤去 |
...
4. **装飾は全幅 OK**: 16px 外枠（とライト/note カバーのグリッド）は外周まで延びる。意味のある情報は枠内 72px パディング内に置く
