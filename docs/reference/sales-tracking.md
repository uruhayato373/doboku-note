...
| パターン | 例 |
|---|---|
| ココナラ 単発サービス | `coconala:<id>`（id は `src/lib/coconala-services.ts` の id と**完全一致**。例: `coconala:coconala-shindan`（¥1,500）/ `coconala:coconala-tensaku-set`（¥6,000）/ `coconala:coconala-bunseki-pdf`（¥2,500・出題分析PDF）/ `coconala:coconala-kanseitoan-pdf`（¥3,500・完成答案集PDF）） |
