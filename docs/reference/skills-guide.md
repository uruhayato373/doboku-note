...
| `/note-edit-magazine` | note 有料マガジン設定（タイトル/説明/アピール/価格）と収録記事の単品価格を `_meta.yaml` 駆動で自動編集→保存（Playwright × システム Chrome）。文字数制限ガード（アピール≤250字）・読み戻し照合・保存後 API 検証。dry-run 必須。読取は `npm run verify-note-magazines`、入口は `npm run note-edit-session` | `noteマガジン編集`, `note価格変更`, `noteタイトル変更`, `/note-edit-magazine --key {key} --meta {_meta.yaml}` |
