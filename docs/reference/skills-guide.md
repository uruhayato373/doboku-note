...
| `/publish-ig-bs` | Playwright × Meta Business Suite で IG カルーセルを**予約投稿**（IG 単独化・spinbutton 時刻・dry-run 必須）。即時は `scripts/publish-ig.mjs`（Graph API） | `IG予約投稿`, `インスタ予約`, `Business Suite 投稿`, `/publish-ig-bs` |
