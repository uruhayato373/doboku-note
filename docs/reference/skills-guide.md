...
| `/keiken-tensaku` | 経験記述の顧客対応ドラフト生成（`civil-keiken-tensaku-drafter`）。①添削（既定・提出原稿→赤入れ）②作成（`--mode sakusei`・ココナラ S3・ヒアリング→答案ドラフト）。捏造禁止（回答にない事実を作らない） | `経験記述を添削`, `添削下書きを作成`, `経験記述の答案作成`, `ヒアリングから答案`, `/keiken-tensaku` |
