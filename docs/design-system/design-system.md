...
| `HubCtaBanner`（`ui/HubCtaBanner/HubCtaBanner.tsx`） | カテゴリ hub 本文に置く**資格別リッチ背景 note CTA**（NextStepNav が指す「季節 note CTA」の実体）。背景は資格ごと 1 枚 `public/images/cta-bg/*.webp` を使い回し、文言・価格は**画像に焼かず HTML 文字を左に重ねる**（データ駆動で保守）。**季節モード**: 直前期（試験日の 6 週間前〜試験日）は売れ筋の特定商品へ直リンク（`mode=product`・価格ピル）、それ以外は資格別 **L2「もくじ」へ集約**（`mode=mokuji`・マガジンが増えても config 追加不要でスケール）。解決は `src/lib/hub-cta.ts`（`resolveHubCta(category)`・ビルド時 `Date.now()` で switch）。もくじ URL は **`.claude/config/note-funnel.json` の L2 レジストリと同一 note 永続 ID**（変更時は両方更新）。GA4 は `data-cta="note"`＋`data-cta-label`（`category-{slug}-hub-{seasonal\|mokuji}`）、色は `--exam-*` トークン | `cta`（`ResolvedHubCta`） |
| `MagazineTopBanner`（`ui/MagazineTopBanner/`） | docs 記事**冒頭**（ArticleHeader と本文 prose の間）に置く 1 行テキスト note CTA（バッジ＋短縮タイトル＋価格＋矢印）。二次系の高 intent ページのみ `resolvePlacement().top`（`src/lib/magazine-placement.ts`）で設定され、記事が長いため冒頭にも到達導線を 1 本置く。**末尾の画像カード（`SidebarMagazineList`＝`footerMagazines`）と別物・重複可**（冒頭=テキスト／末尾=画像で形が違う）。表示可否は `getMagazine()`（published＋noteUrl）ゲート通過で決まり未公開は自動非表示。GA4 は `data-cta="note"`＋`data-cta-label`（utm_content） | `url`/`title`/`price?`/`badge`/`trackLabel` |
...
1. **通常フロー（追従させない）**: 転職アフィリ枠（最上部・唯一のピクセル源）→ note CTA（`SidebarMagazineList`・転職枠直下に 1 枚、TOC 非表示 docGroup は 2 枚）→ 運営者プロフィール。広告・著者は追従させない（「広告が追いかけてくる」体験を避ける）。
...
読書体験を壊さないための収益要素（第三者広告＝転職アフィリ／自社 note CTA）の上限。第三者広告と自社 note CTA を区別する。機械ゲート = `npm run check-cta-density`（ビルド後 `out/docs/**` の `data-cta` 属性を集計し閾値超過で exit 1）。

| ルール | 値 |
|---|---|
| 転職アフィリ計測ピクセル | 1 ページ 1 発火（サイドバー最上部の 1 枠のみ） |
| ファーストビュー（PC） | 本文列は `MagazineTopBanner`（テキスト 1 行）まで／サイドバーは 転職 1 + note 1 まで |
| サイドバー note カード | 既定 1 枚（TOC 非表示 docGroup のみ 2 枚） |
| 本文中間 CTA（`MidArticleCta`） | 1 記事 1 個（guide/pillar/textbook・h2≥5・8,000字以上のみ） |
| footer を除く note スロット（top+sidebar+mid） | ≤ 3 |
| 記事末尾 footer カード | ≤ 7 |
| 連続配置 | 第三者広告同士の隣接禁止。自社 note カードの縦連続は 2 枚まで |
| AdSense 自動広告 | コードで除外指定不可 → 管理画面「広告掲載率」を下げる運用＋`google-auto-placed` 出現数を週次監査 |
