...
**機械ゲート** = `npm run check-cta-density`（ビルド後 `out/docs/**` の `data-cta="note"` ラベルを集計）。守る対象は「ファーストビュー〜本文中の能動的な note 押し出し密度」で、デザイン刷新で追加した サイドバー note 再掲・記事内中間 CTA が将来暴走しないための回帰ガード。全 1056 ページで note-non-footer ≤ 3 を確認済み（2026-07 フルビルド検証）。
...
| 転職アフィリ計測ピクセル | 1 ページ 1 発火（サイドバー最上部の 1 枠のみ） | 既存アフィリ配線規律（※PC枠とモバイル記事末は同一 creative のレスポンシブ複製で静的カウント不可のため機械ゲート対象外） |
