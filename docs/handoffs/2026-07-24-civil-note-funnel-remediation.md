...
- 意味監査の副次提案も**全て反映**（「全てやってほしい」）＝「目的から逆引き」に一次軸（1級/2級一次PDF）、「まず選ぶ1冊」に一次の状況別bulletを追加。これらの `<ul>` は `note-append-list-links` で狙えない（逆引き=アンカーID非一意／まず選ぶ1冊=平文非リンク）ため、**H2見出しアンカーで `<ul>` を特定→スコープ限定 insertAdjacentHTML** の一回限りスクリプト（`.tmp/note-mokuji-{gyakubiki,mazu1satsu}.mjs`・無料記事なので paywall 無し）で live 反映。公開API実査で両セクションの追記を確認。これで一次の可視性が もくじ 3箇所（有料教材／逆引き／まず選ぶ1冊）で揃った。
