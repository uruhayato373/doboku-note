...
- `.claude/state/note-republish-hashes.json` — 上記3記事の再公開ハッシュを in-sync 化（+ updatedAt）
...
## 未解決事項（フォローアップ候補・本タスクのスコープ外）

- **土木もくじ→1級一次PDF の逆引きリンク欠落**（意味監査で surface・回遊 2/3 の唯一の減点）。もくじ（`docs/note/1級・2級土木/土木もくじ/article.md` 76-83付近）は 2級一次PDF(`n4963f45bd6f8`)への逆引きはあるが **1級一次PDF(`n155093f42183`)への逆引きが無い**（片道＝記事→もくじのみ）。2級側に倣い1行追加すれば往復が資格間で対称になる。**もくじは本作業票の編集スコープ外**（対象は3記事＋config）のため未実施。実施時はソース追記＋`note-append-list-links` で live 反映（D2手順）。
- それ以外の完了条件はすべて充足。残D5 9件は tankan の別セッション作業（本タスク非対象）。参考: 有料2記事は末尾 `cta:civil-mokuji` が無料/有料で2回出る（無料プレビュー回遊＋有料エリア著者クロージング）。冗長だが役割分担があり意図的。
