...
## 進捗（2026-07-30朝・このMacで実施）

**202件中195件解消・残7件。** 全量ライブ実測（`check-note-attachments.mjs --live`・取得失敗0）で
**実査279 / 充足272 / 不足7** を確認（`.claude/state/note-attachments-missing.json` 更新済み）。

`note-attach-batch.mjs --commit` を100件で実行し **成功98 / 失敗2**。失敗2件（`n94ee119b3cb9` の
工程7遷移 ERR_ABORTED、`n8d72cf82ca72` の300秒タイムアウト）は**全量実測では充足側に出た**ので、
どちらも添付自体は成立していた（done-log には入っていないため翌日の再実行で再公開のみ走る）。

**本日のアップロード枠は消費済み（約100件）。残7件は翌日以降。** 7件はすべて建設部門
必須科目I（R07・R8予想①〜⑥）で、`note-attach-batch.mjs --commit` の再実行で done-log から続く。
...
> [!caution] 他の note ブラウザ作業と同時に走らせない
> `note-sync-tags` など note を触るスクリプトは**同じ Playwright プロファイルを使う**ため、並行実行するとプロファイルロックで片方が落ちる。**必ず順番に**流す。添付は購入者が既に払った対価が届いていない状態なので優先度が高い。（タグ同期のライブ反映は 2026-07-30 に全件完了済み）
