...
| 経路 | 内容 | PR |
|---|---|---|
| API | Cloudflare zone analytics 日次・zone 設定ドリフト週次。Instagram Graph API 週次は **Meta 利用制限で待機**（dispatch 専用・IG 照合は #549 の `verify-ig-status`）。business-direction に 4 指標 | #548 |
...
| 書き込み | `ops-write.yml`（dispatch + repo 由来 plan hash・18 操作）、`scheduled-publish.yml`（X・30 分刻み・頻度ゲート 12 規則・queue hash）、IG 予約投稿は `instagram.publish-bs`（Business Suite Playwright・Graph 版は待機）、Brain 売上取得 | #550 |
...
2. IG: **Meta 利用制限中は Graph API トークンを作らない**。照合と予約投稿は Business Suite セッション（`npm run auth:export -- --service instagram`）で回す。制限が解けたら Graph API Explorer でトークンを発行し `fetch-ig-insights.yml` の cron を戻す
...
書き込み: `npm run ops-write:plan -- --operation <id> --args '{...}'` → hash を確認 → `gh workflow run ops-write.yml ...`（最初は `commit=false`）。順序 **instagram.publish-bs（Business Suite・headless セレクタ確認から）→ note.sync-tags → note.update-body → note.publish → coconala → brain → X 投稿（`commit:false`×3 → 1 日 1 本 2 週間 → 2 本）→ X Articles → KDP（最後）**。
