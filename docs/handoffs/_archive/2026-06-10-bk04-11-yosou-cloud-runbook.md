...
> [!important] 現況（2026-06-17 更新）
> 8科目のうち **6科目は R08-yosou 生成済み**。**残は `power-civil`（dir `BK-09_電力土木`）/ `railway`（dir `BK-10_鉄道`）の2科目のみ**（この2 dir に `R08-yosou` が無い）。
> また §2 表の **BK 番号↔dir は早期計画値で、6/15 のマガジン確定時に再割当された**ため実体とズレていた → 下表は**マガジン実体の dir に全行是正済み**（subject slug とテーマ軸は不変）。cloud 実行時はこの 2 科目だけを正しい dir に生成すればよい。
...
|---|---|---|---|
| 1 | `construction-planning` | BK-06 / `BK-06_施工計画` | II-1: 仮設・土留め／工程管理(ネットワーク)／品質・出来形管理／積算(歩掛・間接費)・施工計画。II-2: i-Construction 2.0・ICT施工の施工計画／建設DX・自動化遠隔化導入。III: 担い手不足下の生産性向上(週休2日)／現場の安全・品質確保 |
| 2 | `geotechnical` | BK-04 / `BK-04_土質及び基礎` | II-1: 締固め／圧密沈下／液状化判定／基礎形式選定(直接・杭)／斜面安定 から4つ。II-2: 軟弱地盤対策(地盤改良)の設計施工／近接施工の地盤対策。III: 大規模地震の液状化・斜面防災／既設基礎の維持管理・耐震補強 |
| 3 | `steel-concrete` | BK-05 / `BK-05_鋼構造及びコンクリート` | II-1: 鋼材の疲労／溶接継手／中性化・塩害／ASR／プレストレス から4つ。II-2: 鋼橋・コンクリート橋の長寿命化修繕・補修補強。III: 橋梁等の予防保全・更新／脱炭素(低炭素材料) |
...
| 5 | `tunnel` | BK-11 / `BK-11_トンネル` | II-1: 山岳(NATM)／シールド／開削／覆工・変状の維持管理 から4つ。II-2: 都市部シールドトンネル／山岳トンネルの地質リスク対応施工計画。III: 道路トンネルの老朽化・長寿命化／防災(火災・避難)・点検DX |
| 6 | `port-airport` | BK-08 / `BK-08_港湾及び空港` | II-1: 防波堤(混成堤)／係船岸(矢板・桟橋)／浚渫埋立／空港舗装 から4つ。II-2: 港湾施設整備・老朽化対策／空港機能向上。III: 地震・津波・高潮への港湾強靱化・BCP／カーボンニュートラルポート |
...
| 8 | `power-civil` | BK-09 / `BK-09_電力土木` | II-1: ダム／水路・水圧管／発電所基礎／送電鉄塔基礎 から4つ。II-2: 水力・揚水発電の土木施設／再エネ(洋上風力基礎等)の土木計画。III: 再エネ拡大に伴う土木インフラ／既設電力土木設備の維持管理・防災 |
