...
> [!note] 後日解決（2026-06-15、本 handoff 退避と同時に親判断）
> - **P1 → 現状維持で確定**: 当該ファイルは個人用・サイト非配信の学習メモ（市販書籍の独自要約）。`docs/note/`（note 公開 SSOT）・`docs/project/`（戦略）・`docs/textbook/`（変換パイプライン用 PDF 原典）のいずれにも該当せず、`docs/study-notes/` が正しいカテゴリ。提案の統合先は誤配置になるため移動せず、削除もしない（著者の学習資産）。
> - **P3 → 解決（誤診の是正＋改名）**: 「二重存在」は誤診。`2026-W21-review.md`＝W21 レビュー、`2026-W21.md`＝weekly-review Phase4 が生成する翌週計画（中身は「週次計画 2026-W22」）で、別物の 2 成果物。計画ファイル生成の運用自体は W21 限りで停止済み。紛らわしいファイル名のみ内容の週に合わせ `2026-W22.md` へ改名（spec の `YYYY-Www.md` 計画パターンに整合）。参照は `-review.md` 側のみで影響なし。
> - **P2 → 据え置き**: 成果物コンテンツ本体（read-only）。`.local/r2/posts/` との関係設計は引き続き別タスク。
