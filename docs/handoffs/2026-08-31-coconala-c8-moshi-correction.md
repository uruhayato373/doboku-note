...
### 5.8. ブログ2本のライブ反映は自動化できなかった（手作業でお願いします）

編集画面のブロック構造を解析し、置換/挿入をブロック単位で当てるスクリプトを書いて試したが、
**3 回とも構造が壊れた**。ココナラのエディタは段落挿入の Enter で直後の H2 ブロックを複製し、
`## 技術的課題は…` という生の記法を含むブロックを増やす。置換を先に済ませてから挿入する
順序に変えても同じだった。

**保存前検証（エディタ内のブロック列が期待と完全一致するか）が 3 回とも止めたので、
ライブ記事は一度も壊れていない**（実査で確認済み）。「下書き保存」も押していない。

→ **§6 の貼り替えシートで手作業をお願いします。** 置換は既存段落を選んで貼り替えるだけなので
Enter を伴わず、この問題は起きない。
