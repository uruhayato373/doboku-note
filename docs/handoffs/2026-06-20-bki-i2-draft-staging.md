...
> [!done]
> **2026-06-20 全自動カットオーバー完了**: BK-I R03/04/06/07 の「I-1・I-2 両収録版」を Playwright で全自動入替。①ドラフトstaging（/new paste・書式DOM検証）→②公開（有料¥780・境界検証・API実体検証）→③マガジン `m0f3bc3933454` へ追加→④旧4本をマガジンから削除→⑤frontmatter新IDへ書戻し。マガジン=11件（新4 in/旧4 out・重複解消）。**旧4本の非公開化のみペンディング**（note 仕様で販売実績/有料マガジン履歴のある有料記事は「下書きに戻す」不可・残るは不可逆削除のみ＝販売履歴ゆえ保留。マガジン外＋frontmatter新ID＝孤児化で実害最小）。

> [!note]
> **note プラットフォーム制約（実機確認 2026-06-20）**: 「下書きに戻す」（可逆な非公開化）は **有料記事／過去に販売実績がある記事／有料・定期購読マガジンに追加したことがある記事には使えない**（note がダイアログでブロック）。これらを公開停止する唯一の手段は**不可逆「削除」**。販売中商品ラインの旧版を消すには削除しかなく、ツールのギャップではなく仕様。差し替え時は「新規公開＋マガジン入替」で実質置換し、旧版は孤児化（マガジン外）で運用するのが現実解。
