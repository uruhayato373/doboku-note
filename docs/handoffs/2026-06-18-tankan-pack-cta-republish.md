...
- **P1 ペルソナ模範論文92本**: 無料プレビュー(## 試験問題 直前)へ完全パック挿入。**ソース commit `04d7b9835`**（note-lint 92記事OK）。**ライブは note-append-cta --after <personaマガジンkey> --boundary-h2 '試験問題|予想問題' で背景バッチ実行**（1本=河川コンサルR05で price500/can_read False/有料5630/完全✓ 検証済）。anchor=各記事のpersonaマガジンkeyを source から自動導出（.tmp/p1-live-driver.mjs）
- **R8予想6本**: 末尾を完全パック＋もくじへ置換済（commit 76c4fb540・ソース）
- **P2 構成要素**: 無料序章(クロストレードオフ序章 ndb524ed63c92/設問3 00-序章 n3eb135ebdff7)はライブ末尾追記で対応可。**有料本体(型5/設問3 11/精読5)は試験問題型の境界が無く --boundary-h2 が効かない→ライブ自動更新は事故リスクで保留**（手動 or 境界個別確認が必要）。
