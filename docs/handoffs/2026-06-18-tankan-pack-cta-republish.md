...
### P2 構成要素（保留）
- **無料序章(クロストレードオフ序章 ndb524ed63c92／設問3 00-序章 n3eb135ebdff7)**: ライブは実は ¥100・有料残0（全表示）。`## 試験問題` 境界が無く、無理に境界設定すると現状無料表示の本文を有料化する逆リスク→**安全中断・保留**。
- **有料本体(型5/設問3 11/精読5＝21本)**: 試験問題型の境界が無く --boundary-h2 が効かない→ライブ自動更新は事故リスクで保留（手動 or 境界個別確認が必要）。
- いずれもソース・ライブとも未変更（安全）。完全パック導線の中核（P1 92・P3 18・R8 6）は反映済みで、P2 は残タスク。
