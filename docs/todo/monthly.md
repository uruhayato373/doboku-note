...
### 技術士総監 primary の解答・解説が過密 — 計算式・試験対策ポイントを整理（🟡 次月以降）
...
**r07-primary の確認値**:
- KaTeX 数式ブロック: **3式**（損益分岐点の計算手順を KaTeX で全展開）
- 計算導入文（「基本公式で解く」等）: **5件**
...
$$\text{限界利益} = 1{,}000 - 400 = 600\text{円}/\text{個}$$
$$\text{限界利益率} = \frac{600}{1{,}000} = 0.6$$
$$\text{損益分岐点売上高} = \frac{384{,}000}{0.6} = 640{,}000\text{円}$$
```

**あるべき姿（過去問 解答・解説）**:
- **正答 + 各選択肢の正誤と 1 行理由** — これだけで十分
- 計算問題は「正答値と計算式の概要」を 1〜2 行のテキストで。KaTeX 展開不要
- ExamPoint は引っかけポイントのある問題のみ（全問配置は過密）
- 詳細な計算手順・試験対策ノウハウは関連キーワードページへ誘導

**対象ページ（KaTeX あり）**:
- `h21-primary` 〜 `h30-primary`（10本）+ `r04-primary`・`r07-primary`（2本）= **計12本**

**対応方針**:
- `past-exam-qa` エージェントで12本を一括監査（ExamPoint 過密・KaTeX 過剰・計算導入文を指摘）
- `past-exam-rewriter` で KaTeX → 1行テキスト圧縮・ExamPoint を引っかけのある問題のみに絞る
...
**問題**: 問題文の選択肢は `(1) 〜` `(2) 〜` 形式（原典 PDF の表記を引き継いだ半角括弧）で、解説の `<details>` 内は `1. 2. 3. 4.`（markdown 番号リスト）という二重表記になっている。
...
| 資格 | ファイル数 | 該当行数 |
|---|---|---|
| 1級土木 primary | 37本 | 約4,392行 |
| 2級土木 primary | 10本 | 約3,071行 |
| 技術士総監・pe-first-stage | 0（別形式） | — |
| **合計** | **47本** | **約7,463行** |
...
^(4) → 4.
```
テーブル内の `| (1) |` 形式は別扱いが必要（単純置換すると壊れる）。

**実装方針**:
- frontmatter を除いたボディ部分のみに正規表現適用（`writeMdxFile` 経由・CRLF 保持）
- テーブル行（`|` で始まる行）はスキップ
- `past-exam-rewriter` エージェントで一括対応 or スクリプト一括変換（7,463行は手動不可）

**`[Codex候補]`**: 変換ルールが明確でバルク処理に向いている。frontmatter 除外・テーブル行スキップ・CRLF 保持の仕様を AGENTS.md に書けば Codex が実施可能。
