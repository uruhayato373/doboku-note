...
### Kindle 出版（KDP）— 択一式過去問集 全資格展開（🟢 次月以降・中長期）

**やりたいこと**: 択一式（一次検定・学科）の過去問を Kindle 電子書籍として KDP（Kindle Direct Publishing）で販売する。note とは独立した収益軸。Amazon 検索経由で新規読者を獲得できる。

**対象資格と既存コンテンツ数**:

| 資格 | 択一ページ数 | 年度範囲 |
|---|---|---|
| 技術士総監（一次） | 50本 | H21〜（50問×年度） |
| 1級土木施工管理技士（一次） | 24本 | H26〜R07（A問題+B問題×12年度） |
| 2級土木施工管理技士（一次） | 10本 | R03〜R07（前期+後期×5年度） |

**KDP 出版の手順（概要）**:

1. **コンテンツ整形** — MDX 過去問 → 印刷・電子書籍向け整形（問題文・選択肢・解説のみ抽出）
2. **年度別 or 資格別まとめ** — 1冊の構成を決める（例: 「2級土木 R03〜R07 一次検定過去問集」1冊、「1級土木 H26〜R07 一次検定過去問集」1冊 等）
3. **EPUB/PDF 生成** — `magazine-to-pdf.mjs` を拡張または別スクリプトで EPUB 生成
4. **KDP 登録・価格設定** — `¥500〜¥980` 程度（note より安くして Amazon 流入を狙う）
5. **サイト・note からの相互誘導** — Amazon 商品リンクを BookCard で配置

**検討事項**:
- **著作権**: 過去問の問題文は試験実施機関が著者。国家試験の過去問は「著作権なし」の扱い（行政情報の公開義務）が一般的だが、資格別に確認が必要
  - 国土交通省系（土木施工管理）→ 国家試験は著作権なし（公開義務）
  - 技術士試験（文部科学省系）→ 同様に公開義務あり
- **解説文の著作権**: 運営者が書いた解説は独自著作物 ✅
- **既存スクリプトの流用**: `scripts/magazine-to-pdf.mjs` の「問題文＋解答」中心 PDF が近い。EPUB 化は追加対応が必要

**優先着手順**:
1. 著作権確認（検索 or 法律確認）
2. 2級土木 R03〜R07 一次検定でパイロット（コンテンツ量が最小・10本）
3. KDP アカウント作成・税務情報登録
4. EPUB 生成スクリプト実装
5. 1級土木・総監へ展開
