...
### 【事故復旧】施工計画 R08予想 3本の PDF 添付が live から消失（2026-07-31）
タグ: [事故復旧]

**購入者が代金を払って受け取るはずの PDF が live に無い**状態。翌日（アップロード上限リセット後）に最優先で復旧する。

| noteId | 添付すべき PDF |
|---|---|
| `nac1ee98b8b8b` | 建設部門-施工計画-R08予想-II-1-予想問題と模範解答.pdf |
| `nfff423285c77` | 建設部門-施工計画-R08予想-II-2-予想問題と模範解答.pdf |
| `n76a35d3c2113` | 建設部門-施工計画-R08予想-III-予想問題と模範解答.pdf |

```bash
node scripts/note-attach-file.mjs --note nac1ee98b8b8b --file "docs/note/技術士建設部門/magazines/BK-06_施工計画/R08-yosou/建設部門-施工計画-R08予想-II-1-予想問題と模範解答.pdf" --commit
```

復旧後に `npm run check-note-attachments:live` で 3 本とも添付 1 件を実測してから完了とする。

**経緯**: note の 1 日 100 件アップロード上限（99 件到達）で添付復元が失敗 →「保存しない」で中断したがエディタ側は添付削除済み → 失敗理由を確かめず再実行し、添付なしを正として上書きした。再発防止は実装済み（`--reattach-pdf` の事前ゲート＋日次上限カウンタ）。
