...
### 添削つきプラン ¥2,980 → ¥4,980 改定（note 仕様で変更手段なし）
タグ: [収益化]

毎週1本添削で **¥745/本**、ココナラ S2（¥3,000/本）と同一役務で4倍差。競合ちゃんさと技師は ¥12,000/2テーマ（累計866件・評価4.97）で高額添削の需要は実証済み。**現在0名なので無痛で直せる**はずだった（真実源 → [noteコンテンツ計画.md](../note/1級・2級土木/noteコンテンツ計画.md) §7 の決定）。

> [!warning] 2026-08-06 実機確認: **note UI に会費の変更手段が無い**
> プラン編集ページで会費は `2,980 円/月` の**静的テキスト**として描画され、入力欄も編集ボタンも
> 存在しない（人数制限 `20人` も同様。両プランで確認・スクショ `.tmp/mplan-probe-*.png`）。
> スクリプトの制約ではなく **note の仕様**で、運営者が UI から直すこともできない。
> `note-membership-plan-edit --price` は会費を黙って skip するので「変えたつもり」に注意。
>
> 残る選択肢は次の2つ。どちらも運営者判断:
> 1. **プランを削除して作り直す**（プラン編集ページに「削除」あり。添削つきは**会員0名**なので
>    課金影響なし。ただしプラン ID が変わるため、外部から ID を指していないかの確認が要る）
> 2. **据え置く**（¥2,980 のまま。受験シーズン後に他プランと併せて再設計）
