...
### ココナラ やみぎく様への見積り送信（16冊 ¥7,500）
タグ: [収益化][ココナラ]

> [!important] 2026-08-05 夜 更新: 見積りは**送信済み・コンビニ払いで購入済み（入金待ち）**
> DM [10051134](https://coconala.com/mypage/direct_message/10051134) で 14:47 に提案送信（¥7,500・購入期限 8/12・完了予定 8/17）→ 開封済み →
> 「提案がコンビニ払いで購入されました。支払完了後トークルームが開きます」の状態。
> **残るのは入金確認と16冊の納品**（入金でトークルームが開く）。納品は運営者作業。
>
> あわせて実施済み（同日夜）:
> - **C8 と C10 の受付を再開**（不在休止16件の例外・`coconala-pause --resume --commit`・`stop_fg=0` を実測確認）。
>   理由＝購入期限 8/12 が不在期間（8/6-8/16）と丸ごと重なり、元サービスが受付休止だと
>   決済・取引導線に影響する可能性を排除できないため。**不在中の新規購入リスクは承知のうえ**
> - 両サービスの購入時メッセージ先頭に不在告知を追加（`coconala-edit --fields purchaseNote --commit`）。
>   未ログインの公開ページで実在を確認済み
> - **注意**: `coconala-edit` の dry-run は「下書きで保存」を探すため、公開済みサービスでは
>   構造的に必ず ok:false になる（保存はされない）。公開済みの編集は `--commit` 前提

C8 予想模試の購入者から「模擬試験を除いた16冊分の見積り」を依頼された（2026-08-05 12:13・トークルーム 18082691）。

- **送信先は DM**（[10051134](https://coconala.com/mypage/direct_message/10051134) の「見積り提案をする」）。トークルームに見積り機能は無い
- 金額 **¥7,500** ＝ フルパック¥10,000 − 購入済み模試¥2,500。分割購入でも合計がパックと同額になる建て付け
- 納品物は C1/C2/C4/C6 の16冊（`.claude/config/coconala/assets/pdf/`）。**添付は5個上限**なので見積りに付けず成約後にトークルームで分割送付
- **不在期間に注意**: 8/6〜16 は納品不可。提案の完了予定日は 8/17 以降にするか、本日中の承認を促す文面を添える
- 成約したら `orders-log` へ2件目として記録（`talkroomId` は新規取引の room ID）

### ココナラ フィッシング DM の違反報告
タグ: [運用基盤]

2026-08-05 11:07、`yuki7392` からロシア語＋PDF添付の DM（[10051752](https://coconala.com/mypage/direct_message/10051752)）。「注文のPDFを開いてください」という典型的な誘導で、土木教材の出品者宛としては文脈が合わない。**PDF は開かず**、DM画面の「違反報告」から報告する。返信不要。
...
4. **（2026-08-04→08-05）施工管理 note 142 本のライブ反映 — 再開可能**

   > [!important] 2026-08-05 夜: **109/116 まで完了・残り5本**
   > 添付なし群50本＋添付あり群3本を反映（day1 29＋27＋53）。残るのは
   > **2級学科記述テーマ別出る順の5本**（`paid-rest-with-attach.txt`）だけ。
   > この5本は同日に PDF を新規添付したためリスト分類が古くなり、`--reattach-pdf` 無しの
   > 全文置換をゲートが拒否した（**ゲートは正常動作**・ライブは無傷）。翌日に1コマンドで終わる:
   > ```
   > node scripts/note-update-body.mjs --list .claude/state/note-republish/paid-rest-with-attach.txt --commit --reattach-pdf
   > ```
   > 本日回さないのは、アップロードが上限100件に迫っており、上限到達＝記事途中で中断が
   > 2026-07-31 の PDF 消失事故の起点だったため。日付が変われば枠はリセットされる。
   > 無料は 25/26（残1＝nd4c5c13ee445・画像CDN確定タイムアウトで2回連続失敗）。
...
### 🔴 会員1名が課金中・FLOW 未配信（2026-08-05・最優先）
タグ: [収益化] [note運用]

**2026-08-05 に初の入会**（通年プラン ¥1,480・note 管理画面で実査）。しかしプラン説明が約束している「月例の予想問題配信」は**1本も配信されていない**。W1 開講目安は7月末〜8月頭（1級二次 10/4 から逆算・[メンバーシップ/README.md](../note/1級・2級土木/メンバーシップ/README.md) 配信カレンダー）で、既に遅延。原稿は11週分すべて完成済み＝**書けていないのではなく公開していないだけ**。

**Claude 側で完了（2026-08-05）**: W1「01_安全管理-公衆災害の防止」＝下書き `n66570efb6d23` ／ W2「02_品質管理-コンクリート品質の確保」＝下書き `nc92c82ac4ea5` を作成済み。

**運営者の手作業が必要（note UI・15分）** — `note-publish` は会員限定(is_limited)公開に非対応のため:
1. 上記2下書きを開き、公開設定で**メンバーシップ限定**を選んで公開
2. 特典マガジン **「経験記述-週次お題ラボ」を新規作成**して2本を収録（＝会員へ自動配信）
3. W2 は**画像1枚が未挿入**（`filechooser 不発`・同一画像2枚目の既知現象）。末尾の著者権威画像を手で追加するか、無しで公開するか判断
4. 公開後 noteId/noteUrl を frontmatter へ writeback（Claude 側で実施可）

**8/17 以降**: W3+W4 を二本立てで公開して追いつき → 以降週1（W5 8/24・W6 8/31・W7 9/7・W8 9/14・W9 9/21・W10 9/24頃・W11 9/28）で 10/4 に間に合う。

**自動化の見込み（未判定）**: 公開ダイアログの偵察スクリプトを [scripts/note-publish-discover.mjs](../../scripts/note-publish-discover.mjs) に用意済み（`coconala-discover.mjs` と同じ読み取り専用の偵察・「公開に進む」まで進めて公開範囲のコントロールを列挙するだけ・保存も公開もしない）。使い方 `node scripts/note-publish-discover.mjs n66570efb6d23`。**まだ実行していないので「会員限定公開は自動化できない」は未確定**（`note-publish` が非対応なのは事実だが、実装可能かは probe 次第）。実装できれば W3 以降の週次配信を自動化でき、不在中に進められる作業になる。収益アカウントの新規 selector なので probe → dry-run → 1本だけ commit → 検証 の段階ゲートを守ること。

### 添削つきプラン ¥2,980 → ¥4,980 改定（0名の今のうち・2026-08-05 決定）
タグ: [収益化]

毎週1本添削で **¥745/本**、ココナラ S2（¥3,000/本）と同一役務で4倍差。競合ちゃんさと技師は ¥12,000/2テーマ（累計866件・評価4.97）で高額添削の需要は実証済み。**現在0名なので無痛で直せる**（真実源 → [noteコンテンツ計画.md](../note/1級・2級土木/noteコンテンツ計画.md) §7 の決定）。

**自動化不可・運営者の手作業**: `note-membership-plan-edit --plan ceacc4bb4574` で実査したところ、**会費が設定済みのプランは静的テキスト表示になり `input[name=price]` が存在しない**（スクリプト自身がこのケースを想定して skip する実装）。note UI でプラン編集の編集モードに入って ¥4,980 へ変更する。変更後は非ログインで `/membership/join` を実査して表示を確認する。

### 会員特典22本の会員限定公開（メンバーシップ未ローンチ・2026-07-24）
タグ: [note運用]

> [!note] 2026-08-05 更新: 会員1名が入会し「未ローンチ・会員0」の前提は失効。上の🔴項目へ引き継ぎ（W1/W2 は下書き作成済み）。判断①「ローンチ時にまとめて配信」は選択済み＝いま配信するフェーズ。
