...
**択一98問＝オリジナル演習問題へ転換（進行中）**: 調査で3つの壁が判明した。
...
- 記事の位置づけ変更（title/seoTitle/description/冒頭文/下書き注記 Callout・`past-questions` タグの扱い）も併せて必要
- 参照実装は `primary-exercise-01`（13問）

**残作業**: 択一8記事のオリジナル化・欠番3問（問48/56/85）の扱い決定・cd-essay の note 公開＋placement 配線・記述式の公開前人手レビュー・サイト公開配線（`categories.json` の `visible`・`home-exam-cards.json`・`category-curriculum.json`・exam-brand トークン・`exam-calendar.json`・OGP 生成・`npm run refresh-indexes`）。
