...
### 総監 R8予想6本の有料エリア内「旧3ペルソナ個別マガジン導線」を完全パック＋もくじへ置換 🟢
...
**内容**: 各記事の**末尾「有料領域」**に旧3ペルソナ個別マガジン導線ブロックが残る。完全パック（`m171222175fac`）＋総監もくじ（`n3ed4c77ceed6`）へ置換。**現状のまま実害なし**（旧リンクは生きている）。

**リスク（高）**: 編集対象が販売中有料商品の**有料エリア**＝ public API で内容確認不可（ペイウォール越し）。Phase U-B で保存する際に**有料境界の保持**（`note-append-cta.mjs --keep-boundary`/`--boundary-h2` 相当の境界再検証ゲート）が必須。空更新＋ペイウォール消失の二重リスク。**一括 bulldoze せず、各記事のエディタ実体を確認しながら境界保持を組んだ慎重セッションで実施する**。
