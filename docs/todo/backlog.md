...
### トップページ下部（note 教材・アフィリ・書籍）のデザイン統一 ✅（PR #275）

**完了（PR #275）**: トップ全体を socialplus 参考の全幅帯構造へ刷新する中で、旧 `max-w-sm` 左寄せの note 教材セクションを**全幅 CTA バンド（`CtaBand`）へ格上げ**。"Premium" 見出しもデザインシステム（eyebrow＋serif）に統一。アフィリ/書籍枠は PR #272 で撤去済みのため本項の残課題は解消。以下は当時の記録（歴史資料）。
