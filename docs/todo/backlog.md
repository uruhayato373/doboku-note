...
### 2級土木 41ページの図版整備（Phase 4）
タグ: [コンテンツ]

1級土木は 2026-08-03 に textbook/guide 全 40 ページへ概念図 105 枚を入れ終えた（audit-svg findings 208・HIGH=0 のまま）。2級（`civil-construction-2`）は 41 ページすべて figure ゼロで未着手。

**対象の切り分け（調査済み）**

- guide 23 本のうち **技術系は 8 本のみ**（earthwork / concrete / foundation / quality / schedule / law / safety / construction-plan）。残り 15 本は career・転職・年収系で図版の対象外。
- primary 10 本・secondary 8 本は過去問系。図は原典クロップの領分なので概念図の対象外。
- したがって **実質の対象は技術系 guide 8 本**。各 100〜124 行と 1級（185〜563 行）より大幅に薄く、H2 が 4 本前後。

**1級図の転用について（そのままコピーは不可）**

論点は 1級と重なるので `.local/r2/posts/civil-construction-1/guide-*/img/figure-*.svg` を土台にできるが、**2級は各 H2 の末尾に必ず `<Callout type="note" title="試験のポイント">` が付く構成**で、1級図の「読み取り方」ボックスと内容が衝突する。例: 2級 土量変化率の Callout は「運搬（ダンプ台数）は L、盛土の仕上がりは C」で、`figure-soil-volume-change` の注記 5 と同一。**転用時は注記ボックスを 2級の Callout と重ならない内容へ書き替えること**（figure-canvas-policy §2.5）。

**進め方**

1 ページずつ「Callout が担う内容」を先に読み、図は散文で表せないもの（対応関係・反復ループ・2軸比較・幾何/空間・散在する数値基準）に限定する。PNG 目視は省略不可（1級では是正 20 件のうち機械監査が拾えたのは 4 件だけ）。
