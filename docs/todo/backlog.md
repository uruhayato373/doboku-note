...
### 公開済み note 記事 約29本のライブ CTA を ¥14,800→¥9,800 に修正 🟡

**発端**: 2026-06-20 に完全パックを ¥14,800→¥9,800 へ改定（実売価格は note 上で ¥9,800 反映済み）。repo の CTA（`<!-- cta:pack-top -->`「完全パック（¥9,800）」）は全42ファイル修正済みだが、**既に公開済みの note 記事 約29本の本文 CTA はライブ上で旧 ¥14,800 のまま**（購入ページ自体は ¥9,800 なので取引破綻ではないが表示不整合）。

**対象**: `docs/note/技術士総監/**/article.md` のうち `noteUrl` 設定済み・本文に cta:pack-top を持つ記事（立場別模範論文の選び方 na030d9cb3060 含む）。

**方法**: publish-note update-mode Phase U-B（Selection 限定削除＋type 再入力）で「完全パック（¥14,800）」→「完全パック（¥9,800）」を1記事ずつ。**空更新事故リスクあり**＝needle 限定・全消去禁止。各記事公開ページで実体検証。
