...
### note 添付 残56本（明日いちばん・購入者が受け取れない）
タグ: [収益化][note運用]

本文でPDFを約束しているのにライブに添付が無い記事の復旧。**136本中94本を 2026-08-11 に添付済み**、
note の1日100件上限で打ち切り。残 41 本。

**2026-08-12 に BK-01 道路の15本を新規公開したため +15 で 残 56 本**（15本とも本文で
印刷用PDFを約束済み・PDF実体もリポジトリに存在）。同日に添付を試みたが、前日 94 件を
消費していたため 2 回とも「ファイルカード未検出」で ABORT した（＝1日100件上限の症状）。
**カバー画像は同じ日に問題なく上がったので、画像とファイルはアップロード枠が別**。
