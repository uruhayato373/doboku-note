...
### NSM（GA4）と GSC の乖離を分離する（3週連続の持ち越し）
タグ: [計測]

GA4 の Organic users は 1,188 → 835 → 753 → **520** と 3 週連続で約 30% ずつ下落する一方、**GSC は clicks・CTR・平均順位のすべてが改善**している。同じ検索流入を測って符号が逆である以上、どちらかが実態を映していない。

仮説は ①GA4 側にボット/自動化トラフィックが混入していて剥がれ落ちている（country=Japan フィルタ後も同じ形で落ちる）②試験期終了の季節性が GA4 に強く出ている、の 2 つ。

**この判定が出ないと W31 以降の全施策の効果測定が保留のまま進まない**。前年同期（2025-08）の GA4/GSC と突き合わせて根拠つきで断定し、結論を `.claude/knowledge/reference/measurement-incidents.md` か週次に記録する。
...
### ~~note 添付 残56本（購入者が受け取れない）~~ → **2026-08-13 完了**
タグ: [収益化][note運用]

2026-08-13 に 41 本＋16 本を添付し、残 0。1日100件上限は日付が変わって解消した。

- 1本（`nb605bd0a573c`）が保存中のボタン探索で ABORT → 単独再実行で成功
- **「残 0」を一度誤読した**: バッチの母集団は 08-11 のスナップショットで、
  08-12 公開の建設部門 16 本が母集団に入っていなかった。実査で採り直して追加添付。
  再発防止として `note-attach-batch` に鮮度警告を入れた（実測日が当日でなければ警告）
...
### ~~ココナラ 出品文面に残る「採点者」表記の掃除~~ → **2026-08-12 完了**
タグ: [収益化][ココナラ][品質]

> [!done] listings 24 / カタログ 2 / サムネ 3 を置換し、**listed 11件へライブ反映**。
> 「※採点者・添削者ではありません」は正しい否定文なので保護（2件残置）。
> 最終実査 13/13・残存 0。

記録: 当初「8箇所ほど」と報告したが実際は **33箇所**（過少報告の訂正）。
さらに**ライブ実査が SoT に無い発生源を暴いた**——全13ページに出ていた
「採点者に伝わる答案へ」は listings ではなく**プロフィール bio**（各サービスページに描画）だった。
同時に bio の「答案の代筆（作成代行）は受験の公正のため行いません」が
**S3/S3+ を実際に売っている実体と正面から矛盾**していたため、Red Line #2 の再定義
（捏造禁止＝本人の実工事のヒアリング文章化は行う）に沿って書き換え、提供中サービス一覧も実体に合わせた。
教訓: **出品文面の掃除は SoT の grep だけでは終わらない**（プロフィールが全ページに載る）。
