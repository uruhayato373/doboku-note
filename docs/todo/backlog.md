...
**前提**: 「NSM（GA4）と GSC の乖離を分離する」は 2026-08-17 に決着した。**季節性で説明がつき計測異常ではない**（試験日と一致・売上も同形で下落）。経緯と恒久ルール → [measurement-incidents.md](../../.claude/knowledge/reference/measurement-incidents.md) 2026-08-17。

残るのは**再発防止**。4 週も持ち越した根本原因は、検証手段に選んだ前年同期比が**実行不可能だった**こと（`.claude/state/metrics/ga4/` の最古が 2026-05-17 で 2025 年分がゼロ）。同じことが来年起きないよう、月次のスナップショットを残す。
...
退役は `docs/sns/x/draft/_archive-*/` へ移すこと（surfacer は `_` 始まりを走査対象から外す）。振り直す場合は見出しの日付を書き換える。

### YouTube pending_overdue 171件の棚卸し（3週連続で拡大）
タグ: [SNS]

`verify-yt-status` の `pending_overdue` が 128 → 149 → **171 件**と拡大の一途。予約投稿の go-live 後処理が回っていない。台帳（`.claude/state/yt-verify/latest.json`）自体も 2026-08-07 生成で 9 日古く、**鮮度も落ちている**ので再生成してから棚卸しする。
...
> [!note] 2026-08-13 に #4 の YouTube 分は解消済み
> 監査は「meta.json 32件が旧ハンドル」と報告したが、実測すると**投稿済みは 7 本**だった
> （meta.json は gitignore の生成物で 193 件は未投稿）。CI（`🔁 Sync YouTube descriptions`）で
> ライブ 6 本の概要欄を是正し、**再実行で差分 0 を実照合**。
> 残件は `pJE0G113lWE`（台帳に videoId があるのに **YouTube に存在しない**＝削除/非公開の疑い）。
> 週次の `verify-yt-status` が拾うはずの状態なので、次回そのレポートを確認すること。
...
| 4 | **送客先の状態検査が X の「計画」だけ** | tweets.md（note 324・ココナラ 12 箇所）・IG（313 ファイル）・note 本文（714 リンク）が未検証。~~YouTube~~ は 2026-08-13 に解消（上の note 参照） |
...
### note ライブ未反映 本文 drift 353 本（2026-07-31 中断・その後増加）
タグ: [運用基盤]

2026-08-16 実測で公開 700 本中 synced 340・**本文 drift 353 本**（ほかタグ drift 137 / メタ 13 / アセット 77）。当初の「352 本中 175 本反映済み」から、その後の編集分が上乗せされて母数ごと増えた。残りは `.tmp/republish-batch2.txt` の 121 行目以降＋第2陣の未実行分。**添付を伴う記事は 1 日 100 件上限があるため日を分ける（4 日規模）**。
...
**死守コア（時間が足りなければこれだけ）**:
1. ~~**完全攻略パック 収録拡充**~~ → **完了**（2026-08-17 実査）。105 本すべて `noteStatus: published`・noteId 付きで**未公開 draft はゼロ**。P1 学科記述 ¥2,480・P3a 暗記ノート ¥980・会員も published。商品側の残作業は無い。なお 2026-07-01 の「¥9,800 パックは実売ゼロ」という転換根拠は**現在は成立しない**（8/3 に ¥9,800 が売れ、単品 ¥1,980 も 7/30・7/31・8/15 に3件。当時の sales-log は7月が18%しか記録されていなかった）。
...
### `check-coconala-orders` の DM「要対応」が恒久 false-positive を出し続けている
タグ: [運用基盤]

毎回「要対応」に挙がる DM は、**4 件とも既に決着している**（2026-08-17 に snapshot を採り直して実体を確認）。

| DM | 実体 |
|---|---|
| 10051134 | やみぎく様の見積り DM。受注→納品→評価まで完了済み |
| 10051752 / 10052267 | フィッシング DM。運営がアカウント制限済みで**返信が構造的に不可能**（本文も「規約違反のため削除されました」に置換） |
| 10075959 | **ココナラ運営スタッフからの出品取り下げ通知**（2026-08-12・4テーマ版）。本文冒頭に「本件についてはメッセージでの返信は行なっていない」と明記された**一方向通知**で、問い合わせフォームへ誘導される |

チェッカが「既読の DM = 人が確認せよ」と保守的に倒す設計なので、決着した DM を落とす術がない。X の 064-067 と同じ**退役していない surfacer が本物の警告を埋もれさせる**構図で、実際にこの 3 件は W33 レビューと W34 計画の両方で「未対応の DM 3 件」として誤って上がった。

- 対処案: `orders-log`（または専用の resolved リスト）に決着済み talkroom/DM ID を持たせ、チェッカ側で除外する。除外した件数は出力する（黙って消すと検査ゼロの偽 PASS になる）
- **機械判定できる形が2つ見つかった**（2026-08-17 実機）。①送信者が「ココナラ 運営スタッフ」＝運営からの一方向通知 ②本文に「返信は行なっていない」または「規約違反のため削除されました」を含む。どちらも**構造的に返信不可**なので、決着リストに手で足す前にこの2条件で落とせる
- ただし**運営通知は落としても読まれる経路を残す**こと。出品取り下げのような重要通知がここにしか来ない（メールは `dobokunotecom` 宛にしか届かず、接続済み Gmail からは見えない）
...
> [!note] 順序は変えない（2026-08-12 に実績順への入替を検討→撤回）
> 実売は PDF 側に偏る（C8 ¥2,500 初受注 8/4 → 同一購入者が ¥7,500 をリピート 8/6・S1/S2 は実売 0）ため
> 実績商品への送客回を先に出す案を検討したが、**公開済み第1回が「次回＝自己採点が成立しない理由」を
> 名指しで予告**しており、各回末尾の予告が 2→3→4 と鎖で連結している。入替はライブの約束を破るので
> 連載順のまま 4 日で出し切る（8/13〜8/16）。実績の非対称は 9月X計画（C8/C11 送客厚め）と
> 第6回以降の選題（実売のある PDF 系を優先）で反映する。
...
**売れ行きを見て検討**:
- ~~**S3 上位版（4テーマ・¥16,000〜）**~~ → **2026-08-12 公開済み**（`coconala-sakusei-4theme`・`https://coconala.com/services/4350199`）。評価を待たず前倒し（売れなくても S3 ¥8,000 の価格アンカーとして機能するため）。売りは倍量ではなく**出題保険**（5管理から2テーマが当日指定）。撤退判定は S3 と合算・受注が入ったら工数を実測（展開キット §6）
