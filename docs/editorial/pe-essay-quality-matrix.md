...
5. **公開判断**: サイトでは公開しない（15 本とも `published: false`）。ペルソナ別フル模範論文は note 有料マガジンの商品で、サイトに載せると有料側が空洞化するため（`content/note/技術士総監/noteコンテンツ計画.md` の三層定義「サイト = 思考フレーム / note 有料 = ペルソナ別フル模範論文」）
