...
- **有料記事 71 本の無料プレビューが下限割れのまま 3 週間公開されていた**（Issue #473・全て 1級土木 完全攻略パック ¥9,800 配下）。中身の判断が要るので機械では直せない。今週の最優先。
...
- 主な変更: 品質ゲートの整備（偽赤 4 件・偽緑 1 件の解消、既存 fail 3 件の解消、マガジン収録ゲート拡張）／CI 失敗の通知経路新設／管理画面 3 タブ／ココナラブログ下書き 10 本／R2 台帳外の一掃（DN-0116 完了）
...
1. **有料記事 71 本の無料プレビューが下限割れ（最優先・要判断）** — 全て 1級土木 完全攻略パック（¥9,800）配下。実測 302〜582 字（中央値 442）で下限 600 字を割っている。ライブ直叩きで確認済み＝検査の誤判定ではない。購入判断の材料が薄いまま 3 週間販売していた。**中身の話なので機械では直せない**。選択肢は（a）71 本の有料境界を下げて再公開（b）下限 600 字（`scripts/lib/note-live-check.mjs:114`）が実態に合っているか見直す（c）理由付きで allowlist。→ Issue #473
