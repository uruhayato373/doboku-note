...
- `docs/design/related-articles.md:3,18`は存在しない`src/lib/related-articles.ts`を真実源としている。実装は`src/lib/related-score.ts`の`rankRelated()`。
...
- `docs/design/speclist-gallery.md`は`grid-template-columns: 38px 1fr`と件数表示を記載するが、実装は`20px 1fr`で件数を描画しない。
...
- `docs/design/callout-gallery.md:218`は旧typeが`note`へフォールバックすると一括記載するが、実装は`warning/caution → warn`、`point → tip`、`error → danger`。
...
2. `docs/design/*.md`は実装から確認できる仕様だけを記載し、履歴・旧移行説明はarchiveまたは短い注記へ移す。
...
- docs/design画像更新の要否を判断し、見た目が変わる場合はgalleryを再生成する。
...
- [ ] `npm run build`
- [ ] docs/designのパス・props・アルゴリズムが実装と一致
