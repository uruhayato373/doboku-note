...
The file pattern ('/ROOT/.local/r2/posts/' <dynamic> | '/ROOT/.local/r2/posts' <dynamic>) matches 18652 files in [project]/
...
- `localContentDirectory = path.join(process.cwd(), '.local', 'r2', 'posts')`
- `findMdxFiles()` が `.local/r2/posts` を再帰走査
...
Turbopack が Server Component import graph 内で `.local/r2/posts/**` の動的参照を広域解析しない形にする。
...
- `.local/r2/posts` はローカル開発・static export build の実体ソース。存在しない環境では R2 fallback が必要。
...
.local/r2/posts/.../article.mdx:123 [unicodeTextInMathMode]
  math: LCC＝C＋...
...
```text
doboku-note の品質改善を、docs/project/04_運営/10_品質改善スプリント_Turbopack_KaTeX_UI.md に沿って実装してください。
