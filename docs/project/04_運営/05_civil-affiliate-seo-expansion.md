...
| 対象 | 配置済 | 書籍 |
|---|---|---|
| `civil` × `textbook` | ⭕ | 合格ガイド（ASIN: 4798176834）|
| `civil` × `secondary` | ⭕ | 二次問題解説集（4886154557）|
| `civil` × **`guide`** | ❌ **未配置** | — |
| `pe` × `keyword` / `guide` / `pastExam` | ⭕ | （省略）|

### 追加設計

| 対象 | 推奨ペア |
|---|---|
| `civil` × `guide`（4ページ） | 合格ガイド（4798176834）+ 一次過去問マスター解説集（要 payload 取得） |
...
- [ ] 1.2.b 1級土木施工管理 第一次検定 問題解説集 2026 年版（地域開発研究所）
- [ ] 1.2.c 1級土木施工管理 第2次検定 徹底解説テキスト&問題集（ナツメ社、経験記述70パターン）
- [ ] 1.2.d 1級土木施工管理技士 第二次検定 テキスト＆過去問題集（CIC）
