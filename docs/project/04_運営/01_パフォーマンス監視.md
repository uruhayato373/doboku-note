...
**ステータス**: 採用・運用開始
**関連**:
- `docs/reference/data-storage-decision.md`（データストレージ判断 ADR）
- `docs/reference/workflows.md`（継続的改善ループ図の真実源）
...
- Must アラート（しきい値違反）は `task-queue.json` に追記、生データは専用ブランチ、分析は docs/ に蓄積
...
| **アクション** | `.claude/state/task-queue.json`（`source: ci:psi-audit` + `dedupe_key: ci:psi-audit:threshold`） | しきい値違反のみ | 人間（即対応要、`docs/project/TODO.md` で一覧） |
...
- **task-queue 追記は対応が必要なときだけ**: 正常値を通知するとノイズになる。`dedupe_key` で同一事象の重複起票を防ぐ
...
日次（自動）
  psi-audit.yml (JST 02:00)
    ...
    ├─ しきい値判定 → 違反あり: task-queue.json に append（dedupe_key で重複防止）+ TODO.md 再生成
    └─ レポート生成 → metrics-data/psi/latest-report.md

週次（人間 + 自動）
  /weekly-review（日曜〜月曜）
    ├─ Agent C2: 過去 7 日の PSI 推移を集計
    ├─ task-queue.json（source: ci:psi-audit）の解消/継続タスクを surface
...
実装
  task-queue のタスクを参照しながら修正 → PR → main merge → 本番反映
  翌日の psi-audit で効果を測定 → 該当タスクを status: done に更新
```

図の真実源は `docs/reference/workflows.md`（変更時はそちらを先に更新）。
...
| workflow | `.github/workflows/psi-audit.yml` | 日次自動実行・task-queue.json への append | — |
...
### 違反を PR に載せる案
- 却下理由: PR は「変更提案」の場。検出した違反は `task-queue.json` への append が本流
