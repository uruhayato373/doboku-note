...
> **季節 × チャネル施策マトリクス**は [27_5チャネル動線設計.md §6](./27_5チャネル動線設計.md) を真実源とする。本セクションは Phase ごとの主要マイルストーンのみを扱う。詳細な実装ロードマップ（UTM 統一・bio link 着地点・Red Line スキル等の Phase 別タスク）は [27 §8](./27_5チャネル動線設計.md) を参照。
...
- [ ] 7 月下旬: **note 売上トラッキング開始**（`.claude/state/note-sales/`）→ [27 §7 弱点 4](./27_5チャネル動線設計.md)
- [ ] 8-9 月: 投稿継続、note E-2/E-3 を YouTube/IG から送客
- [ ] 8 月中旬: **bio link を Linktree 化**（E-1〜E-3 並列のため）→ [27 §7 弱点 2](./27_5チャネル動線設計.md)
- [ ] X でも試験直後の所感をリアルタイム発信 → YouTube/IG/note への送客
- [ ] **6 週運用後（8 月中旬）に IG KPI トリガー判定**（Reels 比率引き上げ/縮小/現状維持）→ [27 §6 / §7 弱点 5](./27_5チャネル動線設計.md)
