...
| レイヤー | 主役割 | 主収益 | 訴求トーン |
|---|---|---|---|
| **doboku-note Web** | SEO 入口・体系参照・無料コンテンツ | AdSense + アフィリエイト + iOS DL CV | リファレンス（中立的） |
...
既存 [02_チャネル動線設計.md §4](./../03_SNS/02_チャネル動線設計.md) のフォーマット表に以下を追加：
...
- `.claude/config/utm-templates.json`（[02_チャネル動線設計.md](./../03_SNS/02_チャネル動線設計.md) §4 で計画中）に iOS 行を追加
...
3. ⏸️ [02_チャネル動線設計.md](./../03_SNS/02_チャネル動線設計.md) の UTM 統一表に iOS 行を追加（実装フェーズ近接時に同期）
