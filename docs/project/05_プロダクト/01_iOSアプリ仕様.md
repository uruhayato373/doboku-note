...
> **v2 (2026-05-19)** — 仕様/設計フェーズ先行整備。着手判断（Web 月収 ¥15k 達成）までは実装に入らず、本仕様書 + [画面設計](./02_iOS画面設計.md) + [データパイプライン契約](./03_iOSデータパイプライン.md) + [エコシステム動線](./04_iOSエコシステム動線.md) の 4 ドキュメントを完成させる方針に再構成。
> 主な変更点: ①開発フェーズ表を相対日付（T+N）化、②キーワード件数を実体に整合（649 → 657）、③次リリース目標を 2027-07 試験に再ベース、④筆記合格済みステータスを反映、⑤画面設計・データ契約・3 層エコシステム動線を別ドキュメントに分離。
...
| **0-3. 画面設計** | 1-2 週 | [02_iOS画面設計.md](./02_iOS画面設計.md) 確定（主要 8 画面ワイヤー + 画面遷移図 + Free/Premium UI 挙動） | ✅ v1 (2026-05-19) |
...
| **0-6. アプリ名確定** | — | A〜F から user 選定、商標調査、bundle ID 決定 | ⏸️ Phase 1 直前まで保留 |
