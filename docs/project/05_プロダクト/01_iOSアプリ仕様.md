...
> **v2 (2026-05-19)** — 仕様/設計フェーズ先行整備。着手判断（Web 月収 ¥15k 達成）までは実装に入らず、本仕様書 + [画面設計](./02_iOS画面設計.md) + [データパイプライン契約](./03_iOSデータパイプライン.md) の 3 ドキュメントを完成させる方針に再構成。
> 主な変更点: ①開発フェーズ表を相対日付（T+N）化、②キーワード件数を実体に整合（649 → 657）、③次リリース目標を 2027-07 試験に再ベース、④筆記合格済みステータスを反映、⑤画面設計・データ契約を別ドキュメントに分離。
...
- **着手条件**（[戦略 §7](./../01_戦略/04_収益化戦略.md) より）: ①Web 月間収益 ≥ ¥15,000 ②筆記合格発表済み
...
- **緊急ケース**: Web ¥15k 早期達成 + 2026-07 試験前 6 週間以上の猶予があれば、2026-07 試験前リリースを再判断（現状は前提に置かない）
...
|---|---|---|---|
| **0-1. 戦略整合** | 完了 | [04_収益化戦略.md §7](./../01_戦略/04_収益化戦略.md) と整合した価格・スコープ確定 | ✅ v3.1 / 本仕様書 v2 で確定 |
| **0-2. 仕様書 v2** | 完了 | 本ドキュメント v2 確定 | ✅ 2026-05-19 |
| **0-3. 画面設計** | 1-2 週 | [02_iOS画面設計.md](./02_iOS画面設計.md) 確定（主要 8 画面ワイヤー + 画面遷移図 + Free/Premium UI 挙動） | 🔄 進行中 |
| **0-4. データパイプライン契約** | 1 週 | [03_iOSデータパイプライン.md](./03_iOSデータパイプライン.md) 確定（JSON schema + version 規約 + delta fetch） | 🔄 進行中 |
| **0-5. アプリ名確定** | — | A〜F から user 選定、商標調査、bundle ID 決定 | ⏸️ Phase 1 直前まで保留 |
...
1. ✅ **本仕様書 v2 確定**（2026-05-19）
2. 🔄 **[02_iOS画面設計.md](./02_iOS画面設計.md) 作成**（ワイヤーフレーム + 画面遷移 + Free/Premium UI 挙動）
3. 🔄 **[03_iOSデータパイプライン.md](./03_iOSデータパイプライン.md) 作成**（JSON schema 契約 + delta fetch 規約）
4. ⏸️ **アイコンデザイン方針確定**（Phase 0-3 画面設計と並走）
