...
**進捗・追跡**:
- **進捗の真実源（機械データ）**: `.claude/state/exam-keyword-cycles/progress.json` — 年度 × 設問のカバー状況を `/exam-keyword-cycle` スキルが読み書きする
- **人間用ビュー（年度別まとめ）**: `.claude/state/exam-keyword-cycles/umbrella-drafts/*.md` — 年度別の進捗・残設問を散文で確認
- 残タスク（Phase 3 の 3-2/3-3・補強候補）は本ロードマップ §3 / §4 の記述を参照

**関連**:
- 実装スキル: `.claude/skills/content/exam-keyword-cycle/SKILL.md`
- 情報の置き場ルール: `.claude/reference/information-architecture.md`
- 継続改善ループ全体図: `.claude/reference/workflows.md`
- 類似参考（ADR）: `24_パフォーマンス監視アーキテクチャ.md`（PSI 日次監査）

---

> **注**: 本 md は Why / 設計思想 / 中止判定の真実源。**カバー状況は `.claude/state/exam-keyword-cycles/progress.json` が真実源**。情報の置き場ルールは `.claude/reference/information-architecture.md` に従う。
...
**設計思想**:
- PSI 日次監査（ADR #24）と同じアーキテクチャを踏襲。cron + workflow_dispatch + 失敗時は `task-queue.json` に append（`source: ci:exam-keyword-cycle` + `dedupe_key`）
...
残タスク: 3-2 実行頻度確定、3-3 失敗時の `task-queue.json` append 実装、remote trigger 接続。
...
**補強候補**（サイクル横断ダッシュボード・frontmatter cycleHistory・PR レビューコメント学習化）は本ロードマップの残タスク。着手判断時は `.claude/state/task-queue.json` にタスクを登録する。数サイクル蓄積後に優先度判定。
...
**中止時の処理**: `.claude/state/task-queue.json` の関連タスクを `status: done`（または中止理由を `notes` に記載）にし、本 md の「進捗・追跡」行を更新して理由を本節に追記。
