...
- **横断戦略 SSOT**: `1級・2級土木/noteコンテンツ計画.md` — **会員単一商品「土木セコカン 1発合格ラボ」**（買い切り廃止・全コンテンツを会員へ統合）。1 メンバーシップ・2 層: ベース ¥1,480/月・通年・無制限（過去問模範解答全アクセス＋定期予想）＋ 添削 ¥2,980/月・定員 10・シーズン制。設計・移行・級別サブ文書の索引
- **会員ベース層へ移行する旧買い切りコンテンツ**: 1級 完成答案集・2テーマ組合せ大全・過去問模範答案集／2級 完成答案集ほか（**買い切り単品は廃止 → 非公開化＋既存購入者を会員へ移行**。制作履歴は各級プラン文書、移行 backlog は計画 §5.3）
