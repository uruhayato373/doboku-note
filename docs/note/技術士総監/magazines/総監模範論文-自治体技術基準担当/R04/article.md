...
技術士総合技術監理部門 令和4年度 記述式試験（テーマ：DX推進）を、**都道府県**（○○県 技術管理課）**の技術基準担当**（発注者）の立場で **A 案: 設計基準・標準仕様・積算基準の策定改定**（技術標準のナレッジ管理） と **B 案: BIM/CIM・電子納品・技術情報DB の整備**（デジタル技術情報基盤） の 2 つの組織パターンで解いた模範論文です。いずれも県発注工事の技術標準を定め、設計・積算・電子納品の基準を通じて品質を統一的に確保するという、発注者として技術の標準化と継承を担う事務を前提としています。設問の構造は「過去からの変遷 → 現在の DX 取組 → 5 か年 DX 推進計画のタスクフォース設計」であり、各段階で 5 管理間トレードオフを明示します。
...
> R4 は「デジタル技術の利用」と「DX」を明確に区別します。基準類の PDF 化・電子納品の単なる導入は「デジタル技術の利用」であり、設計・積算・施工のプロセスや技術継承のあり方そのものの変革を伴うものが「DX」として扱われます。設問(2)では「DX」として記述し、設問(3)では来年度から5か年の推進計画を自らタスクフォースリーダーとして策定する設定になっています。
