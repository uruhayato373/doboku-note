...
技術士総合技術監理部門 令和4年度 記述式試験（テーマ：DX推進）を、**建設コンサルタント会社の道路・橋梁設計部門**（受注者）の**プロジェクトマネージャー**の立場で **A 案: 橋梁点検・補修設計業務**（既存施設管理型） と **B 案: 道路設計・4車線化検討業務**（改良型） の 2 つの業務パターンで解いた模範論文です。設問の構造は「過去からの変遷 → 現在の DX 取組 → 5 か年 DX 推進計画のタスクフォース設計」であり、各段階で 5 管理間トレードオフを明示します。
...
> R4 は「デジタル技術の利用」と「DX」を明確に区別します。CAD のデジタル化や点検データの電子化は「デジタル技術の利用」であり、コンサルとしての**サービスモデル・提供価値・業務プロセスの変革**を伴うものが「DX」として扱われます。設問(2)では「DX」として記述し、設問(3)では来年度から5か年の推進計画を自らタスクフォースリーダーとして策定する設定になっています。
