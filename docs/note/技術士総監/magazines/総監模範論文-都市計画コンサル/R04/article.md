...
技術士総合技術監理部門 令和4年度 記述式試験（テーマ：DX推進）を、**建設コンサルタント会社**（受注者）**の都市計画・まちづくり部門**のプロジェクトマネージャーの立場で **A 案: 立地適正化計画策定支援業務**（計画策定型） と **B 案: 市街地整備計画・再開発基本計画業務**（整備型） の 2 つの業務パターンで解いた模範論文です。設問の構造は「過去からの変遷 → 現在の DX 取組 → 5 か年 DX 推進計画のタスクフォース設計」であり、各段階で 5 管理間トレードオフを明示します。

> **本記事の構成**: 前半（A 案）は立地適正化計画策定支援業務版の完全模範論文、後半（B 案）は市街地整備計画業務版の完全模範論文を併記しています。計画系（将来構想・区域設定）の業務経験が中心なら A 案、整備系（土地区画整理・再開発）の業務経験が中心なら B 案を本番選択の基準にしてください。

> R4 は「デジタル技術の利用」と「DX」を明確に区別します。GIS や CAD の導入は「デジタル技術の利用」であり、都市計画の意思決定プロセスそのものや市民参加の仕組みを変革するものが「DX」として扱われます。設問(2)では「DX」として記述し、設問(3)では来年度から5か年の推進計画を自らタスクフォースリーダーとして策定する設定になっています。
