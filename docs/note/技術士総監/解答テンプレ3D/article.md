...
![図 1: 3D マトリクス：テーマ × 5 管理 × ペルソナ = 300 セル](./img/figure-01-3d-matrix.png)
...
本書は 20 テーマ × 5 管理 × 3 ペルソナ = 300 セルをカバーする逆引き辞書として設計されています。テーマが決まり、自分のペルソナが決まり、主役管理が決まれば、論文骨子の 8 割が埋まります。
...
本書の 3 ペルソナ（ゼネコン / 河川コンサル / 道路発注者）のうち、自分が最も近いものを確認します。これは試験前に決めておくものです。試験当日に悩まないこと。
...
**管理対象設定の選択肢**:
- 選択肢 A: 河川環境計画における生物生息環境の保全設計（コンサル向け）
- 選択肢 B: 発注者としての環境配慮設計基準の整備と業者指導（発注者向け）
...
- 選択肢 C: 多言語対応ノウハウの形式知化と組織展開（コンサル向け）
...
- 選択肢 B: 環境関連法令（環境影響評価法・自然公園法等）の業務適用（コンサル向け）
...
- 選択肢 C: 環境影響評価の住民意見手続きと事業修正の管理（発注者向け）
...
論文を書くとき、「どのトレードオフを中核に据えるか」に悩む受験者は多い。その悩みの根本は、業務経験が「自分の言葉」に変換されていないことにある。本章では 3 つのペルソナ（ゼネコン土木支店、河川コンサル、道路発注者）ごとに、自業務から論点を掘り起こすための質問プロンプトと推奨フローを提示する。自分のペルソナに対応するシートを使い、論文を書く前に「自分の論点リスト」を埋めることが、本章のゴールである。
...
- [3 ペルソナ別 模範論文ハブ](https://doboku-note.com/docs/pe-comprehensive-management-pattern-essay-river-consultant) — 本書第 4 章の業務別実例

### doboku-note 有料コンテンツ（実戦適用）

本書と組み合わせるとさらに効果的:

- **国土交通白書 R7 完全対応集**（無料公開） — 本書第 2 章 20 テンプレの「テーマ×政策」を白書原典で深掘り。https://note.com/dobokunote/n/n60efbccd728b
- **3 ペルソナ別模範論文 R03-R07**（¥1,980-2,480 × 3） — 本書第 4 章の業務別論点を実例で確認
...
2. **ペルソナ確認 5 分**: 自分のペルソナで該当セルを思い出す（第 4 章 3 ペルソナ）
...
本書（300 セル逆引き辞書）と組み合わせて使うと効果的な doboku-note 公式記事を紹介します。すべて note の同じ著者から公開されており、3D マトリクスの周辺装備として設計されています。

### 試験当日に「型」が出てこなくなったときの保険

ペルソナ別フル模範論文。本書が「骨子」を提供するのに対し、模範論文は「完成形」を 3,000 字級で示します。自分のペルソナに最も近い 1 セットを試験 1 ヶ月前までに通読しておくと、本書の各セルを「論文として展開する語彙」が自然に身につきます。

ゼネコン視点（土木支店工事部長）— 「安全 × 経済性 × 人的資源」が主軸、R03〜R07 の 5 年分

https://note.com/dobokunote/m/m32aaa137f22e

河川コンサル視点（河川・砂防部門部長）— 「社会環境 × 安全 × 情報」が主軸、R03〜R07 の 5 年分

https://note.com/dobokunote/m/m32132ecb3033

自治体 道路担当の視点（発注者）— 「経済性 × 安全 × 社会環境」が主軸、R03〜R07 + R08 予想 2 本の計 6 記事

https://note.com/dobokunote/m/m52186ffd12ca

### 5 管理の体系理解を深めたいとき

総監テキスト精読ガイド。本書の「5 管理軸」をテキスト精読レベルで深掘りした約 7 万字。サイト doboku-note の 650+ キーワード解説に直リンク。本書の各セルで「この管理の頻出論点が思い出せない」となったときの逆引きインデックスとして機能します。

https://note.com/dobokunote/m/m607bf095b02a

### 白書 R7 の具体数字をテンプレに落とし込みたいとき

国土交通白書 R7 完全対応集（**無料公開中**）。7 大政策テーマ × 5 管理トレードオフ × R08 再出題確率スコア + 過去 9 年連続出題の検証データ + 70 ワークシートを 33,925 字で網羅。本書のテーマ別テンプレ（第 2 章 20 種）の背景知識として、白書原典の数値（特定技能 18.1 万人 / GX 認定 2023-10 / 予防保全 30 年 3 割縮減 等）を仕入れるのに最適です。

https://note.com/dobokunote/n/n60efbccd728b

### R08 で出題される可能性の高い 6 大テーマで実戦演習したいとき

技術士総監｜記述式 R8 予想問題集（6 テーマ別フル模範論文）。本書の 300 セルを「予想問題 → フル解答 → 採点ポイント」まで完成形で確認できる予想問題集。本書とセットで使えば、試験当日に予想問題が的中したときも外れたときも、骨子の組立フローが完全に再現できます。2026 年 6 月 W4 公開予定で、公開後に本記事末尾へリンクを追記します。

---

## おわりに

本書は「読む」教材ではなく「使う」ツールとして設計されています。試験当日、問題文を受け取った瞬間に該当セルを引いて、30 分で 3,000 字級論文の骨子を組み立てる。そのフローを身体に染み込ませることが、本書が提供する最大の価値です。

試験前 2 週間でテーマ別テンプレ（第 2 章）と 5 管理別テンプレ（第 3 章）を流し読みし、自分のペルソナ（第 4 章）を確定し、三層構造（第 5 章）への当てはめ手順を 3 回シミュレートしてください。それで本番では本書を「辞書」として開くだけです。

合格を祈ります。

---

*本書は、運営者が技術士総合技術監理部門に合格した実体験と、過去 9 年の総監記述式試験データの分析をもとに執筆しました。R08（2026 年 7 月実施）受験者の合格の一助となれば幸いです。*
