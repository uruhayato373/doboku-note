...
私自身は現在も総監の受験対策中であり、独立経験はない。以下はあくまで他者が公開した情報の整理であることを最初に断っておく。

![独立技術士5事例の収入推移イメージ。食える独立と食えない独立の分岐点を図示](img/figure-1-income-trajectory.png)
...
完全独立までの事例ではないが、副業段階の数字が可視化されている点で貴重だ。副業技術士として3ヶ月で約16万円という数字は、「始めれば即収入」ではなく「土台作りに時間がかかる」という実態を示している。
...
- 副業段階での月収が固定費（月10〜15万円）を超えているかを確認
- 取引先候補5〜10社を明確にする
- 技術士事務所の開設準備（屋号・青色申告・損害賠償保険）
- **目標**：独立後1年目の収入見込み300〜500万円を根拠ある数字で立てられること

### ステップ4（独立後・1〜5年目）：損益分岐点超えからサラリーマン水準へ

- 独立後1〜2年目：年商350〜500万円（損益分岐点超え）
...
私は元自治体の技術職員で、現在は総監の受験対策中だ。独立・開業は選択肢の一つとして視野にあるが、現時点ではまだ「どう生かすか」を模索している段階だ。
