---
notePricing: free
noteSeries: 総合案内
utmCampaign: 100-content-index
noteUrl: "https://note.com/dobokunote/n/n296a88f64ac2"
noteId: "n296a88f64ac2"
notePublishedAt: "2026-06-16"
noteStatus: published
coverTitle:
  - doboku-note
  - コンテンツ総合案内
cover:
  leadIn: "土木・建設の資格対策を"
  hi: "1ページ"
  hiSuffix: "で迷わず探せる"
  banner: "総合もくじ"
  meta: "無料・更新型"
  chips:
    - { icon: map, text: "資格別もくじ" }
    - { icon: layers, text: "無料→有料の導線" }
    - { icon: target, text: "目的から逆引き" }
---
# 【もくじ】doboku-note コンテンツ総合案内 — 土木・建設の資格対策まるごとマップ

doboku-note をご覧いただきありがとうございます。記事と有料マガジンが増えてきたので、**どこから読めばいいか・何を買えば仕上がるかを1ページで探せる総合もくじ**を用意しました。このページはブックマーク推奨です（更新のたびに追記していきます）。

筆者は自治体土木職（発注者）として勤務し退職、1級土木施工管理技士・技術士（建設部門・総合技術監理部門）ほかを保有しています。**「発注者・合格者の視点」**を軸に、各資格で「無料で全体像をつかむ → 有料で答案・論文を仕上げる」導線でコンテンツを揃えています。

**このページの使い方**

- 受験する資格の見出し（H2）まで飛んでください
- 各資格は「① まず読む（無料）」→「② 仕上げる（有料マガジン）」の順で並んでいます
- 目的から探したい方は、最後の「目的から逆引き」セクションへ
- note アプリでリンクが開きにくいときは、リンクを長押し→「ブラウザで開く」をお試しください

---

## 技術士（総合技術監理部門・総監）

総監は「部門合格後の上位資格」で、択一＋記述式の両方を5管理（経済性・人的資源・情報・安全・社会環境）の横串で攻めるのがコツです。

**総監だけの詳しい案内は [総監もくじ](https://note.com/dobokunote/n/n3ed4c77ceed6) にまとめています**（無料記事＋有料マガジン＋パックを学習段階で逆引き）。

### ① まず読む（無料）

- [【はじめての方へ】R08 合格のための note ロードマップ](https://note.com/dobokunote/n/n3d73729e6cc7) ← 最初の1本
- [総監 二次試験 学習スケジュール完全版（6-7ヶ月を4フェーズ）](https://note.com/dobokunote/n/n6f9854578518)
- [総監キーワード集 2026年の変更点と学び直す優先順位](https://note.com/dobokunote/n/n3923cbfb651b)
- [キーワード集を1周したのに点が取れない3つの理由](https://note.com/dobokunote/n/n14c71f3b4d72)
- [一般部門の合格者が総監で落ちる3つの理由](https://note.com/dobokunote/n/n7fb7f92f7841)
- [択一17年分680問を徹底分析｜分野別の頻出と学習戦略](https://note.com/dobokunote/n/n3bcb87efddad)
- [総監記述式 出題傾向の変遷マップ（H21〜R07）](https://note.com/dobokunote/n/nc360aaa381b0)
- [国土交通白書 R7 完全対応集](https://note.com/dobokunote/n/n60efbccd728b)
- [技術士総監をAIで勉強する（NotebookLM・Claude の使い分け）](https://note.com/dobokunote/n/n89da1120ccaa)
- [令和8年度 総監記述式 R8予想問題（無料版）](https://note.com/dobokunote/n/n8e92e4673a99)
- [設問(3)国家施策バンク 序章（無料）](https://note.com/dobokunote/n/n3eb135ebdff7)

**公務員・発注者の方へ**（クラスター）

- [自治体の技術職員が総監を取る5つのメリット](https://note.com/dobokunote/n/n279ac7c6fe6a)
- [働きながら総監に挑む学習設計（繁忙期を避ける逆算カレンダー）](https://note.com/dobokunote/n/nc7d70c92b8b0)
- [定年後を見据えた資格戦略（再就職・OB活動でどう効くか）](https://note.com/dobokunote/n/nbdfe9446f887)
- [発注者の日常を5管理に翻訳する](https://note.com/dobokunote/n/nd34fa843e977)
- [発注者（自治体）の立場で記述式をどう組み立てるか](https://note.com/dobokunote/n/nced3f11b7641)
- [自治体の技術職員が総監択一でつまずく3つの盲点](https://note.com/dobokunote/n/nb2acfecc2df7)
- [自治体 道路担当が記述式の「お題」をどう選ぶか](https://note.com/dobokunote/n/n6a992fc189c8)
- [自治体の技術職員のキャリアと資格の地図](https://note.com/dobokunote/n/nb052deac97b1)

### ② 仕上げる（有料マガジン）

**コア4**（型・論点・予想）

- [5管理 テキスト精読ガイド（doboku-note 連動）](https://note.com/dobokunote/m/m607bf095b02a)
- [5管理クロス・トレードオフ全網羅（20セル）](https://note.com/dobokunote/m/m921fbe060575)
- [設問(3)国家施策バンク（将来課題11テーマ）](https://note.com/dobokunote/m/m91516dfc27ac)
- [令和8年度 R8予想問題集（出る6テーマ）](https://note.com/dobokunote/m/m6854c7437d4d)

**模範論文**（あなたの立場で選ぶ）

- [ゼネコン版 R3-R7](https://note.com/dobokunote/m/m32aaa137f22e)
- [建設コンサル 河川・砂防 R3-R7](https://note.com/dobokunote/m/m32132ecb3033)
- [道路・橋梁設計コンサル R3-R7＋R8予想](https://note.com/dobokunote/m/m09440aa379cf)
- [都市計画コンサル R3-R7＋R8予想](https://note.com/dobokunote/m/mf0f98993407f)
- 自治体担当者向け（部署別）— [道路](https://note.com/dobokunote/m/m52186ffd12ca)／[河川](https://note.com/dobokunote/m/m32a8a5b3b473)／[砂防](https://note.com/dobokunote/m/m7cd44bf57187)／[都市計画](https://note.com/dobokunote/m/mf8c77e995511)／[契約・調達](https://note.com/dobokunote/m/m55b930cbfcf9)／[技術基準](https://note.com/dobokunote/m/mf9f281e2cb32)／[上水道](https://note.com/dobokunote/m/mf4c6792b4f9c)／[下水道](https://note.com/dobokunote/m/mf1cbc32d53aa)／[港湾](https://note.com/dobokunote/m/mf762f616c065)／[公園緑地](https://note.com/dobokunote/m/m6d1810e50b0a)

**単発**（択一の得点源）

- [総監択一 頻出計算問題 6パターン完全攻略](https://note.com/dobokunote/n/ne190c3ef2fca)

**まとめ買い**（お得）

- [総監記述式 コアパック（型×設問3×R8予想）](https://note.com/dobokunote/m/m6e7de5e4ea3d)
- [総監記述式 完全パック（型×設問3×予想×全ペルソナ＋精読 全部入り）](https://note.com/dobokunote/m/m171222175fac)

> 自分の立場の模範論文を単品で揃えるか、横断で読みたいならパックが割安です。迷ったら **コアパック → 自分の立場の模範論文** の順がおすすめです。

---

## 技術士（建設部門・第二次試験）

元公務員（発注者）の視点で書いた**全選択肢網羅のフル模範解答集**です。合格3科目（道路・河川・都市計画）は「合格者が書いた」解答、それ以外は「発注者が監修した」解答という位置づけです。

**建設部門だけの詳しい案内は [建設部門もくじ](https://note.com/dobokunote/n/n7279ca0d926f) にまとめています**（必須科目I＋選択科目別の模範解答集を一覧）。

### 有料マガジン（科目別・R03-R07＋R8予想）

- [必須科目I 模範解答集（R03-R07＋R8予想）](https://note.com/dobokunote/m/m0f3bc3933454)
- [道路 選択科目 模範解答集](https://note.com/dobokunote/m/m9e825cfd8348)
- [河川・砂防及び海岸・海洋](https://note.com/dobokunote/m/mba17c3f8b894)
- [都市及び地方計画](https://note.com/dobokunote/m/mc8bd949f1f51)
- [土質及び基礎](https://note.com/dobokunote/m/me7ebb48b319e)
- [鋼構造及びコンクリート](https://note.com/dobokunote/m/md38f1de30c31)
- [施工計画・施工設備及び積算](https://note.com/dobokunote/m/m1562f66d9654)
- [建設環境](https://note.com/dobokunote/m/m76f1e545c541)

> 港湾及び空港・電力土木・鉄道・トンネルは順次公開予定です。

---

## 1級・2級土木施工管理技士

第2次検定の山場「施工経験記述」を、**工種×テーマ別の完成答案**と**過去問の年度別模範答案**で攻略します。答案はそのまま写すのではなく、自分の工事概要に合わせて改変する前提の「型」です。

**1級・2級土木だけの詳しい案内は [土木もくじ](https://note.com/dobokunote/n/n4fde0f62dc20) にまとめています**（経験記述の完成答案・過去問模範答案を一覧）。

### ① まず読む（無料）

- [1級 施工経験記述で落ちる答案の共通点](https://note.com/dobokunote/n/nfea4a39cf108)
- [2級 施工経験記述で落ちる答案の共通点](https://note.com/dobokunote/n/na5e045a1c6f8)
- [1級 工事概要の書き方](https://note.com/dobokunote/n/na1f84193571a)
- [2級土木 二次検定の全体像](https://note.com/dobokunote/n/n27455b88bcd5)

無料記事は全16本あります。一覧は[土木もくじ](https://note.com/dobokunote/n/n4fde0f62dc20)から。

### ② 仕上げる（有料マガジン）

**1級土木**

- [施工経験記述 完成答案集（5管理）](https://note.com/dobokunote/m/m150c9db08902)
- [施工経験記述 過去問 模範答案集（R03-R07）](https://note.com/dobokunote/m/m3a578194a0a9)
- [2テーマ組合せ大全（5管理 全10組合せ）](https://note.com/dobokunote/m/m74cfd7c695d6)

**2級土木**

- [施工経験記述 完成答案集（安全・品質・工程）](https://note.com/dobokunote/m/m1881a9578027)
- [施工経験記述 過去問 模範答案集（R03-R07）](https://note.com/dobokunote/m/md3aa0f9a37d7)

---

## コンクリート系（コンクリート主任技師・診断士）

小論文・記述式の模範答案集を準備中です。公開しだいこのもくじに追記します。

---

## 資格を「取った後」の話（横断）

- 土木・建設の資格は「取った後」どう得するか — キャリア×お金マップ（note 公開後にここへ追記）

---

## サイト・SNS

過去問演習・キーワード解説・全文検索は本サイトが充実しています。

- 本サイト（過去問・キーワード解説・全文検索）: https://doboku-note.com/?utm_source=note&utm_medium=referral&utm_campaign=100-content-index
- リンク集（各 SNS・商品への入口）: https://doboku-note.com/links?utm_source=note&utm_medium=referral&utm_campaign=100-content-index

---

## 目的から逆引き

- **まず全体像を知りたい**（総監） → [R08 合格ロードマップ](https://note.com/dobokunote/n/n3d73729e6cc7)
- **択一の点を上げたい**（総監） → [択一17年分分析](https://note.com/dobokunote/n/n3bcb87efddad)＋[計算6パターン](https://note.com/dobokunote/n/ne190c3ef2fca)
- **記述式の型がほしい**（総監） → [コアパック](https://note.com/dobokunote/m/m6e7de5e4ea3d) → 自分の立場の模範論文
- **建設部門2次の模範解答がほしい** → 上の「技術士（建設部門）」の科目別マガジンから該当科目を
- **施工管理技士の経験記述で詰まっている** → [1級](https://note.com/dobokunote/m/m150c9db08902)／[2級](https://note.com/dobokunote/m/m1881a9578027) 完成答案集
- **公務員・発注者の立場で考えたい** → 上の「技術士（総監）」内の公務員・発注者クラスターを

---

最後までご覧いただきありがとうございます。スキ・フォローいただくと更新（新しいマガジン・予想問題）の通知が届きます。質問・リクエストはコメントでお気軽にどうぞ。
