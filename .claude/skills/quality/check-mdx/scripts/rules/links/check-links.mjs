 ...
 *
 * 対象スコープ:
 *   - site: content/site 配下の .mdx（公開コンテンツ。相対リンク [text](/docs/slug)）
 *   - note: content/note 配下の article.md（note ドラフト。絶対 URL・裸 URL・相対リンク）
 *   - all : 両方（既定）
 *
 * 検証内容:
 *   - /docs/{slug} の slug 存在（content/site 由来の有効 slug 集合）
