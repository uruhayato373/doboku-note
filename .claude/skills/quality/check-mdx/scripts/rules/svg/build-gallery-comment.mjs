 ...
 *   SVG デザイン一貫性の継続改善（docs/todo/ で追跡）の進捗を視覚化する。
...
const allFiles = globSync(".local/r2/posts/**/img/*.svg").map((f) =>
  f.replace(/\\/g, "/")
);

function getCategory(path) {
  const m = path.match(/\.local\/r2\/posts\/([^/]+)/);
  return m ? m[1] : "other";
}

function getSlug(path) {
  const m = path.match(/\.local\/r2\/posts\/([^/]+)\/([^/]+)\/img\//);
  if (!m) return null;
  return `${m[1]}-${m[2]}`;
}

function getImgUrl(path) {
  // .local/r2/posts/... → https://storage.doboku-note.com/posts/...
