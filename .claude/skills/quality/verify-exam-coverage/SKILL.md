...
- **MDX パス**: `content/site/pe-comprehensive-management/{slug}/article.mdx` 等
...
- 引数パス → slug 抽出
- slug → MDX パス正規化（`content/site/{category}/{slug}/article.mdx`）
- ファイル存在確認
...
**検索対象パス**:
- 技術士総監: `content/site/pe-comprehensive-management/{h,r}*-primary/article.mdx` と `-secondary/article.mdx`
- 1 級土木: `content/site/civil-construction-1/primary/*.mdx` と `secondary/*.mdx`
...
- 修正は Edit ツールのみ使用（CLAUDE.md の CRLF/LF 保持原則）
- 追記後に `node .claude/scripts/lint-mdx-mobile.mjs <path>` を実行、HIGH ゼロを確認
...
**対象**:
- 技術士総監キーワードページ（`content/site/pe-comprehensive-management/{slug}/article.mdx`）
- 1 級土木キーワード/教科書ページ（`content/site/civil-construction-1/textbook/{slug}.mdx` 等）
...
対象: content/site/pe-comprehensive-management/cash-flow-statement/article.mdx
