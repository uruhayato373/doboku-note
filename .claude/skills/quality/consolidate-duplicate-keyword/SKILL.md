...
```bash
grep -rn "B-slug" src/ content/site/ public/_redirects
```

想定される参照箇所:

1. **`src/config/pe-chapters.json`** — キーワード登録（通常 1 箇所）
2. **`content/site/pe-comprehensive-management/*/article.mdx`**
...
```bash
rm -rf content/site/pe-comprehensive-management/B/
```
