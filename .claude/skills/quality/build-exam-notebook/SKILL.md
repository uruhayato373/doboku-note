...
| ファイル | 役割 |
|---|---|
| `.claude/scripts/build-exam-notebook.mjs` | MDX → MD 変換（JSX 削除、frontmatter 簡略化、U+FFFD 検出） |
| `.claude/scripts/notebooklm-notebook-builder.mjs` | notebook 操作 CLI（find-or-create / list-sources / add-source / bulk-add） |
| `.claude/scripts/notebooklm-cross-query.mjs` | 構築後のクエリ用（既存ラッパー） |
...
- 対象年度の過去問 MDX が `content/site/pe-comprehensive-management/{year}-primary/article.mdx` に存在
...
```bash
node .claude/scripts/notebooklm-notebook-builder.mjs list-sources --notebook "総監一次択一過去問"
```
...
```bash
# 1. 既存 source を削除
node .claude/scripts/notebooklm-notebook-builder.mjs delete-source \
  --notebook "総監一次択一過去問" \
  --title "R07一次択一40問"
...
# 3. 再投入
node .claude/scripts/notebooklm-notebook-builder.mjs add-source \
  --notebook "総監一次択一過去問" \
