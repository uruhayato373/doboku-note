...
3. `content/site/{category}/{slug}/img/` に配置

`content/site` は git 追跡対象かつ `public/posts` からシンボリックリンクされているため、ダウンロード完了時点でローカル開発サーバーにそのまま反映される（別途コピー工程は不要）。
