...
- exit 2 … **検査不成立＝接続できていない**（HTTP 000 または既知の社内プロキシ・ブロック HTML）。合格でも不合格でもない
...
> スクリプトは `--ssl-no-revoke` 付き curl を使い、000 と社内プロキシが返す「ブロックされました」HTML（503）を exit 2（検査不成立）として区別する。
