...
親エージェント（Claude Code 本体）が Opus で計画・判断・統合を担い、サブエージェントは Sonnet で高速・低コストに実行する。これによりコスト効率と判断の質を両立できる。Opus を固定指定するのはサブエージェント側で深い推論が不可欠なケースのみで、原則は親の Opus がレビューする前提で Sonnet に任せる。詳細は CLAUDE.md「ハーネス設計原則」§5。
