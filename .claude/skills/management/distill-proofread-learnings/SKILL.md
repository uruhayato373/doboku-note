...
4. `git diff <開始コミット>..HEAD -- content/site/` で差分を取得
...
- `git log` / `git diff`（`content/site/` 配下の MDX/SVG 変更）
...
1. **git で差分取得**:
   ```bash
   git log --since="<range>" --oneline -- content/site/
   git diff <from>..HEAD -- content/site/
   ```
...
| **採点・リライト議論候補** | 採点ルーブリック（cem-qa 5 軸）・リライト方法論（4 視点 × Phase 対応）の改修候補。議論を要するため **即適用せず判断を仰ぐ** | `.claude/todo/backlog.md` の「🟣 判断待ち」セクションに起票 |
...
- **議論の置き場**: 採点・リライト方法論の改修は即適用せず、`.claude/todo/backlog.md` の「🟣 判断待ち」セクションに1件として起票してユーザーの判断を仰ぐ。タスク・判断の単一正源は `.claude/todo/`（**GitHub Issue は使わない**＝真実源 `.claude/knowledge/reference/information-architecture.md`）
...
**起票先**: `.claude/todo/backlog.md` の「🟣 判断待ち」セクションに1件として記録し、ユーザーの判断を仰ぐ（即適用しない）。
