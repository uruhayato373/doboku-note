...
| **採点・リライト議論候補** | 採点ルーブリック（cem-qa 5 軸）・リライト方法論（4 視点 × Phase 対応）の改修候補。議論を要するため **`docs/project/30_採点ルーブリック方針.md` / `31_リライト方法論方針.md` の「検討中の改善」セクションで議論** | **30 の検討中セクション**（採点側） / **31 の検討中セクション**（リライト側） |

**採点・リライト議論候補の判定基準**:

- **採点側候補**（`30_採点ルーブリック方針.md` で議論）: 採点軸の重み・閾値・新軸追加・既存軸の判定基準改修など
  - 反映先（議論決定後）: `cem-qa.md` + `content-principles.md` + `templates/cem.md` の **3 ファイル同期更新が必要** + `30_採点ルーブリック方針.md` の「合意済み」セクションへ移動
- **リライト側候補**（`31_リライト方法論方針.md` で議論）: 視点タグ追加・Phase 対応の見直し・拡張パターン A-G 改修・NLM 照合プロンプト調整など
  - 反映先（議論決定後）: `exam-keyword-cycle/SKILL.md` ほか該当 SKILL.md の修正 + `31_リライト方法論方針.md` の「合意済み」セクションへ移動
- **真実源を一元化する理由**: 採点とリライトの真実源は `30` / `31` の md に一元化。議論を分散させると「最新版はどれか」が分からなくなる。GitHub Issue は使わない（[information-architecture.md](../../../reference/information-architecture.md)）
...
- ユーザー嗜好: N 件
- ワークフロー改善: N 件
- 採点・リライト議論候補: N 件（30_採点ルーブリック方針 N 件 / 31_リライト方法論方針 N 件）
...
**議論先ドキュメント**: `docs/project/30_採点ルーブリック方針.md`（採点） / `docs/project/31_リライト方法論方針.md`（リライト）のいずれか
**サイクル**: 2 回以上浮上（[サイクル1リンク], [サイクル2リンク]）
**観察**: [両サイクルで共通して見られた現象]
**仮説**: [採点軸の改修案 or リライト方法論の改修案]
**反映先（議論承認後、Claude が同期更新）**:
- 採点側の場合: `.claude/agents/cem-qa.md` + `.claude/content-principles.md` + `templates/cem.md`（3 ファイル同期）+ `docs/project/30_採点ルーブリック方針.md` の「合意済み」へ
- リライト側の場合: `.claude/skills/quality/exam-keyword-cycle/SKILL.md` ほか該当 SKILL.md + `docs/project/31_リライト方法論方針.md` の「合意済み」へ

**議論の進め方**: 観察・仮説・反映先のドラフトを `30_採点ルーブリック方針.md` ／ `31_リライト方法論方針.md` の「検討中の改善」セクションに追記する。GitHub Issue は使わない（[information-architecture.md](../../../reference/information-architecture.md)）。承認後に「合意済み」セクションへ移動し、真実源ファイルを同期更新する。
