...
3. 既存CI計測を優先し、note売上とココナラ分析も確認する。認証済み環境で必要なら `npm run fetch-business-metrics -- --monthly --commit`、`npm run note-sales-fetch -- --month YYYY-MM --commit`、`npm run coconala-analytics -- --append-kpi` を使う。ココナラ30日窓は暦月へ換算しない。noteアクセスは管理画面で対象月・新PV定義を確認し、計測記録として追記する。取得不能・不足は欠測で残す。
4. 全重点資格の集客、学習、商品別販売、運営時間、教材不備を点検する。試験カレンダーと前年同時期を参照し、同じ期間・定義以外の増減を効果と呼ばない。数字から原因を断定せず、対象の本文・商品説明・導線を読む。
