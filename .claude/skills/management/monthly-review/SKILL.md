...
8. 年間の重点はバックログのカードの `[時期:]` が正本（管理画面 計画 ＞ 年間ロードマップ）。今月・来月の行が現状と合っているかを確かめ、ずれていればカードの `[時期:]` を直す。月間計画（monthly.md）の選択タスクは `[時期:]` が今月を含むカードから選ぶ。時期を過ぎて残るカードは `npm run check-backlog-health` の S15、今月〜2か月先で月 12 件を超える月は S16 に出る。S16 が出たら、見出し（重要度）と `[期日:]` を見て後ろの月へ `[時期:]` を散らすか、やらないカードを削除して今月を 8〜12 件に収める。
