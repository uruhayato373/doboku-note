...
**対策**: 太字はリンクテキストの内側に置く: `[**環境基本法第15条**](url)`。`/check-mdx` のチェック項目6に検出パターンを追加済み。
...
**対策**: アンカーリンクではローマ数字を含めず `#1-40` の形式で記述する。`/check-mdx` のチェック項目6 および `/keyword-page` SKILL.md に記載済み。
