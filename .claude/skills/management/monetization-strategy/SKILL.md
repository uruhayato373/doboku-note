...
doboku-note は土木工学に関するドキュメントサイト（Docusaurus）。以下の特性を前提に分析すること:
...
- **技術基盤**: Docusaurus + Cloudflare Pages（サーバーコスト極小）
- **運営**: 個人開発
...
| **アフィリエイト** | 技術書籍（Amazon）、資格試験教材、通信講座 |
...
会話内で出力する（恒久保存が必要な結論は `docs/project/strategy/monetization.md` を更新）。
...
- 将来的なハイブリッドモデル（広告 + アフィリエイト + 教材）を見据える
