...
- **完了 prose 蓄積 ≥ 3（S12）** — 本文の「済み」「完了し」等の完了報告表現が 1 枚に 5 件以上
  ...
  肥大した型）
- **チャネル状態複製の疑い ≥ 3（S13）** — `noteStatus`/`status`/`published` の値や「残N本」が
  カードにそのまま写されている＝SSOT が真実源のはずが本文へ複製され、後で陳腐化する候補
  （`todo-standards.md` §1-2 の対象）
- **期日超過 ≥ 1（S14）** — `[期日:YYYY-MM-DD]` を過ぎたまま残っているカード。当日は hit させない
  （まだ間に合う）。期日は 2026-09-22 まで機械の読み手が無く、過ぎても誰も気づかなかった
  （`schedule-view` は人が月ビューを pull したときだけ出る）。超過したカードは
  「やる／期日を引き直す／落とす」のどれかへ必ず動かす
- **時期超過 ≥ 1（S15）** — `[時期:]` の終わりの月を過ぎたまま残っているカード。年間ロードマップと今月のカードは `[時期:]` だけで決まるので、時期を見直すか完了を記録する
- **月の過積載 ≥ 1（S16）** — 今月〜2か月先に `[時期:]` のカードが目安（月 12 件）を超える月がある。重要度（見出し）と期日を見て、後ろの月へ `[時期:]` を散らすか、やらないものを削除する
