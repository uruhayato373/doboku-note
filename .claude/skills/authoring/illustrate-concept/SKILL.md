...
| 引数 | 必須 | 説明 |
|------|------|------|
| 対象記事のパス | 必須 | `content/site/{slug}/article.mdx` の絶対パスまたは相対パス |

概念を事前指定する引数は無い（Discovery First 方式のため）。「この概念で図を作って」と決め打ちしたい場合は本スキルを使わず、`/create-svg` のルールに従って直接作図する。
...
4. 既存 `content/site/{slug}/img/` を確認し、既存 SVG と重複しない概念に絞る
...
```bash
git add content/site/{category}/{slug}/
...
- 出典 URL は 2 箇所に必ず記録:
  ...
  2. git コミットメッセージ本文
- tmp 画像を `content/site/` 配下に保存することは **絶対禁止**
