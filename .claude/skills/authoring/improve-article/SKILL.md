...
| 引数 | 必須 | 説明 |
|---|---|---|
| `<slug-or-path>` | 必須 | `civil-construction-1-textbook-histogram` のような slug、`/docs/...` URL、または `content/site/.../article.mdx` 絶対パス |
...
1. 引数を MDX ファイルパスに解決する
   - slug が来たら `content/site/**/article.mdx` を Glob で探す（Convention B 優先、見つからなければ Convention A）
...
- **Callout 化されていない試験頻出ポイント** — 過去問で繰り返し問われる数値・区分は `<Callout type="exam" title="頻出論点：...">` でハイライト（全 12 種と使い分けは [`docs/design/callout-gallery.md`](../../../../docs/design/callout-gallery.md) 参照）
...
# guide ページ
/improve-article content/site/civil-construction-1/guide/concrete-key-points.mdx --mode verify
...
- [.claude/skills/quality/quality-cycle/SKILL.md](../../quality/quality-cycle/SKILL.md) — CEM バルク処理（本スキルと補完関係）
- [.claude/knowledge/reference/image-policy.md](../../../knowledge/reference/image-policy.md) — 図/写真の判定フロー
