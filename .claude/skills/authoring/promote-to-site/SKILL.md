...
| Obsidian 記法 | MDX 変換 |
|---|---|
| `[[リンク先]]` | `[リンク先](/docs/slug)` に変換（リンク先を `content/site/` から検索） |
...
`exam-topic` と `tags` から配置先カテゴリ（`content/site/{category}/{slug}/`）を決定する:
...
1. 変換した MDX を `content/site/{category}/{slug}/article.mdx`（Convention B）として `writeMdxFile`（`.claude/scripts/lib/mdx-io.mjs`）経由で書き込む（直接 `writeFileSync` は CRLF 混在を招き pre-commit で reject される）
...
1. 対応する画像ファイルが `content/site/{category}/{slug}/img/` に存在するか確認
2. **存在する場合**:
...
- 画像ファイル自体は `content/site/{category}/{slug}/img/` に配置する必要があります（Obsidian の attachments フォルダから Obsidian vault 同期時にコピー）。この配下は git 追跡対象

### Step 5: ファイル配置確認（オプション）

1. `npm run dev` を実行（`public/posts` → `content/site` のシンボリックリンク経由でローカル配信されるため、別途同期は不要）
2. 変換先ページをブラウザで確認：
...
|---|---|
| ソース | ~/obsidian/{path} |
| 変換先 | content/site/{category}/{slug}/article.mdx |
