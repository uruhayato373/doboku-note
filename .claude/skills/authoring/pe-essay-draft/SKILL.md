...
(3) 枚数: 各施策1枚以内
    ②  有効性・実現性（世界情勢・技術革新・経済財政政策・予算等を含む）
    ③  最も重大な障害と克服策（複数の視点に留意）
特殊制約: 「データ利用可能性に関するリスクは対象外」等があれば追記
```

抽出した制約は MDX の各設問セクションを書く前に必ず照合し、取りこぼしがないか確認する。
...
- `.claude/reference/content-authoring.md` — MDX コンポーネント・リンク系の使い分け（note 記事リンクは `<NoteLink>`）
