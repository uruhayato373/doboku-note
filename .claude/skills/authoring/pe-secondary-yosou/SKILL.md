...
- **テーマの絞り込み**: `content/site/pe-construction/{subject}-exam-themes/article.mdx`（サイトの無料テーマ分析記事）の「出題傾向の分析」「R08予想テーマ表」からテーマを抽出する。これが forecast の予想根拠源。
- **区分ごとのテーマ化**:
...
### Step 0: 準備
1. dir = `content/note/技術士建設部門/magazines/{magazine_id}_{subject名}/R08-yosou/`。無ければ作成。
...
## 参照
- 道路パイロット（完成形・テンプレ）: `content/note/技術士建設部門/magazines/BK-01_道路/R08-yosou/III-1_脱炭素/article.md`
