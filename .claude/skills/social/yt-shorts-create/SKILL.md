...
`content/sns/instagram/{exam}/exam-packs/<year>/pack-NN/reels/` の **IG Reels mp4 から、1080×1920 縦型 30-60 秒の YouTube Shorts を派生生成**する。本スキルは mp4 + thumbnail.png + meta.json の生成まで担当。YouTube への投稿は `post-from-schedule.cjs`（台帳 CI 運用）が担う。
...
2. **対象パックの Reels mp4 が生成済み**（`slide-NN.mp4` は gitignore・再生成可＝手元に無ければ先に `ig-reel-create` で生成すること。`ls` 確認が空でも異常ではない）
   - `ig-reel-create` で生成: `content/sns/instagram/{exam}/exam-packs/<year>/pack-NN/reels/slide-NN.mp4`
   - 必要なファイル: `slide-00.mp4` (cover) / `slide-01.mp4` (problem 1) / `slide-02.mp4` (answer 1) / `slide-09.mp4` (cta)
3. **slide-data.json が存在**: `content/sns/instagram/{exam}/exam-packs/<year>/pack-NN/slide-data.json`
...
| `--out` | - | `content/sns/youtube/<date>-<pack-id>/` | 出力ディレクトリ |
...
```
content/sns/youtube/<date>-<pack-id>/
...
```
content/sns/instagram/{exam}/exam-packs/<year>/pack-NN/reels/
  slide-00.mp4 (cover)        ┐
...
# → content/sns/instagram/cem/exam-packs/<軸>/<year>/pack-<NN>/reels-pp/q<N>/{video.mp4, caption.txt}
```

| | YT（既定） | IG（`--ig-mode`） |
|---|---|---|
| 出力 | `content/sns/youtube/<date>-…/` | `<pack>/reels-pp/q<N>/`（自己完結） |
...
- 出力 dir: `content/sns/youtube/<date>-<year>-pack-<NN>-q<N>/`。タイトルは `--titles` JSON（`{"<year>-pack-<NN>-q<N>": "<full title>"}`）を採用、無ければ `answer.correctText` から機械フォールバック。
...
ls content/sns/instagram/cem/exam-packs/r03/pack-01/reels/slide-{00,01,02,09}.mp4

# 3. YT 派生実行
node .claude/skills/social/yt-shorts-create/scripts/yt-shorts-create.mjs --from-reels r03-pack-01

# 4. 出力確認
ls content/sns/youtube/$(date +%Y-%m-%d)-r03-pack-01/
# 期待: shorts.mp4, thumbnail.png, meta.json

# 5. 動画再生確認
# Windows: start content/sns/youtube/.../shorts.mp4
# macOS:   open content/sns/youtube/.../shorts.mp4
...
- 戦略: [`docs/marketing/01_SNS集客戦略.md`](../../../../docs/marketing/01_SNS集客戦略.md) v7
...
- 親タスク: YT 派生スクリプト + Evaluator（v7 Phase D。.claude/todo/ で追跡）
