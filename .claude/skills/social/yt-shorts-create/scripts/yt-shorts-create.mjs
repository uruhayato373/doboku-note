 ...
 * docs/marketing/01_SNS集客戦略.md v7 を参照。
 *
 * Usage:
 ...
 *
 * 派生フロー (v7):
 *   1. content/sns/instagram/{exam}/exam-packs/<year>/pack-NN/reels/ の
...
function parseCliArgs(argv) {
  ...
  if (values.slug) {
    throw new Error(
      ...
      '詳細: docs/marketing/01_SNS集客戦略.md v7 の §1 基本方針・§2 YouTube Shorts。\n' +
...
export async function createShorts({ category, slug, date, speaker, outDir, reset = false, configOnly = false }) {
  ...
  const docsDir = outDir ?? join('content', 'sns', 'youtube', `${date}-${slug}`);
...
export async function createShortsFromReels({ packId, outDir, examDir = '技術士総監' }) {
  ...
  const reelsDir = join('content', 'sns', 'instagram', examDir, 'exam-packs', year, `pack-${packNum}`, 'reels');
  const slideDataPath = join('content', 'sns', 'instagram', examDir, 'exam-packs', year, `pack-${packNum}`, 'slide-data.json');
