...
ドラフト slug から試験を自動判定し（`-civil1-`→1級 / `-civil2-`→2級 / それ以外→総監）、色とヘッダを切替（真実源 `docs/reference/x-post-policy.md` §7）：
...
総監（既定）は従来挙動と完全互換。1級/2級ドラフトは `x-post-writer` が上記ヘッダ形式で生成する。
