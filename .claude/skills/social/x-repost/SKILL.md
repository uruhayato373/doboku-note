...
#    → .claude/state/x-repost/approved.json が生成される

# 4. ★必須★ 引用RP セレクタを dry-run 検証（投稿せず composer 到達まで）
npx tsx .claude/skills/social/x-repost/x-repost-exec.ts --dry-run
...
ローカルの Claude Code セッションで `/loop` を使う:
...
```bash
touch .claude/state/x-repost/PAUSED      # 停止（exec が即中止）
rm .claude/state/x-repost/PAUSED         # 再開
```
