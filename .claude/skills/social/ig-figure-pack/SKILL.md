...
```
content/sns/instagram/{exam}/keyword-packs/{keyword}/   ← keyword-packs/ サブdir 必須
  carousel/
    img/
      00-cover.svg
...
> **IG 図の品質ゲートは目視であって `svg/audit.mjs` ではない**: audit はサイト図（`content/site/**`）専用で既定スコープが content/sns を除外する。`--file=` で IG 図に当てると navy 帯＋白文字が `P8-dark-bg`（HIGH）で偽陽性になる（IG 図では navy 帯は正規仕様）。重なり検知の `P2-overlap` は MEDIUM=非ブロックなので頼らない。
...
```bash
mkdir -p "content/sns/instagram/{exam}/{keyword}/carousel/img"
```
...
const PACK_DIR = 'content/sns/instagram/{keyword}/carousel/img';
const FIGURE_SRC = 'content/site/{exam}/{slug}/img/figure-1.svg';
...
```
content/sns/instagram/{keyword}/carousel/caption.txt
```
...
1. Windows Explorer で `content/sns/instagram/{keyword}/carousel/img/` を開き → ブラウザで drive.google.com にドラッグ＆ドロップ
...
- **figure-*.svg を Stories / Reels の 9:16 キャンバスに配置する静止画変換** → `.claude/knowledge/reference/sns-image-policy.md §13`（本スキルが生成した 4:5 PNG はそのまま 9:16 中央に配置できる）
...
- 一時出力先として `.tmp/` を使うケースは `.gitignore` 対象なので C:\tmp\ または `content/sns/` 配下に最終成果物を置く
