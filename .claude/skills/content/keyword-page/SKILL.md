...
## 総合技術監理における位置づけ
  ...
  - 文中で関連キーワードに言及する場合は Markdown リンク `[キーワード名](/docs/pe-comprehensive-management-{slug})` を使用する
  - `<RelatedKeywords>` コンポーネントは文中のインラインリンクには使わない（ブロックレベルUIのため）
...
- **表が不適切な場面**: 冒頭セクションと重複するキーバリュー表（正式名称・目的・所管・適用範囲など）。これらは散文として冒頭に統合し、読者に同じ情報を2回読ませない
...
| 参考資料 | 最低1件の官公庁資料を含む。適切な公的資料が存在しない場合はセクション自体を省略する（不適切なリンクを無理に入れない） |
