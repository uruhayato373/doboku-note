...
- `pe-chapters.json` の duplicate 検出（`scripts/find-duplicate-keywords.mjs`）
- 7 フェーズの標準作業手順
- 過去の統合履歴と経験則警告
...
- 長い表はモバイル視認性のため箇条書きに変換（`.claude/reference/content-authoring.md` の「モバイル視認性（詳細ルール）」参照）
...
| 連携先 | 役割 |
|---|---|
| **`scripts/find-duplicate-keywords.mjs`** | Step 0 の候補検出 |
...
- `scripts/find-duplicate-keywords.mjs` — 重複検出（本スキルの入口）
