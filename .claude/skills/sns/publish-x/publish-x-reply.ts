 ...
 *
 * 入力:
 *   docs/sns-drafts/<draft-id>/x/captions/<key>-reply.txt
 *   docs/sns-drafts/<draft-id>/x/img/answer-<NN>-<mgmt>.png
 */
...
function parseArgs(): ReplyConfig[] {
  ...
  return keys.map((key) => {
    const baseDir = path.join(PROJECT_ROOT, `docs/sns-drafts/${draft}/x`);
