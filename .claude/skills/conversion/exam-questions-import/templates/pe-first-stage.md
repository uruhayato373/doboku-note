...
- **PDF パス**: `content/sources/textbook/技術士第一次試験/{R##}-{基礎|適性|建設}.pdf`
- **正答 PDF**: `content/sources/textbook/技術士第一次試験/{R##}-正答.pdf`（年度1ファイル、全科目の正答番号表。テキスト抽出可）
...
```
content/site/pe-first-stage/{year}-{subject}/article.mdx
```

subject = `basic` / `aptitude` / `construction`
例: `content/site/pe-first-stage/r07-basic/article.mdx`
...
```bash
python3 -c "import fitz,os; os.makedirs('/tmp/verify',exist_ok=True); d=fitz.open('content/sources/textbook/技術士第一次試験/R07-基礎.pdf'); [d[i].get_pixmap(dpi=200).save(f'/tmp/verify/p{i+1:03d}.png') for i in range(d.page_count)]; d.close()"
```
...
- `content/sources/textbook/技術士第一次試験/` — ソース PDF ディレクトリ
