...
```json
{
  "srcDir": "content/note/magazines/総監模範論文-自治体道路担当",
  "deliverTo": "desktop",
  "articles": [
...
## 記入欄マーカー（印刷して手書きする冊子用）

模擬試験の問題冊子など「印刷して書き込む」PDF は、源 md に次のマーカーを**単独段落**で書くと罫線つきの記入欄になる。**空のコードフェンス（```）は空白として潰れて何も印字されない**（2026-08-04・C8/C9 模試で記入余白ゼロのまま販売されていたのが発覚）ため、記入欄は必ずマーカーで明示する。

| マーカー | 描画 |
|---|---|
| `[[記入欄:12]]` | 12 行の罫線ボックス |
| `[[記入欄:9\|② 検討した内容と対応処置]]` | 見出し帯つきの罫線ボックス（ボックスは改ページで分割されない） |
| `[[記入欄表:工事名\|工期\|あなたの立場]]` | ラベル付きの記入表（`\|` 区切り） |
| `[[解答欄:5]]` | 穴埋め用の〔１〕〜〔５〕記入マス（番号は全角） |
| `[[解答欄:①\|②\|③]]` | ラベルを明示した記入マス（設問文の番号表記に合わせる） |
| `[[改ページ]]` | 改ページ |
