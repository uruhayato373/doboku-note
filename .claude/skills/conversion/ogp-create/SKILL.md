...
| `dark-wood` | 信頼性系（guide/textbook） | `.claude/skills/conversion/ogp-create/assets/fonts/ogp-backgrounds/dark-wood.png`（任意） |
| `red-line` | 体系・構造系（過去問解説等） | 不要 |
| `blackboard` | 教育・解説系 | `.claude/skills/conversion/ogp-create/assets/fonts/ogp-backgrounds/blackboard.png`（任意） |
...
```bash
node .claude/skills/conversion/ogp-create/scripts/ogp-create.mjs pe-comprehensive-management-mbo --template dark-wood --force
```
...
1. `.claude/skills/conversion/ogp-create/scripts/lib/ogp-templates.mjs` の `renderers` に新しい render 関数を追加
...
- `.claude/skills/conversion/ogp-create/assets/fonts/NotoSansJP-Bold.ttf` と `.claude/skills/conversion/ogp-create/assets/fonts/Inter-Bold.ttf` が配置済みであること
...
| 背景画像が反映されない | `.claude/skills/conversion/ogp-create/assets/fonts/ogp-backgrounds/{id}.png` が存在するか確認、`--force` で強制上書き |
...
- レンダラ: `.claude/skills/conversion/ogp-create/scripts/lib/ogp-templates.mjs`
