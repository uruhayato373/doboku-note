...
既定 psm 3（自動分割・横書き）。縦書きの本は --lang jpn_vert --psm 5 --subdir tess_vert を追加で回す（横書きの表も混ざるので置き換えでなく追加。compare がページごとに良い方を採る）。compare の本文一致率中央値が 0.6 台に沈み Tesseract 出力で左右の段が混ざっていたら psm の問題（--force で読み直す）
"""
...
def read_one(task):
    img, dst, width, psm, force, lang = task
    ...
    try:
        ...
        r = subprocess.run(["tesseract", "stdin", "stdout", "-l", lang, "--psm", str(psm)],
...
def main():
    ...
    ap.add_argument("--lang", default="jpn", help="jpn（横書き・既定）/ jpn_vert（縦書き。psm 5 と組む）")
    ap.add_argument("--subdir", default="tess", help="出力先。縦書き読みは tess_vert に置き、compare が横書き読みと良い方を採る")
    a = ap.parse_args()
    jobs = json.load(open(f".tmp/ocr/{a.book}/jobs.json", encoding="utf-8"))
    tess = os.path.join(jobs["workDir"], a.subdir)
    os.makedirs(tess, exist_ok=True)
    tasks = [(p["image"], os.path.join(tess, p["id"] + ".txt"), a.width, a.psm, a.force, a.lang) for p in jobs["pages"]]
    ok = cached = bad = 0
    ...
    print(f"[tesseract] {a.book} ({a.lang} psm{a.psm} → {a.subdir}): 読んだ {ok} / キャッシュ {cached} / 失敗 {bad} （全 {len(tasks)}p）")
    if bad:
        sys.exit(1)
