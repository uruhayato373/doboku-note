...
| 図を公開R2に出してしまう懸念 | `content/sources/textbook/**/img` は r2-sync 対象外。本文・画像は git 追跡外で、実体は Drive vault のみ。**文字起こし .md は `npm run drive-vault-sync -- --group source-transcript --commit` で各原資料の `ocr/` へ登録**。書籍 bundle の監査済み図は `npm run record-reference-book-artifacts` で `crops/` へ登録。未移行の `img/*.png` は `npm run drive-vault-sync -- --group textbook-page-image --commit`（手コピーは台帳に載らない） |
