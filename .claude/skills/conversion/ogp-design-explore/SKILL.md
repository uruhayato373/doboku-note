...
2. デザイン SSOT と現状を読む:
   - `.claude/knowledge/reference/ogp-prompts.md`（レイアウト・配色・テーマ色・変更履歴の真実源）
   - `.claude/knowledge/design-system/note-cover-tokens.json` の `exams[].base`（資格別テーマ色の真実源）
   - 現行 `ogp.png` を 2〜3 枚 Read（例: `content/site/{category}/{localSlug}/ogp.png`）して現状の見た目を把握。一括俯瞰は `npm run ogp-gallery -- --open`。
...
- **出力は必ず `.tmp/` 配下**に置く（例: `.tmp/ogp-explore/`）。本番 `content/site/**/ogp.png` は上書きしない。
...
2. デザイン SSOT を更新: `.claude/knowledge/reference/ogp-prompts.md`（レイアウト・配色・**変更履歴**）。テーマ色を変えるなら `.claude/knowledge/design-system/note-cover-tokens.json` の `exams[].base`。
3. 量産＆QA:
   ```bash
...
- 資格別 AI 背景（背景だけを Gemini で生成・全記事共有）: `npm run ogp-backgrounds`（`scripts/generate-ogp-backgrounds.mjs`）。mono-tag の最背面に敷く下地用。**課金が発生するため実行前にユーザー確認**（`--dry-run` は無料）。意匠の全面試作は本スキル、背景下地だけならこちらが軽量。※**2026-07-02〜、現行の資格別背景は Codex 生成のブランド写真プール**（`.claude/config/ogp/backgrounds/<exam-key>.png`・真実源 [`.claude/knowledge/reference/brand-image-system.md`](../../../../.claude/knowledge/reference/brand-image-system.md)）。この Gemini 生成はレガシー/代替扱い
- デザイン SSOT: `.claude/knowledge/reference/ogp-prompts.md`（レイアウト・配色・テーマ色・変更履歴）
- テーマ色トークン（真実源）: `.claude/knowledge/design-system/note-cover-tokens.json`
...
- スキル設計規約: `.claude/knowledge/reference/skills-design-guide.md`
