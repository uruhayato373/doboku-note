...
- **赤いのに誰も見ていない検査は無いのと同じ**: `scripts/quality-audit.mjs` の検査は `ci: true`（ゲート＝赤落ち）・`ops: true`（運用アラート＝日次 `ops-audit.yml` → `automation-failure` Issue channel ops・復旧で自動クローズ）・`ci: false`（report＝報告のみ）に分かれ、report は落ちても CI が緑のまま通る。**ci:true は結果が PR の diff だけで決まる検査に限る**。配信予定日・転記日などの壁時計に依存する検査を ci に入れると diff と無関係に全 PR が赤くなり、赤が読まれなくなる（2026-09: membership-drip が 30 日で 13 回 Pre-merge を落とし、unit-tests の赤 6 回が埋もれた）。そうした検査は `ops: true, ci: false` に置く（`tests/quality-audit-ops-tier.test.mjs` が再混入を止める）。`note-meta-lint` は Node 22+ の `glob` を import して Node 20 で起動即クラッシュし、report 扱いのため 3 週間 1 件も検査していなかった。**新規検査を report で追加するときは「誰がいつ読むか」を `note:` に書く**。読む人がいないなら `ci:true` にするか、作らない
