...
- **実行系も同じ**: 「対象 0 件で何もしなかった」と「全部失敗して何もできなかった」を区別して出力し、`skip` を無言で積み上げて最後に「全て in-sync」と言わない（経緯は measurement-incidents.md「2026-07-28」）
- **赤いのに誰も見ていない検査は無いのと同じ**: `scripts/quality-audit.mjs` の検査は `ci: true`（ゲート＝赤落ち）・`ops: true`（運用アラート＝日次 `ops-audit.yml` → `automation-failure` Issue channel ops・復旧で自動クローズ）・`ci: false`（report＝報告のみ）に分かれ、report は落ちても CI が緑のまま通る。**ci:true は結果が PR の diff だけで決まる検査に限る**。配信予定日・転記日などの壁時計に依存する検査を ci に入れると diff と無関係に全 PR が赤くなり、赤が読まれなくなる。そうした検査は `ops: true, ci: false` に置く（`tests/quality-audit-ops-tier.test.mjs` が再混入を止める）。**新規検査を report で追加するときは「誰がいつ読むか」を `note:` に書く**。読む人がいないなら `ci:true` にするか、作らない
- **検出器そのものが無い領域が最も危険**（quality-audit に未登録のスクリプトは壊れても誰も気づかない）。週次・月次が読むデータを生成するスクリプトは `--check` モード（成果物を書かずに完走だけ確認）を設けて CI で担保する
...
- 新しい script を足したら `package.json` の scripts と [commands.md](../knowledge/reference/commands.md) に用途と罠を 1 行書き、`quality-audit.mjs` に登録するか「誰が読むか」を決める（`npm run check-command-guidance` が案内の実在を検査し、`agent-hook.mjs check-doc-sync` が新規追加時に配線と `/doc-sync` を促す）
