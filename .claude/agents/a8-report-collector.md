...
- CI では本エージェントを使わない。成果 CSV の定期取得は `login-collectors.yml`（encrypted-state・canary 卒業後） が `fetch-a8-ui-csv` を直接回す。本エージェントはローカル対話呼び出しの Generator（会社 PC プロキシ下でもブラウザ経由は到達可）。
