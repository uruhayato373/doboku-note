...
| `angle` | 編集角度（**角度モードのみ**）。`conclusion` / `reason` / `experience` / `counter` / `number` / `howto` | `counter` |
| `source` | 角度の源となる既存 note 記事 dir / サイト slug（**角度モードのみ**） | `docs/note/キーワード集が点にならない理由` |

> モードは 3 系統: **keyword**（`slug` 指定・既存）／**exam**（過去問パック・既存）／**angle**（`angle` + `source` 指定・本ファイルで新設）。`angle` 指定があれば角度モードとして下記ルールで執筆する。
