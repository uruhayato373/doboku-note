...
5. **出品文面の改訂案** — ココナラ展開キット.md（散文）／listings.json（投入本文）を SoT として改訂（同一 commit で更新）。構成の型は `.claude/shared-policy/STRUCTURE.md`「強化コンポーネント」を参照（C系PDF=直適用／S系人力=翻案・body は1000字上限・誠実表現必須）
...
7. 運営者へ引き継ぎチェックリストを提示（返信文 QA の判定と未確認点を添える。最終赤入れ→送信→`status: 'delivered'` 更新→所要時間 `tensakuMinutes` 記録）。

### ケース1.5: 受注（S3 指導＝ヒアリング→骨子シート→本人が書いた答案の添削）

2026-09-25 に作成（答案ドラフトの納品）が運営に「学校の課題の代行」として取り下げられ、代筆しない指導へ作り替えた（id は `coconala-sakusei` 等のまま）。**答案を書くのは本人**。

1. カタログで該当 id の status 確認（`full` なら週枠超過を警告）。
2. **指導用ヒアリングシート**（キット §4b・宣誓チェック付き）を scratchpad/`.tmp/` に `.md` 保存。宣誓チェック未記入なら追加質問の文面を出して停止（**創作で埋めない**）。
3. `/keiken-tensaku <sheet.md> --grade N --mode kosshi` で**骨子シート**を生成（事実は回答の「」引用だけ・答案の文章なし・`check-kosshi-sheet` exit 0）。
4. 骨子シート＋キット §4c「骨子シート送付」文を返信文として QA に通し、運営者へ引き継ぐ（途中経過のメッセージ。正式納品にしない）。`orders-log.json` に append（`status: 'kosshi-sent'`）。
5. 本人の答案が届いたら S2 と同じ添削（`/keiken-tensaku <答案> --grade N`）→ 返却を正式納品（`delivered`）→ 再添削1回（`revised`）。
