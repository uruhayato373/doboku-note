...
note 公開用ドラフトの本文中の図版（`docs/note/{slug}/img/figure-*.png`）が `docs/reference/note-svg-policy.md` の規約を満たしているかを監査する **Evaluator エージェント**。**audit-only**（修正は行わない）。
...
| ルール | `docs/reference/note-svg-policy.md`（真実源） |
