...
| `textbook_md_path` | 標準テキスト md ファイル（Pattern H 使用時、省略時は frontmatter から自動解決） | `docs/textbook/技術士（総監）/テキスト/総監標準テキスト/社会環境管理.md` |
| `textbook_section_anchor` | テキスト md 内の該当アンカー（任意） | `§1 地球的規模の環境問題` |

拡張パターンの内容・選択ロジック・弱点軸 → 推奨パターン表 → [03_リライト方法論方針.md §「系統 B: keyword-rewriter の拡張パターン A-H」](../../docs/project/02_コンテンツ/03_リライト方法論方針.md)
