...
> [!important] 中央値の母数は「その URL が測れているバッチ」だけ
> PSI は一時的に `500 Something went wrong` を返すことがあり、バッチによって URL が欠ける。
> 直近 14 バッチ 308 計測中 21 失敗＝**欠測率 6.8%**（429 は 0 件＝クォータ問題ではない）で、
> 1 バッチで 6/22 が欠けた日もある。2026-08-18 に 5xx のみのリトライを入れたが、
> **欠測が 0 になる保証は無い**。
>
> - 「直近 5 バッチ」を**バッチ数**で数えると、欠測した URL は古い値を引きずるか件数が足りなくなる。
>   **その URL の値が存在するバッチだけ**を集めて中央値を採ること
> - 母数が 3 バッチ未満なら回帰判定をしない。**「劣化なし」ではなく「判定不能」と書く**
>   （測れていないことを「違反が消えた」と読ませない — CLAUDE.md §9）
> - バッチごとの計測件数が 22 に満たない日は、レポートに欠測件数を 1 行添える
...
- 参考ファイル: `content/site/.../article.mdx:25`
