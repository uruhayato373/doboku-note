...
- 出力は**1問=1ファイル+解答=1ファイル**構成・**解答は全選択肢○×**・**巻末に学習導線+著者プロフィール**（ビルダーが自動付与）。件数が 150 を割ったら価格帯（¥490→¥390）を 08 と突合して報告
- 収録件数は「見出し文 lead 判定後」の実数で 08 と突合（旧 body 全体マッチの過大カウントを引き継がない）
