...
本エージェントは構成物（THEMES 定義・spec JSON・書き下ろし原稿）の**作成・改訂のみ**を行い、生成した書籍の品質合否は判定しない。`content/kindle/books/**` の前付けは本エージェントだけが編集し、評価は対の `kindle-book-qa` が行う。
...
- 書き下ろし前付け: `content/kindle/books/{id}/front-matter.md` を**spec の原稿ソースを実読して**執筆する
  - はじめに（本書の使い方・対象読者）
  - 7 年分の出題傾向分析（頻出分野・出題形式の変化を実データで。憶測の統計は書かない）
  - 学習ガイド（科目の攻略順序・時間配分）
  - **書き下ろしは「無料 web 公開コンテンツの転載ではない」ことを担保する KDP 差別化の中核**。サイト記事からのコピペ流用は不可
- 原稿ソース: B/D/E系は `content/site/**/article.mdx`、C/F系は `content/note/**/article*.md`
