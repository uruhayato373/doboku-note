...
1. **§17 散文充実**: 各 H2 セクションに散文導入 200〜400 字。見出し直下にいきなり箇条書き・表・図を置かない（§2/§17-2、lint 6-2〜6-4 解消）。
...
`node .claude/scripts/lint-mdx-mobile.mjs <file>` で **(15-1)/(6-2)/(6-3)/(6-4)/(7-1)/(0-2) が全て0件**、本文 3,000 字以上、U+FFFD なしを確認。
