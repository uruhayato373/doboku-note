...
- CI では本エージェントを使わない。GSC/GA4 UI CSV の定期取得は `login-collectors.yml`（encrypted-state・canary 卒業後） が同じ script（`fetch-gsc-ui-csv` / `fetch-ga4-ui-csv`）を直接回す。本エージェントはローカル対話呼び出しの Generator。

## 参照

- `docs/operations/gsc-ga4-playwright-automation-spec.md` — 実装指示書（真実源）
