...
| **コンテンツ原則** | 20% | content-principles.md §5 完全準拠（ExamPoint 1〜2個・総括位置・禁止パターンなし）、概念→位置づけの順、導入文完備、重複表なし | 軽微な配置ズレ（ExamPoint位置がやや早い等） | 概念説明と試験対策の混在 | **`<ExamPoint>` 3個以上、または `summary`/`items` に「誤り選択肢」「代表的な誤り」を含む、または冒頭「とは」直後に `<ExamPoint>`**（lint 9-1/9-3/9-5 違反） |
