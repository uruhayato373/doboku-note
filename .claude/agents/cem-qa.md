...
| **コンテンツ原則** | 20% | content-principles.md §5・§7 完全準拠（ExamPoint 1〜2個・総括位置・禁止パターンなし、**Callout 1〜3 個で引っかけ/例外点が視覚分離されている**）、概念→位置づけの順、導入文完備、重複表なし | 軽微な配置ズレ（ExamPoint 位置がやや早い、**長文 H3 が 4 つ以上あるのに Callout ゼロ = 視覚分離不足**、**Callout 3 個超で強調希薄化**等） | 概念説明と試験対策の混在 | **`<ExamPoint>` 3個以上、または `summary`/`items` に「誤り選択肢」「代表的な誤り」を含む、または冒頭「とは」直後に `<ExamPoint>`**（lint 9-1/9-3/9-5 違反） |
