...
1. 過去問 MDX の該当設問本文を Read（`.local/r2/posts/pe-comprehensive-management/{exam_slug の末尾}/article.mdx` の `## Ⅰ-{anchor}` から次の `## ` まで）
2. 現紐づけ各 slug の本文を Read（`.local/r2/posts/pe-comprehensive-management/{slug}/article.mdx`）— 平均 4 件
