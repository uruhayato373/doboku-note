...
| 対象 | 内容 |
|---|---|
| 入力 | メインスレッドが事前準備した以下:<br>1. PDF 全ページ画像 `.tmp/pdf-pages/{R{年}-1ji-{AB}}/page-XX.png`（200dpi）<br>2. PDF テキスト `.tmp/pdf-text/R{年}-1ji-{AB}.txt`（pdftotext -layout）<br>3. 対象 MDX `.local/r2/posts/civil-construction-1/primary-r{年}-{ab}/article.mdx`<br>4. 既存 backup `.tmp/backup/{exam}/img/`（あれば参考に）<br>5. 前回反復の `auditor-feedback.json`（あれば） |
...
{
  "exam": "r06-a",
  "pdf": "docs/textbook/１級土木施工管理技士/R6-1ji-A.pdf",
  "figures": [
    {
      "problem_no": 1,
      "page": 3,
