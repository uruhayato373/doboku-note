...
- **`cover:` frontmatter ブロック**の有無（`leadIn`/`headline`/`hi`/`hiSuffix`/`benefit`。`coverTitle` だけでも生成はされるが見出しの精密制御ができない）
