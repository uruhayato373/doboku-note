...
| 5. note完成度・実用性 | 「設問構成と論述方針」「採点者が見るポイント（pipe表でなく箇条書き）」の構造が揃う。見栄え（見出し・段落・項目番号）が整い、答案が手書き再現可能な字数密度。**末尾は必須科目I マガジンへのCTA**（選択科目の場合。サイト無料ページ導線でなく有料ファネル、URL単独行のリンクカード）。**「合格者からのコメント／元公務員からのコメント」節があれば減点**（2026-06-10 廃止）。サイト無料ページ（doboku-note.com/docs）を末尾CTAに使っていれば指摘 |
