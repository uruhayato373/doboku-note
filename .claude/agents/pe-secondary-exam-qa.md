...
- **マガジン階層**: `_meta.yaml` / `_cover.png` / `hashtags.txt` / `note-magazines.ts` 登録（公開前 published:false）/ `magazine-placement.ts` 配線の有無。**`note-magazines.ts` の登録確認は id（`pe-construction-road-magazine` 等）で検索する**（"BK-01" 文字列では空振りする＝誤検知に注意）
- **`_meta.yaml` の整合（区分1ファイル方式）**: `articleCount: 15`・`files: [article-II1, article-II2, article-III]`・`articleStructure: "R03-R07 × (II-1 / II-2 / III)"` であること。**旧命名（`article-II1-1`〜`article-III-2`）・`articleCount: 35`・「20記事」・`description`/`appealPoint` 内の「元公務員（発注者）からのコメントを収録」等の廃止節言及が残っていれば `issues` に列挙**
