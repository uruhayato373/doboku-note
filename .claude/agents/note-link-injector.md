...
| URL | `https://doboku-note.com/exam/pe-comprehensive-management/keywords/{slug}?utm_source=note&utm_medium=referral&utm_campaign={記事の安定slug}&utm_content=kw-{slug}`（旧 `/docs/pe-comprehensive-management-{slug}` は 301 なので使わない） |
...
4. 編集完了後、機械的検証を実行:
   ...
   echo "総リンク数: $(grep -oE '\[[^]]+\]\(https://doboku-note.com/exam/pe-[^)]+\)' "$F" | wc -l)"
   echo "ユニーク slug: $(grep -oE '/exam/pe-comprehensive-management/keywords/[a-z0-9-]+' "$F" | sort -u | wc -l)"
   echo "UTM 欠落リンク（0 であること）: $(grep -oE '\]\(https://doboku-note\.com/(exam|practice|standards|topics)/[^)]+\)' "$F" | grep -vc 'utm_source=note')"
