...
2. **縦余白は「埋めて使い切る」**——空白で放置しない。要素の拡大、1 行サマリー（体言止め）、凡例の追加で密度を上げる。ただし情報の捏造はしない（元の概念の範囲内）。
