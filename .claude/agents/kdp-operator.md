...
| `scripts/kindle-published/catalog.json` | 全書籍の status/ASIN/draftAsin（提出後の更新先・冊数もここが真実源） |
...
3. **カテゴリー未検証時**（`catVerified=false`。2026-09-23 時点で全カテゴリー verified のため、新しい系統を足したときだけ）: `--diag-category --asin <既存draft>` で末端ラベルを確認し
