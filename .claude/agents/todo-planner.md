...
- 月初は、今月やるカードの `[時期:]` を付ける・直す（monthly.md に表を書かない＝`npm run check-backlog-schema` が止める）。成果目標（3 つまで）だけ monthly.md に書く
...
monthly.md は `## 今月の成果目標`（3 つまで）だけ。今月のタスクはカードの `[時期:]` が決める。件数は絞らない。終わらなかったカードは翌月へ回す（月初に `npm run roll-backlog-when -- --write` が `[時期:]` の終わりを今月へ延ばす。開始は残すので持ち越しの長さが見える。2026-09-26 のユーザー方針）。週間は重要度と `[期日:]` の近いものから選ぶ。
...
3. **優先順位の決定**
   - 試験日まで何週かを exam-calendar.json から計算
   - **候補は `[時期:]` が今月を含むカード**（＋期日の近いもの・進行中）。monthly.md の成果目標を踏まえて週内に収まる範囲を選択
...
5. **時期の見直し**（月初）
   - 終わらなかったカードは手で直さず `npm run roll-backlog-when -- --write` で翌月へ回す（`[時期:]` の終わりを今月へ延ばし、開始は残す）。終わったカードは回さずに削除する。時期を過ぎたまま残るカードは `npm run check-backlog-health` の S15 に出る（monthly.md に状態は書かない）
   - 月次レビューが未実施なら `npm run check-monthly-review-due` が知らせる（回すのは月次レビュー手順 8）
