...
> **マガジン公開後の冒頭回遊ブロック（総監と同方式）**: マガジンを note 公開し `note-magazines.ts` に `noteUrl`／`published:true` を登録したら、各記事の**冒頭（「この記事でわかること」の直後・本文導入「本記事は…」の直前、`---` で挟む）にマガジン回遊ブロック**を追加する（例：「{科目} 選択科目の模範解答（R03-R07・全15記事）をまとめた**マガジン**もあわせてご覧ください。／各記事を単品でそろえるよりお得なセット価格（¥1,980）です。／{マガジンURL}」）。**公開前（URL未確定）は置かない**（プレースホルダー禁止）。マガジンURLは PDF抽出レンジ外なので PDF再生成は不要。
