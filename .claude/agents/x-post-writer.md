...
3. `exam` と `type` からネタ源を決める：
   ...
   - **コンクリート系**（`concrete-chief` / `concrete-diagnosis`）: サイト記事 `.local/r2/posts/concrete-chief-engineer/**`（19 本）・`concrete-diagnostician/**`（18 本）。**表記は必ず「コンクリート主任技士」**（技師ではない・`exam-calendar.json` policy）。診断士は試験終了済み（7/26・合否 9/30 頃）なので**直前対策は書かない**——合否待ち期の心理・来年度受験の検討・実務での活かし方に寄せる。
...
| Kindle | LIVE 済みの書籍のみ | **g-01 / g-02（コンクリート2冊）を「発売中」と書く**（提出待ち・ASIN 未取得） |
