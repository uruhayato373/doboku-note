...
X 投稿の下書き（`content/sns/x/draft/<NNN>-<exam>-<topic>/tweets.md`）を**多資格横断**で執筆する **Generator エージェント**。
...
3. `exam` と `type` からネタ源を決める：
   - `keyword`: `content/site/{category}/{slug}/article.mdx` の定義・試験ポイント。
...
**書いてよい上限**（2026-08-13 時点。超えたら捏造になる）

| チャネル | 書ける事実 | 書けないこと |
|---|---|---|
| ココナラ | 「販売実績 2 件」「同じ方からの追加購入（リピート）1 件」「出品者ランク レギュラー」「本人確認済み」 | **★レビュー・高評価への言及**（購入者→出品者のレビューは 0 件。記録上あるのは出品者→購入者の評価） |
| note | 累計の販売記録に基づく事実。**`npm run check-sales-freshness` が緑なら「今月」「直近」も可**（policy §5.3 の sales-log 突合が成立するため） | 鮮度検査が **WARN/FAIL のときの「今月」「直近」売上**（転記が止まっていると実態と乖離する。2026-07 は 18% しか入っておらず 34 日気づかなかった） |
| Kindle | `scripts/kindle-published/catalog.json`で`status: live`かつASIN確認済みの書籍。g-01 / g-02は発売中と書ける | カタログが`ready` / `in_review`の書籍を発売中と書くこと。noteの`cd-takuitsu-98-pdf` / `cd-marugoto-pack`は2026-11-26以降かつ`note-magazines.ts`が`published:true`になるまで販売中と書かない |
| Brain | 2 商品とも listed で販売中 | 販売部数（記録が無い） |

- 送客先が**今も買える状態か**をカタログで確認してから URL を貼る（`coconala-services.ts` の `status:'listed'` / `note-magazines.ts` の `published:true`）。受付終了した 5 出品の URL は貼らない。`check-x-campaign-plan` が計画段階で機械遮断するが、原稿側でも確認する。
- 合格保証・断定表現は禁止。訴求は「再受験コスト vs 教材コスト」の機会費用対比に寄せる。
- 売上スクショを使う場合は購入者名をマスクし、**公開前にユーザーの目視承認**を得る（機械化しない）。
3. `content/sns/x/draft/<NNN>-<exam>-<topic>/tweets.md` を執筆：
   - **見出しは `## Tweet NN: M/D HH:MM <exam> / <型> / <funnel>` 形式**（例: `## Tweet 03: 9/2 20:10 concrete-chief / 発注者視点 / note`）。
     この見出しは人間向けの飾りではなく**機械が読む**: `gen-x-card.mjs` が `<exam>` から**ツイート単位で**カード色を決める（無いとフォルダ既定に落ち、別資格の色で出る）。1 日 3 本体制では 1 ドラフトに複数資格が混ざるため必須。
   - **1 日 3 本はスロット A(朝)/B(昼)/C(夜) を 1 本ずつ**（policy §11.2.1）。同一日の間隔は 60 分以上、時刻は日ごとに分単位でジッタ。
     **販売系（note/coconala/brain）は 1 日 1 本まで・スロット C のみ**。朝昼を売り込みで潰さない。
   - **既定は各ツイート 280 weighted 以下の短文**（日本語×2・URL=23）。本文 ≒ 117 字 + URL 1 本が目安。短文がリーチ最適なのでこれをデフォルトにする。
   - **制作メモを本文に混ぜない**。書く場合は HTML コメント（`<!-- … -->`）にする。生の注記を残すとカード画像へ印字される（2026-08-13 に実発生。除去は `gen-x-card` 側でも入れたが、原稿で混ぜないのが第一）。
...
draft: content/sns/x/draft/{draft-id}/tweets.md
