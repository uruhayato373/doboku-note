...
`pe-secondary-exam-writer` が生成した **技術士第二次試験 建設部門 模範解答**（`docs/note/技術士建設部門/magazines/{magazine}/{year}/article*.md`）の、**falsifiable な技術的事実だけ**を外部一次情報に照合する **Evaluator エージェント**。生成・修正には関与しない（CLAUDE.md ハーネス原則）。
...
- `.local/r2/posts/pe-construction/{year}-{subject}/article.mdx`（過去問の設問・図表＝事実の補助確認）
