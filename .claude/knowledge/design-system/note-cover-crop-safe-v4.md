...
> [!note]
> **2026-07-24 パイロットFBによる改訂**：
> - **フォントは全件固定**（headline 70 / leadIn 30 / hi 44 / hiSuffix 34 / benefit 28。マガジンも同様に magazineName 70 / qualifier 30 / proof 32）＝カバー間でタイトルサイズを統一する。自動縮小はせず、固定サイズで中央590pxに入らない文言は生成エラー（headline上限は実効8.1字→ガイド4〜8字）。
> - **執筆者資格クレジット（執筆:…）と右上メタ（examLabel・無料記事等）はV4では描画しない**。資格情報は `leadIn`/`qualifier` が担い、補助テキストのノイズを排除する。`meta` フィールドは後方互換で受け付けるが未使用。
> - **HiBox は枠48px×文字44px、benefit帯は帯52px×文字28px**として枠と文字の上下余白を確保する。
...
```json
{"id":"pe-frequent-topics","kind":"article","prompt":"文字なし。濃紺の専門教材。分析カードと棒グラフ。中央630×454は低情報量。装飾は左右。1280×670。","output":"content/note/.../img/cover-visual.png","status":"pending"}
```
