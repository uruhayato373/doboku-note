...
12 種の Callout（note/tip/warn/danger/success/exam/formula/standard/example/reference/faq/quote）× bg/border/fg。light/dark 両定義。実装・使い分けは `src/components/ui/Callout/README.md` と `docs/design/callout-gallery.md`。
...
| `--disclosure-chevron` / `.disclosure-chevron` | 細線シェブロン SVG（mask 用 data URI）／開閉回転クラス | アコーディオン開閉アイコン（`DisclosureChevron`／prose details）。回転を素の CSS に置くのは **prose 記事内の `<details>` が React を通らず class を付けられない**ため（旧「Tailwind の transform 変種が無効」は 2026-08-20 の実測で誤りと判明） |
...
- トップ Hero の高さはモバイル `420px` / タブレット `500px` / PC `540px` を基準とする。背景は `brand-image-system.md` の wide 原版（2.4:1・文字焼き込みなし）を使い、見出しと CTA は HTML で重ねる。
