...
| `ExamMatrix`（`category/ExamMatrix.tsx`） | 「**行ラベル × 種別リンク**」マトリクスの**全資格共通**レンダラ（過去問＝年度 × 問題種別、建設部門キーワード節＝選択科目 × 種別）。デスクトップ=現行様式の table／モバイル(<993px)=**1 行 1 レコードのチップリスト**（行ラベル＋存在するリンクだけ `ExamChipLink` 横並び）で 4 列表の折り返し崩れを回避。`CategorySections.tsx` の各 `*ExamTable`（civil-1/civil-2/総監/一次）は年度整形して `columns`/`rows` を渡すのみ。建設部門の科目×年度マトリクスも `ExamChipLink` を共用 | `columns: string[]` / `rows: {key,label,cells:{label,doc?}[]}[]` / `rowHeader?`（行ラベル列の見出し・既定 `年度`。キーワード節は `選択科目`）/ `rowLabelWidth?`（`default`=年度 nowrap／`wide`=長い科目名を全幅行見出しにしてチップを次行へ）。チップ=`ExamChipLink` |
