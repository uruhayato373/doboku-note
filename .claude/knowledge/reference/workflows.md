...
CLAUDE.md §1・§2・§5・§10 の根拠と経緯（ルール本文は CLAUDE.md が正。2026-09-08 に CLAUDE.md から移設）。

### origin との遅れを先に解消する（§1）

複数セッション・worktree が常態で、CI が deploy のたびに `develop` → `main` を自動マージするため、ローカルが数十コミット遅れるのは高頻度。2026-06-11 に 43 コミット遅れの古いツリーで作業し、既存作業を重複・劣化させた。SessionStart フック `scripts/check-git-sync.mjs` が開幕で警告する。遅れていれば同期してから着手し、古いベース上のコミットを push しない。破壊的な reset・同期はユーザー確認後、独自コミットは退避ブランチで保全する。
...
- reflog・`develop` 先頭・未コミットが自分の操作と無関係に動くのは正常（共有 `.git/logs/HEAD` に全プロセスの記録が混在する）
- push 前に `git log origin/develop..HEAD` で巻き込みを確認する。commit は `git commit -- <pathspec>`（`git add -A` 禁止）。他テリトリ不可侵。重要な変更は feature ブランチへ即 push して保全する
- `git stash` は共有スタック。別セッションの autostash を pop して競合させた実例があるので使わない（やむを得なければ `git stash push -u -m <tag>` → SHA を控えて `apply`）
- 同一セッション内の並行エージェントは、各エージェントが編集したファイルを即 commit する（`git status` で staged を確認）

### ブランチの使い分け（§2）

doc・コンテンツは `develop` へ直 push で蓄積する（「1 修正 = 1 PR」禁止）。コード系のみ feature ブランチ + PR（base = `develop`）。`develop` → `main` は `/deploy` スキル経由でユーザーが判断する。
