...
納品は有料エリア内の R2 リンク（リンクを知る人のみ＝Drive リンク共有と同等）。通常の版更新では**新トークンで別名**にし、本文のURLも更新する（旧URLの無効化はできないため）。公開済み配布物の誤記訂正など、既存購入者へ同一URLで修正版を届ける必要がある場合に限り、`r2-brain-dist.yml` の `file=<正確な basename>` と `overwrite=true` を指定する。上書きは単一ファイル指定が必須で、`upload-brain-dist-r2.mjs` が R2 から再取得した bytes の SHA-256 と `Cache-Control: no-cache, no-store, must-revalidate` を照合できなければ失敗する。workflow は Pages のカスタムドメイン設定から `zone_tag` を取得できる場合、当該配布URLだけを即時 purge する。API token の権限上 `zone_tag` または purge が使えない場合も、上書き object の no-store により現在の edge TTL（最大4時間）満了後は旧 bytes が再キャッシュされない。ワークツリーだけを上書きしても workflow は `main` を checkout するため、修正版 ZIP を先に commit・main へ昇格してから実行する。
