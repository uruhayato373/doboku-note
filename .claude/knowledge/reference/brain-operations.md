...
| listings[id] のキー | 意味 |
|---|---|
| `bodyText` | 販売記事の全文（無料LP＋paidMarker＋有料エリア）。**¥価格の直書き禁止**（真実源はカタログ）・**配布URLを有料エリア内に必須**（check-brain-wiring が強制）。**構成の型は [STRUCTURE.md](../../shared-policy/STRUCTURE.md)「強化コンポーネント」を参照**（Brain DL キット＝情報商材型で直適用）。誠実表現は §5-6 |
...
納品は有料エリア内の R2 リンク（リンクを知る人のみ＝Drive リンク共有と同等）。通常の版更新では**新トークンで別名**にし、本文のURLも更新する（旧URLの無効化はできないため）。配布物は初回 upload から `Cache-Control: no-cache, no-store, must-revalidate` を必須にし、後日の同一URL訂正で旧 bytes が edge に残る状態を作らない。公開済み配布物の誤記訂正など、既存購入者へ同一URLで修正版を届ける必要がある場合に限り、`r2-brain-dist.yml` の `file=<正確な basename>` と `overwrite=true` を指定する。上書きは単一ファイル指定が必須で、`upload-brain-dist-r2.mjs` が R2 から再取得した bytes の SHA-256 と Cache-Control を照合できなければ失敗する。workflow は Pages のカスタムドメイン設定から `zone_tag` を取得できる場合、当該配布URLだけを即時 purge する。既存のキャッシュ済み object を初めて no-store 化する際に API token の権限上 `zone_tag` または purge が使えない場合も、現在の edge TTL（最大4時間）満了後は旧 bytes が再キャッシュされない。ワークツリーだけを上書きしても workflow は `main` を checkout するため、修正版 ZIP を先に commit・main へ昇格してから実行する。
...
- **本文（LP）変更**: `brain-publish` は既定で既存本文を保持（>50字なら挿入スキップ＝二重挿入防止）。差し替えは **`--replace-body`**（既存を Meta+A→Backspace で**2回**全消去→再挿入・クリア後 text>50字 or img>0 なら ABORT＝画像ノード残りの保険）。本文変更も**再審査**に入る。SoT は `content/brain/listings.json` の bodyText（価格直書き禁止・配布URLは有料ラインより後）。**構成の型の真実源は [STRUCTURE.md](../../shared-policy/STRUCTURE.md)「強化コンポーネント」**（社会的証明/価値3点先出し/数え上げ/3ステップ/FAQ/図版）。2026-07-23 に両商品 LP をこの型で改稿し `--replace-body --set-category 資格` で再申請（誠実表現は §5-6 準拠）。
