...
残工程も同じworkflowでphaseを指定する。`migration-audit` は旧版の字幕・再生リスト所属と新版の処理完了を取得し、`commit=true` の場合だけprivate台帳へ記録する（YouTubeへの書き込みなし）。`migration-thumbnail` は再試行可能日時を守り、設定後のCDN画像を比較する。受理済みで表示待ちの場合は再送しない。

`migration-activate` はサムネ実表示・再生・再生リスト/手動字幕・Shorts関連動画の状態・依存リンクの確認が台帳にそろったものだけ、旧版の公開範囲と未来の予約日時へ切り替え、APIで読み直す。確認記録は自動で「確認済み」にせず実査後にprivate台帳へ残す。`migration-delete` はさらに1時間以内の旧新IDに紐づく依存リンク確認を要求し、削除意図を先に記録してから旧版だけを削除し、APIで不存在を確認する。削除応答を失っても新たな対象を推測しない。自動アップロードworkflowを移行中に一時停止した場合は、旧新IDの参照更新と残投稿の素材更新が済んでから元の設定へ戻す。
