...
`monthly.md` は **`## 今月の成果目標`（3 つまで）だけ**を持ち、タスク表を置かない（`check-backlog-schema` の `monthly-table` が止める）。今月のタスクは `[時期:]` が今月を含むカードで、管理画面 計画 ＞ 月間 がバックログと同じ表で出す。件数は絞らない。終わらなかったカードは翌月へ回す（月初に `npm run roll-backlog-when -- --write` が `[時期:]` の終わりを今月へ延ばす。開始は残すので持ち越しの長さが見える。2026-09-26 のユーザー方針）。週間は重要度と `[期日:]` の近いものから選ぶ。
