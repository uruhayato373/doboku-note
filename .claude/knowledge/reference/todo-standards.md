...
| 週間 | `weekly.md` | 今週実行する分 (今月のカードから選ぶ ID 表＋定常運用・手動キュー) | 両方 |
| 月間 | `monthly.md` | doboku: **今月の成果目標だけ**。今月のタスクは `[時期:]` が今月を含むカード (導出・表を書かない) | 両方 |
| 年間 | `annual.md` | doboku: **年間の方針の文章だけ** (注力しない・四半期定例)。年間の重点は `[時期:]` のカードで、管理画面 計画 ＞ 年間ロードマップ が描く | doboku のみ |
...
doboku-note のフロー (2026-09-26〜) は `backlog の [時期:] → 年間ロードマップ・月間 (導出) → weekly (週ごとに選ぶ)`。月初の pull はカードの `[時期:]` を付ける・直すこと。週次計画はカードの ID を参照し、
...
## 2-2. monthly は成果目標だけ・weekly は ID 参照ビュー (doboku-note・2026-09-26 改訂)

`monthly.md` は **`## 今月の成果目標`（3 つまで）だけ**を持ち、タスク表を置かない（`check-backlog-schema` の `monthly-table` が止める）。今月のタスクは `[時期:]` が今月を含むカードで、管理画面 計画 ＞ 月間 がバックログと同じ表で出す。件数は 8〜12 件を目安に `[時期:]` で調整する。
`weekly.md` は**タスク本文・状態・完了経緯を複製しない**。backlog の ID を選ぶ表だけを持つ（表の ID はバックログに実在すること＝`weekly-ref`）。
...
- 管理画面は weekly の ID を backlog 索引と join し、タイトル・tier・種類・担当・期日を表示する。参照切れ ID は赤く出す（コミット時は `weekly-ref` が止める）
...
| 軸 | 語彙 | 意味 |
|---|---|---|
| **tier** (見出し) | 🔴 高 / 🟡 中 / 🟢 低 / 🟣 判断待ち | 重要度のみ。**いつやるかは `[時期:]` だけが持つ**（doboku: 🔴・🟡 は `[時期:]` 必須、`[期日:]` があれば期日の月を `[時期:]` に含める＝`check-backlog-schema` の when-missing・when-due。2026-09-26 に見出しの「来月中」と `[時期:]` が食い違い、時期の無い 28 枚が月間に出ていなかった）。**🟣 は「やるかどうかの意思決定が未了」であって、ユーザー作業待ちの置き場ではない** (待ち先が人であることは本文に書く) |
...
| `[時期:YYYY-MM]` / `[時期:YYYY-MM..YYYY-MM]` | 🔴・🟡 は必須（`[期日:]` があれば期日の月を含める）。🟢・🟣 は任意 | やる月。いつやるかの唯一の正本（見出しは重要度）。年間ロードマップ（管理画面 計画 ＞ 年間ロードマップ）と「今月のカード」はこの値だけで決まる（重点の別台帳を持たない）。形式は `check-backlog-schema`、過ぎたまま残るカードは `check-backlog-health` S15、今月〜2か月先に目安（月 12 件）を超える月は S16。doboku-note だけの拡張 |
