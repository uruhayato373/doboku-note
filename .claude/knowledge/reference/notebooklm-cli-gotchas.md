...
**コンテキスト節約の既定**: 1 回の調査では notebook 1 冊・質問 1 件を基本とし、下流へ渡すのは必要な結論と引用元だけに絞る。全文キャッシュや自動ルーターは、同じ問い合わせの重複またはコンテキスト肥大を実測するまでは作らない。数値・制度は NotebookLM の回答だけで確定せず、示された一次資料へ照合する。個人認証情報を CI へ持ち込まない。
