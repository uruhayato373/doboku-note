...
| `coconala-2kyu-sakusei` | S3【2級】**指導**（ヒアリング→骨子シート2テーマ→本人が書いた答案の添削＋再添削1回・¥7,000・納期7日）。2026-09-25 に作成版として出品→**同日17:30 運営が取り下げ**（理由は 2026-08-12 と同じ「学校の課題の代行」。「学校の課題の代行ではなく」の断り書きがあっても通らなかった）→同日、代筆をしない指導へ作り替えて同じ id・URL で再出品。id は `sakusei` のまま。骨子シートの納品手順は未整備（`/keiken-tensaku` に骨子モードは無い） |
| `coconala-2kyu-sakusei-3theme` | S3+【2級】作成 上位版（3管理フル・¥10,000・週1枠・72時間以内）。2026-09-25 新設。2級は3管理で全出題をカバー。**draft（未出品）** |
