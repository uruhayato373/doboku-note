...
| `coconala-sakusei-4theme` | S3+【1級】**指導** 上位版（全5テーマ・骨子/添削とも72時間以内・納期14日・¥20,000・週1枠）。旧: 4テーマ作成（2026-08-12 運営取り下げ→修正再出品）→5管理フル作成（09-25）→同日、指導へ作り替え（経緯は `coconala-sakusei` 行）。旧¥16,000＝priceHistory 記録済み |
| `coconala-2kyu-sakusei` | S3【2級】**指導**（2テーマ・骨子/添削とも48時間以内・納期7日・¥7,000）。2026-09-25 に作成版で出品→**同日17:30 運営が取り下げ**（理由は 2026-08-12 と同じ「学校の課題の代行」。「学校の課題の代行ではなく」の断り書きがあっても通らなかった）→同日、指導へ作り替えて同じ id・URL で再出品。これを受けて作成系4件すべてを指導化 |
| `coconala-2kyu-sakusei-3theme` | S3+【2級】**指導** 上位版（全3テーマ・骨子/添削とも72時間以内・納期10日・¥10,000・週1枠）。2026-09-25 に作成版で新設→同日、指導へ作り替え（経緯は `coconala-sakusei` 行） |
...
| `faq` / `options` | v1 では自動投入しない（保持のみ・公開後に UI 手動）。**`coconala-edit` は FAQ を触らないので、listings の faq を直しただけではライブは変わらない**。2026-09-25 に指導4件・添削4件の FAQ を一回限りのスクリプトでライブへ反映した（編集ページの `textarea[name="data[Faq][i][question|answer]"]`・削除 `button.faqDeleteLink{i}`・追加「QAを追加」）。`check-coconala-live` は FAQ を照合しない |
...
> `content/note/1級・2級土木/メンバーシップ/添削事例アーカイブ/` へ（1対多の資産化）。
...
添削3ステップ（字数→論点抽出→最終赤入れ）の真実源は [2級経験記述-添削テンプレ.md](../../../content/note/1級・2級土木/2級土木/2級経験記述-添削テンプレ.md)。トークルーム定型文（初回挨拶・シート送付・C系 PDF 送付・満枠断り・書き直し受付・S1 診断返却テンプレ）は [ココナラ展開キット.md §4c](../../../content/note/1級・2級土木/ココナラ展開キット.md)、S2 納品文面テンプレは `.claude/agents/coconala-operator.md`。
...
**`--replace-image` は全スロットを消す（2026-09-23）**: 画像2枚の商品（診断・添削）はサムネ差し替えでギャラリーも消えるので、直後に `--image gallery-*.png --force-image` で入れ直す。間を空けると別セッションの note 一括更新がブラウザを取り、同時1本ガードで入れ直しが弾かれて「サムネ1枚だけ」の状態が公開に残る（9/23 に実際に20分残った）。並走が続くときは `DOBOKU_PW_ALLOW_PARALLEL=1` にして空きメモリガード（既定は搭載メモリ 8GB 以下で 1200MB・それ以上で 2048MB）は残し、`LaunchGuardError` のときだけ待ち直す。途中で「未ログイン」になったら `npm run auth:login -- --service coconala` で人がログインし直してから再開する。
...
- 戦略・出品文面・ヒアリングシート・撤退ライン: [ココナラ展開キット.md](../../../content/note/1級・2級土木/ココナラ展開キット.md)
- 出品文面の**構成の型**: [STRUCTURE.md](../../shared-policy/STRUCTURE.md)「強化コンポーネント」（C系PDF=直適用／S系人力=翻案・詳細は展開キット §3）
- 添削パイプライン: `/keiken-tensaku`（`civil-keiken-tensaku-drafter`）／添削テンプレ: [2級経験記述-添削テンプレ.md](../../../content/note/1級・2級土木/2級土木/2級経験記述-添削テンプレ.md)
- 売上: [sales-tracking.md](sales-tracking.md)（`coconala:<serviceId>` 命名）
- 会員（主戦場）: [noteコンテンツ計画.md](../../../content/note/1級・2級土木/noteコンテンツ計画.md)（ココナラは第3チャネル・会員の価格アンカー）
