...
## 9. ココナラブログ／コンテンツマーケット（2026-08-12 実機調査・未着手）

**現状 = 投稿ゼロ**（`/mypage/blogs` が「ブログを投稿しましょう」の空状態）。以下は着手判断のための実機確認記録で、実装はまだ無い。
...
| 項目 | 実測値 |
|---|---|
| ブログ管理 | `/mypage/blogs`（投稿導線 `?new=true`） |
| 種別 | 記事 / コンテンツ / 告知（radio 3つ→「入力へ進む」） |
| 記事エディタ | `/mypage/blogs/add?kind=1` |
| フォーム | カバー画像（`input[type=file]`・JPEG/PNG/GIF）＋ タイトル（`placeholder="ブログタイトル"`）＋ 本文（contenteditable） |
| 保存 | 「下書き保存」／「公開設定」の2ボタン |
| 効果測定 | マイページ「サービス・ブログ分析」 |
| 購入者側 | 「購入ブログ」`/mypage/blog_orders` |
...
**着手する価値はある。優先度は納品義務（未添付の解消）より下。**
着手するなら **9.3（コンテンツマーケット）から**＝在庫が既にあり、効果検証が早い。9.2 は記事執筆の継続コストが乗るので後。
