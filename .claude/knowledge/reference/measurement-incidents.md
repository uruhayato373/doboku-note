...
| 対策 | 実体 |
|---|---|
| 履歴ベース判定 | `scripts/check-weekly-review.mjs` の `existedInHistory()`（`git log --all --diff-filter=A`） |
| shallow 誤判定の防止 | 同関数で `--is-shallow-repository` を検出し「判定不能」と明示／workflow に `fetch-depth: 0` |
