...
## 2026-08-16: 「赤いのに誰も見ていない検査」— report 扱いの検査が 3 週間クラッシュし続けていた

### 現象

`quality-audit.mjs` の `note-meta-lint` が `node:fs/promises` の `glob`（Node 22+ API）を import しており、Node 20 の CI では**起動即クラッシュ**。2026-07-25 から 3 週間、1 件も検査していなかった。同じ時期、`report-monetization-coverage` は 2026-07-06 の import 破損以来 6 週間実行不能だったが `quality-audit` に登録されておらず、週次レビューは古い集計を貼り続けた。

### 根本原因

`quality-audit.mjs` の検査は `ci: true`（ゲート＝赤落ち）と `ci: false`（report＝報告のみ）に分かれ、report は落ちても CI が緑のまま通る。壊れた検査が FAIL を出し続けても誰も見ない。検出器そのものが存在しない領域（未登録スクリプト）はさらに危険。

### 対策・教訓

- 新規検査を report で追加するときは「誰がいつ読むか」を `note:` に書く。読む人がいないなら `ci:true` にするか、作らない
- 週次・月次のレビューが読むデータを生成するスクリプトは、`--check` モード（成果物を書かずに完走だけ確認）を設けて CI ゲートで実行可能性を担保する
- 規約は CLAUDE.md §9 と `.claude/rules/code.md`

## 2026-07-28: 検査ゼロの偽 PASS が 5 スクリプトで同時に起きていた

### 現象

「異常 0 件」と「1 件も検査していない」は出力が同じ緑になりやすく、後者は事故を隠す。以下 5 本が同時にその状態だった。

- `check-note-structure`: fetch がプロキシで全滅しても `FETCH_ERR` は INFO 扱いで exit 0
- `check-note-site-utm`: `join()` の `\` 区切りをパス正規表現で判定し、Windows の全量実行が常に 0 件
...
- `note-sync-tags`（実行系）: 全件取得失敗を skip で積み上げ「追加すべきタグなし（全て in-sync）」と表示して exit 0＝1 件も同期していない

その陰で有料記事 2 本が無料プレビュー 0 字、送客リンク 64 件がライブでリンク切れ、note ライブのタグが 675 本中 250 本で 90 未満のまま公開されていた。

### 対策・教訓

- 書くとき: 検査対象数と実検査数を必ず出力する／取得失敗が支配的なら exit 1（検査不成立）／ファイル判定はパス全体でなく**ファイル名**で行う（`join()` は Windows で `\` を返す）／note 記事の走査は `/^article(-[^/\\]+)?\.md$/`／外部取得は `fetch` でなく `curl --ssl-no-revoke`／実行系も「対象 0 件で何もしなかった」と「全部失敗して何もできなかった」を区別して出力し、`skip` を無言で積み上げない
- 読むとき: 緑を見たら「何件検査したか」を確認する。0 件や対象数より極端に少ない緑は故障の可能性が高い
- 規約は CLAUDE.md §9 と `.claude/rules/code.md`
