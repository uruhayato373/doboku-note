...
- **年間計画の手書きの写しが誤っていた**: `annual.md` の試験カレンダー表が正本と別に日付・受験者規模・合格発表見込みを持ち、主任技士の試験日（11/30→正 11/29）・申込締切（9/01→正 8/25）・受験者規模（1級一次 約3.5万→R7 47,715）・合格発表見込み（1級二次「2月初」→正 2027-01-08）が食い違っていた。`check-exam-calendar` は `.claude/todo/` を走査しておらず、禁止パターンも「資格名→日付」の語順とハイフン表記しか見ていなかった。
  - 対策: 表を撤去して正本と管理画面への案内だけにした。走査対象に `.claude/todo/` を加え、「日付→資格名」とスラッシュ表記も止める（`scripts/lib/exam-calendar-guards.mjs`・回帰テスト `tests/exam-calendar-guards.test.mjs`）。
- **WebFetch の要約が日付を取り違えた**: 日本技術士会の実施案内（Shift_JIS の HTML）を WebFetch で読むと、申込期間と試験日が入れ替わった要約が返った。原文は郵送 4/1〜4/15・WEB 4/1〜4/14。
  - 対策: 公式ページの日付・数値は `curl` で原文を取得して読む（Shift_JIS は `decode('cp932')`、PDF は `pdftotext -layout`）。WebFetch の要約を正本の根拠にしない。
- **調査担当（サブエージェント）の引用が原文ではなかった**: 技術士の日程の引用に「7月19日（金）」（実際は日曜）のような暦と合わない曜日が混じっていた。また 2級管工事・造園の一次を「前期＋後期の合算」と誤って説明し、公務員試験の倍率（申込者÷最終合格者）を合格率欄へ入れていた。
  - 対策: 正本の各資格に照合記録 `verification`（`checkedBy: self|agent`・`unresolved`/`pending`/`notPublished`）を持たせ、展開中の資格は主担当の原文照合（self）を必須にした。合格率は合格者÷受験者（小数1位）と一致しなければ落ちる。いずれも `scripts/lib/qualification-registry.mjs`（`npm run check-exam-calendar`・回帰テスト `tests/qualification-registry.test.mjs`）。
- **公式の結果 PDF が消える**: JCTC は過年度の結果 PDF（`datasheets2025*.pdf`）を削除する。Web Archive の保存版（`https://web.archive.org/web/2025id_/<URL>`）は gzip で返ることがあり、`gunzip` してから読む。取得元は `verification.viaArchive` に残す。
- **同じ語でも定義が違う**: 建設業振興基金の結果表の先頭列は「受検予定者数」で申込者数ではない（`plannedExaminees` に置く）。採用試験の倍率は人事院が申込者÷最終合格者、東京都が受験者÷最終合格者（`competitionRatioBasis` に定義を残す）。
- **合格発表日を追加したらサイトの「次の試験」が発表日を選びかけた**: `getNextExamEvent` は events の最も近い日付を選ぶため、発表日を入れると試験後に「合格発表」が次の試験として出る。
  - 対策: 全イベントに `kind`（application/exam/result）を必須にし、サイトは result を除外する。名前と種類の矛盾（「発表」なのに exam 等）は検査で落ちる。
...
- 原因: 有料マガジンに収録された無料記事で試し読み範囲が未設定だった。[note公式ヘルプ](https://www.help-note.com/hc/ja/articles/48514744718361)のとおり、記事タイプ「無料」だけでは未購入者への本文公開を保証しない。
- 対策と検証: 試し読みラインを末尾直前に設定し、未ログインAPIの本文14,114文字と8リンクを確認。最終行の後への設定では更新が成立せず、価格・公開ステータスだけの確認は使わなかった。学習用モデルの説明も公開範囲へ置く。
- 併発した操作の罠: `insertAfter` でURLを8件連続入力すると末尾URLが編集後検証で欠落し停止した（原因未確定）。既存の `insertBeforeHeadingHtml` によるリンク一覧挿入へ変更。`replaceText` は旧文の残存を検出するため、新文に旧文全体を含める追記型指定も検証NGになる。旧文が残らない文単位の置換にした。
- 教訓: 無料導線は未購入状態の本文と必要リンクを照合する。UIの更新クリックや `price:0` を成功証拠にしない。既存の公開後API検証が停止できたため、新しい重複ゲートは追加しない。
- 同日のCIでは、新規有料記事15本のL2導線の無料域配置漏れも検出された。`audit-note-funnel --ci` の合格だけでは `magazines/` 内の配置を保証しない。既存ゲート `node scripts/wire-note-paid-cta.mjs --check` の変更対象0件を確認する。ライブでCTAを境界直前へ移す場合、既存境界の保持だけではCTAが有料域に残るため、元の問題見出しへ境界を再設定し、未購入APIのリンクと添付PDFの不変を照合する。

## 2026-09-09: YouTubeサムネイル更新が送信上限で停止

- 現象: 採用カバー75本の更新後、`thumbnails.set` が HTTP 429 / `uploadRateLimitExceeded` を返した。停止runは34340759814。
- 原因: チャンネルのサムネイル送信上限。APIは解除時刻を返しておらず、分単位の制限とは断定できない。[公式ヘルプ](https://support.google.com/youtube/answer/72431?hl=ja)は日次上限への再試行を24時間後と案内する。
- 検出と対処: 暗号化された失敗記録を取得し、受理75本と最後の1本を読み取り専用で再照合。75本は機械照合56本＋全画像の対比目視19本で確認し、最後の1本は旧画像のままだった。再送は行わず、再開条件を `youtube-thumbnail-rollout.json` へ記録。
- 教訓: ID順の全件実行では、公開中の未更新動画が上限待ちに残る。再開時は `public_only=true` で公開分を優先する。白背景・極太文字の縦型画像はCDNの圧縮差で機械照合が厳しすぎる場合があるため、受理済みを未更新と即断せず、記録した画像を対比確認してから送信対象を決める。
