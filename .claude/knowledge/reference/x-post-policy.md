...
- 真実源の戦略は `docs/marketing/01_SNS集客戦略.md` の X 節。
...
| `pe-first-stage`（技術士第一次試験） | `#技術士` `#技術士第一次試験` | 基礎・適性・専門（建設）の論点タグを必要時に1個追加 |
...
| `rccm`（RCCM） | `#RCCM` `#建設コンサルタント` | 2026-09-15 追加。論点で `#管理技術力` 等を 1 個追加可。受験者は年 5,500 人規模でコンサル勤務が中心 |
...
| 型 | 内容 | 主な試験 | ネタ源 |
|---|---|---|---|
| キーワード解説 | 用語を 1 ツイートで要約＋サイト誘導 | 総監（キーワードページが厚い） | `content/site/{category}/{slug}/article.mdx` |
| 過去問 1 問 1 答 | 1 問の主題＋正答の論点（ネタバレ最小）＋サイト誘導 | 技術士一次 / 1級 / 2級（過去問が主資産） | `public/quiz/pe-first-stage.json` または `src/config/{civil-1,civil-2}-exam-questions.json`（IG パックと同素材） |
...
| 技術士第一次試験 | 「技術士一次 過去問 #N」 | 試験色 濃紺（`exam-palette` pe-first-stage） |
...
技術士第一次試験、1級・2級土木の試験日は `.claude/config/exam-calendar.json` を唯一の機械可読SSOTとする。投稿本文、カウントダウン、カード、販売計画へ日付を書く前に同ファイルを参照し、年度更新時は各試験実施機関の公式ページで再確認する。`npm run check-exam-calendar` が既知の誤日付とSSOT破損を検出する。
...
>
> **背景（経緯）**: 2026-06-12 凍結後、いったん完全手動・低頻度に縮退した（[01_SNS集客戦略 §凍結縮退](../../../docs/marketing/01_SNS集客戦略.md)）。**このチェックリストは §11.2/11.5 の運用値を変更しない**（頻度・間隔・機械ゲートの数値はそれぞれの節が真実源）。
...
- 戦略: `docs/marketing/01_SNS集客戦略.md`（X 節）/ `02_チャネル動線設計.md`（UTM）
...
### 実予約の定期監視（2026-09-14追加）

`npm run check-x-queue-health` は対象アカウントのX実予約を読み取り、本文冒頭と予約分を台帳と照合する。翌日から7日間の空白、未来のqueuedの欠落、7日以内の未投入計画、同分重複、日3件超、予約終了まで7日以内を検出する。Articleは予定時刻の1時間後から未公開を検出し、公開後の告知予約・告知の公開未確認も対象にする。通常投稿の公開成否はこの検査では判定しない。

結果は `.tmp/x-queue-health/latest.json`（再生成可能なローカル出力）。exit 0=実査PASS、1=対応必要、2=検査不成立。認証・別アカウント・不完全な取得・壊れた台帳は予約ゼロに変換せずBLOCKEDにし、前回PASSを失敗結果で上書きする。検査はXの予約を追加・削除せず、status.jsonも書き換えない。

Codexのこの作業タスクに紐づく監視は毎日09:30・21:30（JST）。変化のない正常時は通知せず、新しい問題・悪化・復旧を通知する。予約の再投入前には公開済み投稿と予約一覧の両方を照合し、取消や公開済みを再投稿しない。MacとCodexが稼働している時間に実行するため、停止中の常時監視を保証するものではない。Articleの時刻指定自動化も同じ実行条件になる。
