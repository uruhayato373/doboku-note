...
- [ ] **汎用の検証ステップを入れていない**（「ダブルチェック」「答える前に再確認」「検証用サブエージェントを起動」）— モデルは自律検証するため過剰検証になりトークンだけ増える。書いてよいのは**決定的ゲート**（実行コマンドと合格条件が特定できるもの: `check-*` スクリプト・lint・`curl`・件数突合）だけ。CLAUDE.md §9 準拠
