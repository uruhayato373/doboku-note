...
- **観測（面別 CTR・GA4 2026-07-02〜07-29）**: `affiliate_cta_impression` を分母にすると
  面の格差が桁で出る。サイドバー 1,218 表示 → 3 クリック（**0.25%**）、
  本文中間ネイティブカード 160 表示 → 7 クリック（**4.4%**）。
  内訳は DXConsulting-sidebar 764/2、BuildJob-sidebar 325/1、KensetsuJobs-sidebar 264/0。
  同じ位置の note CTA が 4.6% を取れている（affiliate-operations 2026-07-28 の観測）ことと合わせ、
  **案件ではなく「300×250 画像バナー」という形式が負けている**（§6.5 判断マトリクスの
  「クリック自体が僅少 → 面の問題」に該当）。
  ※ `affiliate_cta_impression` は 2026-07-25 実装のため、この CTR は**窓が分子と揃っていない**
  （分子はほぼ 4 週間・分母は約 5 日）。実際の CTR はさらに低い可能性が高く、**桁の比較にのみ使う**。
- **判定**: 面の形式変更（バナー → ネイティブカード）は**見送り**（ユーザー判断 2026-08-04）。
  記事末 300×250 は 2026-07-28 の変更から日が浅く評価不能で、同時に動かすと切り分け不能になる。
