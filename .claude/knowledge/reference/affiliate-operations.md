...
2. **1 ページ 1 ピクセル**（同一 mat の二重発火を避ける）。複数面に同案件を出す場合、発火源を 1 つに決め他は href のみ。docs 記事は 2026-09-26 から `DocPage.tsx` が決める: 本文に転職広告（手書き inline／中間カード）があれば本文側で 1 発、無ければ記事末バナーで 1 発、どちらも無いページは 0 発（広告を出さないページで発火させない）
...
1. **面ごとに広告の型を分けた**。デスクトップが流入の 82%（8,557 sessions）を占めるのに
   ...
   - **ピクセルはサイドバー側のみ**が持ち、記事末・本文中間は href のみ＝「1 ページ 1 ピクセル」不変（当時。2026-09-26 のサイドバー撤去後は本文の転職広告、無ければ記事末バナーが 1 発＝「2026-09-26（DN-0322）」節）
...
- ピクセルは引き続きサイドバー 1 箇所のみ＝「1 ページ 1 ピクセル」不変（当時。2026-09-26 以降の発火源は「バナー実装の必須ルール 2」）
...
### 2026-09-26（DN-0322・記事サイドバー撤去）

- **判断**: 記事サイドバーの転職広告を撤去（ユーザー決定）。GA4 2026-08-20〜09-16 で表示 12,673・クリック 0。表示は IntersectionObserver で 50% 以上見えたときだけ数える（`AnalyticsProvider.tsx`）ので、PC 限定表示のサイドバーでもモバイルの非表示分は数えていない＝実表示に対する 0 クリック。9/22 の「継続」判定は露出の総量の話で、クリックの無い面を残す根拠にはならない。
- **ピクセル**: サイドバーが唯一の発火源だったため、発火源を本文／記事末へ移した（上の「必須ルール 2」）。build 後 HTML 1,987 ページで、転職広告のあるページは全て 1 発・広告の無いページは 0 発、同一 mat の二重発火 0（2 発のページ 25 は転職＋ココナラの別プログラム）。
...
## 将来の掲載先（X・Threads）— 記録のみ（2026-09-26）

アフィリエイトの掲載先はサイト・note に加えて X・Threads も考えられるが、今は着手しない。着手するときは次を先に確かめる: (1) 案件ごとの SNS 掲載可否（禁止案件がある）(2) X の凍結対策（`x-post-policy` §11・アフィリリンク直貼りはスパム判定リスク）(3) 成果を分けるため A8 に掲載先ごとの別サイトを登録する（note は 2026-09-26 に副サイト 006＝`doboku-note（note）` として登録済み・`a8-report-automation.json` の `relatedSites`）。投稿はアフィリリンクを直接貼らず、サイトの記事へ送る形を第一候補にする（クリックはサイト側の GA4 で取れる）。掲載先ごとのリンク所在は管理画面 `/affiliate/placements`（`scripts/lib/affiliate-placements.mjs`）が数える。
