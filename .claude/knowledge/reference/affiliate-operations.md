...
- CTA の縦順序は全ページ一律にしない。学習意図では note を主導線、キャリア意図では転職アフィリエイトを主導線にする。非キャリアの学習記事では、文脈に合う本文中の転職 CTA は補完として残し、最初の自動中間 CTA と記事末タイルで note の優先を確保する。DOM 上の前後関係だけを理由に一括移動せず、配置別の計測結果が出たときに見直す
...
| 案件 | 現行の面 | 適用 |
|---|---|---|
| ビルドジョブ | civil等の記事サイドバー・本文カード・記事末（既存の表示条件を維持）、カテゴリhubは1枠 | 2026-09-08 JST以降のビルド。通常条件で継続 |
| 建設JOBs | 一般配置停止。creativeは過去分岐として保持 | 対象年齢・経験・電話確認条件を踏まえ再設計する場合のみ検討 |
| GKSキャリア | 一般配置停止。creativeは過去分岐として保持 | A8終了予定2026-09-28を実機確認 |
| ハイクラス DX・コンサル | 総監の記事サイドバー・記事末・hub | 既存配置継続。今回の条件照合対象外 |
...
旧期間内で `true` にすると従来のハッシュ A/B（arm A = GKS）になる。根拠は §6.5 裁定ログ 2026-08-04。
...
- **CTAの内部キー `.consultation` / `.registration` は報酬条件そのものを表さない。** 建設JOBsも相談・求人紹介を伴い、A8では20〜30代・建設経験・30日以内の電話本人確認が承認条件。文面は実際の登録後の流れを伝え、成果条件は現行カタログで確認する。
...
- **凍結済みの基線は上書きされない**（2026-09-22 追加）。ファイル名は GA4 窓の終端日なので、
  同じ窓の取得データのまま記事だけ増やして再実行すると、名前は同じまま `careerArticles` が
  増えて差分が消える。実際に第 2 波の出荷後に再実行して 39 本の基線が 44 本へ書き換わった。
  既存があれば `--freeze` は exit 1 で中止し、`-latest` も書かない。意図した撮り直しだけ
  `--refreeze` で許す
...
既存の note L1/L2 再編は2026-09-05にライブ反映まで完了済み。キャリア L2 の追加判断では、
この現行ファネルを基線として比較する。
...
| 観測 | 解釈 | 打ち手 |
|---|---|---|
| 確定成果 3 件未満 | 判定不能（母数不足） | EPC比較だけを根拠に撤去しない。対象条件の不適合・案件終了・訴求との不一致は別判断 |
...
> [!important] 2026-09-08の現行方針
> 8月BuildJob増額期、9/1〜7の旧配置、9/8以降のBuildJob通常条件を分ける。SSGなので公開切替日は実際の本番再ビルド・デプロイ日時で確定する。時系列比較には季節性・単価・配置変更が混ざり、案件の勝敗の証明にはならない。
> 以下は旧A/B設計の検証記録。現行配置や再実行手順として使わない。
...
- **観測**（取得は hosted CI の a8 cron・`a8-results.json` 2026-05〜2026-08 / GA4 2026-08-20〜09-16）:
  ...
  254 表示 / 2 クリック。
- **判定**: **(1) 継続（追加投資なし）**。当初は (2) 露出を絞る を推していたが、根拠にした
  機会費用が成り立たないため撤回した。理由は 3 点。
...
- **backlog**: DN-0120 は本記録で閉じる。以降の再計測は第 2 波の新 DN が持つ。
...
- 建設JOBsの登録だけという前提を訂正し、GKSの対象地域・年齢と9/28終了予定を確認。EPCの優劣判定ではなく、一般読者との対象条件の不一致から一般配置を停止。
- BuildJobを通常条件で継続。もしもとの差313円だけで既存リンクと成果収集を切り替えない。ASP公開のEPC/確定率は自サイト実績として扱わない。
- 準備シートの編集元は `public/downloads/career-preparation.txt`。工事経験・希望条件・連絡希望・求人比較に使用し、記入内容をサーバーへ送信しない。`/tools/career-check` の結果から保存できる。
- GA4収集に `career_tool_result`（結果表示）、`career_checklist_copy`（質問のコピー）、`career_checklist_download`（記入シート保存）を追加。広告クリックと別々に追う。ダウンロードイベントはツール内のボタンに限定し、記事の直接リンクからの保存を含まない。
- 新規候補はRSG「職人から施工管理エージェント」。40歳未満の職人・現場作業員向けに限定されるため、一般の有資格施工管理記事へ一律追加しない。未申請。コンストワークは提携済みだが電話面談完了が承認条件。両者の掲載は対象記事・計測を揃える別検討。
