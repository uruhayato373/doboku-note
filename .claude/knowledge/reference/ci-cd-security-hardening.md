...
- **Instagram Graph API トークンには `instagram_content_publish` scope が必要**（`ig-graph-publish.mjs` が投稿・カルーセル・リール・ストーリーズを作成する。read-only の `instagram_basic` だけでは公開できない）。トークン発行時に scope を確認する
