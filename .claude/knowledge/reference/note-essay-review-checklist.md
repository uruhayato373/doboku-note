...
- 比較対象: `content/note/magazines/総監模範論文-{persona}/R07/article.md`
...
> **予想テーマの選定理由の表現（2026-06-10 是正）**: 予想テーマは白書重点テーマ＋的中率から「**出題が予想される**」ため選定する。「**ペルソナで論じやすい**（から取り上げる）」と書いてはならない（論じやすさで選んだのではない、というユーザー指摘）。出題予想根拠の直後は「本記事では、その中でも出題が予想される「{テーマ}」を{ペルソナ}の視点で取り上げます。」の型で接続する。複数テーマから絞る場合も「出題可能性が高い上位N テーマ」とし「論じやすい上位…」としない。`grep -rn "論じやすい" content/note/技術士総監/magazines/` = 0 を確認する。
...
`content/note/技術士総監/magazines/総監模範論文-{persona}/note掲載文.txt` を作る。note の入力欄制限に合わせ、以下を厳守：
