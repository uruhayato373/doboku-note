...
不足の解消は `node scripts/note-sync-tags.mjs --list <対象> --commit`（差分だけ追加・本文と有料境界は非破壊）。ライブが上限99で埋まっていると足す余地が無く、原稿を直しても一致しない（2026-09-23 土木もくじ: 余分11・不足7）。dry-run の `余分=N` がその印で、`--prune --commit` で原稿に無いタグを外してから足し、ライブを原稿と同じ集合にする。比較は大文字小文字を区別しない（note は原稿の `GX` を足しても既存の `gx` のまま残す）。タグ欄は `-` `.` `/` を含むタグを受け付けない（Enter で chip にならない・`_` は可）ので、原稿では `_` に置き換え、`check-note-hashtags` が止める。会員限定記事は未ログインの公開 API がタグを空で返すので、dry-run では `[defer]` として計画せず、`--commit` 時にログイン済みブラウザの API で読んでから計画・検証する。構造的にライブへ入らない記事は `.claude/config/note-live-tags-allow.json` に**理由つきで**免除する。
