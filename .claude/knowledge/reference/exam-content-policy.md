...
- **参考文献区分**: 試験元 = 公益財団法人 全国建設研修センター（CECC）。過去問の利用条件と出典粒度は [reference-sources-policy.md §1](./reference-sources-policy.md#1-参考文献の区分) の `exam-official` に従う
...
- **出典表記**: ファイル末尾 `**関連コンテンツ**` の直前に `## 出典` セクション必須（内容は `exam-official` の規則に従う）
...
- **テキスト**: 原典（技報堂スキャン）は [reference-sources-policy.md §1](./reference-sources-policy.md#1-参考文献の区分) の `commercial-book` として扱う。**2026-07-31 に原典図 25 枚を全数置換**（自作 SVG 21・生成画像 3・本文の表と重複のため削除 1）。書籍ページ撮影の webp はゼロ。
...
- **note 連動**: 記述式マガジン `cd-essay-magazine`（8記事・¥2,980／単品¥1,480）を **2026-07-31 公開**（`m/mf2a132408b6f`）。サイト側の CTA は `guide-essay` の冒頭（top）と `textbook-assessment` / `textbook-repair` の MDX 内 `<MagazineCard>`。**診断士は全記事が本文 8,000 字未満で中間 CTA の発火条件を満たさず、concrete 系は非 HUB 資格でもくじタイルも出ない**ため、この 2 経路以外では CTA が出ない（発火条件の一覧は `src/lib/magazine-placement.ts` の診断士ブロックのコメント）。追加9商品の構成・価格・公開条件は`content/note/コンクリート診断士/noteコンテンツ計画.md`を参照する。
- **残課題**: 一次演習98問、既存記述式8本、`guide-essay`、新規構造物別8本の有資格者レビュー。択一98問PDFとまるごとパックはKDP Select満了後の2026-11-26以降に公開する → `.claude/todo/backlog.md`。
...
- **過去問の年度範囲（2026-07-17 拡張）**: **平成24〜令和5年度・計303問**（分野別8記事に `### {年度} 問題N` で横断収録）。2026-07-17 に H24（26問）・H25（12問）を 2022年版底本から追加。**H24/H25 は OCR 品質がまだらで、選択肢文が復元不能に破綻した問題・公式解答表と技術判断が食い違う問題は収録せず skip**。未収録は H25 の18問（Q1,3,4,5,7,8,9,10,12,13,14,15,16,17,19,20,21,26）と H24 の conflict 4問（Q14,16,17,18）。正答は各年度の公式解答表から機械照合済み。**残の年度拡張余地**: R6・R7（原典スキャン未入手）と上記22問。原典入手後に視覚照合し、解答キーに合わせた本文の再構成はしない。年度表記は「令和元年度」へ統一済み（2026-09-04）。
...
**素材と参考文献区分**: `content/sources/textbook/土木施工実務ノート/` は市販書のスキャンから作った
内部ノートであり、[reference-sources-policy.md §1](./reference-sources-policy.md#1-参考文献の区分) の
`commercial-book` に従う。ノート内の `<!-- 要確認 -->` 箇所は公開物に使わず、公開用の図は
figure-canvas 標準で別途作図する。構成を独自編集へ転換した前例は同 policy §5 に集約する。
