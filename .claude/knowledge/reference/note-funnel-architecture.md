...
| `npm run check-note-attachments -- --live` | **live 実査**（要 note ログイン・575 件で約20 分・2026-09-07 実測）。有料エリアの添付カードは未ログイン HTML に出ないため CI では見えない。`--only {noteId,…}` で単発確認。結果は `.claude/state/note-attachments-missing.json` に永続化 |
