...
> - **マガジンの収録記事（期待値）** = 各記事 frontmatter の `noteMagazine` ラベル ＋ `.claude/config/note-magazine-membership.json`（ラベル→マガジン id、`packs` の `labels` / `fromMagazines`、dir 外収録の `extras`）。検査は `npm run check-magazine-membership`（repo 実数 ↔ SoT の price 件数 ↔ ライブ snapshot の**三軸**）。SoT の件数表記は真実源ではなく**検証される側**。収録リストを別に手書きしないのは、それ自体が新たなドリフト源になるため（2026-08-24 制定）。**束ね商品**（まるごと・完全パック）は `fromMagazines` の値を `"all"`（丸ごと同梱）にし、軸 D がライブの**記事 key の包含**（構成元の全記事 ⊆ パック）を照合する。本数指定は選抜同梱だけで `partialReason` 必須。本数で書くと欠けを正解として凍結する（2026-09-25 二次検定まるごとパックが 169 本中 101 本のまま 1 か月緑だった）
...
8. **サブ資格別で冒頭パック CTA を差し替える（`topCtaOverrides`）** — 1 資格の下に「1級／2級」のようなサブ資格が同居する場合、冒頭 CTA はサブ資格ごとに向き先が変わる（2級読者に1級商品を出すのはセグメント違反）。config の `exams.{key}.topCtaOverrides`（`[{dirPrefix, marker, text}]`）で**ディレクトリ接頭辞ごとに topCta 文面/URL を差し替える**。civil は販売導線の入口を高額な網羅パックから完成答案集へ変更し、1級は `m150c9db08902`、2級は `m1881a9578027` へ着地させる（2026-09-06、2級の計測で完成答案集CTR 10.4%、想定工事バンク0.8%を確認）。マーカーは共通（`cta:pack-top`）なので D1 監査はそのまま効き、D5 のライブ反映ターゲットは記事ごとに override 先頭 URL で判定する。
...
| `npm run check-note-funnel` | CI ゲート（`audit --ci`、**ソースのみ**でドリフト exit 1＝D1-D4・D6・高速）。review surfacer は非ゲート。`content-quality-audit.yml`（毎週月曜 07:15 JST・`ref: main`）で発火。**review 候補は機械の読み手を置かない**（ack 台帳が無く、判定済みの候補が毎週再浮上してノイズになるため）。意味判定は `/audit-note-funnel --semantic` をオンデマンドで回す |
| `npm run check-note-republish` | **本文＋ハッシュタグの再公開ドリフト検出**（surfacer・creds不要・ローカルhash突合）。公開記事のソース本文ハッシュ（`.claude/state/note-republish-hashes.json` の `hashes`）と**ハッシュタグ hash（同 state の `tagHashes`・`hashtags*.txt` 単位）**を現ソースと突合し「要再公開（本文drift／タグdrift）」を各々列挙。**D5 が CTA の live 反映を追うのに対し、こちらは blockquote/UTM/本文改稿など CTA 以外の全本文変更＋タグ変更を追う**（直交・補完）。in-sync 化: 本文＝`note-publish`／`note-update-body --commit`、タグ＝`note-publish`（Phase10でタグ適用）／`note-sync-tags`（公開済み記事へのタグ差分適用）。`note-update-body` はタグ非適用のためタグ hash は記録しない。`note-append-cta` は非hook。正常な中間状態も含むためゲートにはせず、`note-live-audit.yml` が週次 artifact に JSON を保存する。baseline は `--baseline --since <ref>`（本文・タグ両方をseed）。**本文 drift のうち記録時の版との差分が旧 `/docs` → 新 URL（`public/_redirects`・クエリ保持）の張り替えだけのものは「301 等価」として要再公開から外し別に数える**（live は 301 で同じ行き先＝再公開不要・JSON は `equivalentDriftFiles`）。記録時の版の canon は台帳 `canonHashes`（`recordPublishedHash` が書く）か git 履歴から取り、どちらも無い記事は「判定できず」として要再公開に残す（浅い clone の CI 用にローカルで `--record-canon` を回して台帳を commit する・2026-09-24 DN-0297） |
| `npm run wire-note-funnel-cta -- --exam {key} [--sync-managed] [--apply]` | 資格別に L3 冒頭/末尾 CTA を**ソースへ**冪等配線（既定は dry-run）。`--sync-managed` は管理マーカー付き冒頭 CTA の差し替え・除外時の削除だけを許可 |
...
| `npm run note-update-partial -- --spec {json} [--commit]` / `--list {txt} [--start N]` | 公開済み記事の語句・カード・節順・本文画像を限定更新。dry-run は read-only、全 operation の対象件数を先に照合し、select-all を使わない。複数記事は同一 Chrome セッションで直列処理し、途中停止後は `--start` で再開する。PDF 添付は更新前・編集直後・公開後再読で完全一致を要求 |
| `npm run check-magazine-cta` / `:ci` | **サイト側**の CTA 到達性ゲート（note 内の導線を見る他ツールと直交）。公開マガジンが `/docs` のどこかで実際に CTA として出るかを、`placement.top` / 中間 CTA（group と h2>=5・本文 8,000 字の発火条件込み）/ MDX 内 `<MagazineCard>` / **記事サイドバーの商品カード**（非 HUB の discovery 資格・`top || inline[0]`・中間 CTA の h2/字数ゲートを通らない）/ **カテゴリページの商品カード**（固定商品）の 5 経路で判定する（2026-09-22 に後ろ 2 経路を追加。それまで非 HUB 資格の到達を過小評価していた）。もくじタイル（HUB 資格）は資格単位でマガジン単位ではないため到達に数えない。**`published: true` にしても CTA が 1 面も出ない**事故（2026-07-31 コンクリート診断士）の再発防止。既存の 0 面は `.claude/config/magazine-cta-baseline.json` に理由付きで計上し、**baseline 外の新規 0 面だけ落とす**ラチェット方式。`placement.sidebar` は 2026-07 の CTA 統一以降どこからも参照されない死に配線なので、配線しても到達手段にならない |
...
| `npm run check-note-attachments -- --live` | **live 実査**（要 note ログイン・575 件で 20〜35 分・2026-09-07 実測）。有料エリアの添付カードは未ログイン HTML に出ないため CI では見えない。`--only {noteId,…}` で単発確認。結果は `.claude/state/note-attachments-missing.json` に永続化 |
...
- **週次 CI（ソース）** — `content-quality-audit.yml` の `check-note-funnel` が D1-D4/D6 を赤落ちで機械検知
