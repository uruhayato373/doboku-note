...
npm run check-note-public-view # note 公開記事を未ログインの読者の見え方で検査（公開 API: 添付 PDF の本数・価格・カバー・無料記事の全文会員限定／ブラウザ・スマホ幅: 画像の読み込み・リンクカード・はみ出し）。週次 note-public-view.yml。`-- --api-only` で API 層だけ・`-- --sample 40` でブラウザ層を抽出。手元はシステム Chrome、CI は同梱 Chromium。例外台帳は .claude/config/note-public-view.json
