...
npm run check-note-public-view # note 公開記事を未ログインの読者の見え方で検査（公開 API: 添付 PDF の本数・価格・カバー・無料記事の全文会員限定／ブラウザ・スマホ幅: 画像の読み込み・リンクカード・はみ出し）。週次 note-public-view.yml。`-- --api-only` で API 層だけ・`-- --sample 40` でブラウザ層を抽出。手元はシステム Chrome、CI は同梱 Chromium。例外台帳は .claude/config/note-public-view.json。`-- --review-shots N` で目視確認用に N ページの「最初の画面」と「有料エリア直前」を撮る（判定には使わない・週ごとに抽出をずらす・週次 CI は 24）
npm run fetch-note-public-view-shots # 週次 note-public-view の目視確認用の画像を .tmp/note-public-view-review/<runId>/ へ取る（/weekly-review でエージェントが見る）。exit 2 は未確認（定期 run 無し・成果物切れ）。`-- --run <id>` で任意の run
...
npm run ig-graph-publish -- --pack <pack> --format carousel --commit --json # Instagram Graph API で即時公開（Meta 利用制限中は使えない・代替経路。主経路は ops-write の instagram.publish-bs。予約不可・publish-ig-bs とは別経路）。env: IG_GRAPH_ACCESS_TOKEN / IG_BUSINESS_ACCOUNT_ID / IG_GRAPH_API_VERSION。罠: 投稿用メディアは public R2 に一時公開されるため stage-ig-media-r2 の --cleanup 実行を確認する（残すと公開URLが残置）
