...
npm run check-note-public-view # note 公開記事を未ログインの読者の見え方で検査（公開 API: 添付 PDF の本数・価格・カバー・無料記事の全文会員限定／ブラウザ・いちばん狭い帯 360px: 画像の読み込み・リンクカード・はみ出し）。`-- --review` で代表ページ（資格×記事の種類ごと 1 本）を note のブレイクポイントの帯ごとの画面幅（.claude/config/public-view-breakpoints.json）で検査・撮影し、CSS の切り替わり幅の変化も WARN。週次 note-public-view.yml。`-- --api-only` で API 層だけ・`-- --sample 40` でブラウザ層を抽出。手元はシステム Chrome、CI は同梱 Chromium。5xx は 1 回やり直し、それでも 5xx なら「開けない」に数える。例外台帳は .claude/config/note-public-view.json
