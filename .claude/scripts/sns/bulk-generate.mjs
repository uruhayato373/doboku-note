 ...
 *
 * 使い方:
 *   node bulk-generate.mjs --slug-list docs/sns/priority-slugs.txt --start-date 2026-05-13 --mode both
 ...
 *
 * 出力:
 *   docs/sns/youtube/{ytDate}-{slug}/    (yt または both)
 *   docs/sns/instagram/{igDate}-{slug}/  (ig または both)
 *   docs/sns/schedule.json               (常に出力)
 */
...
const YT_OUT_DIR = resolve(ROOT, 'docs/sns/youtube');
const IG_OUT_DIR = resolve(ROOT, 'docs/sns/instagram');
const POSTS_DIR  = resolve(ROOT, '.local/r2/posts');
...
console.log(`\n完了: ${ok} 件成功 / ${ng} 件エラー`);
console.log(`スケジュール → docs/sns/schedule.json`);

function writeSchedule(data) {
  const outPath = resolve(ROOT, 'docs/sns/schedule.json');
