...
// `content/site/<category>/dir/article.mdx` → `<category>-dir` (Convention B)
...
function main() {
  ...
  for (const filePath of files) {
    ...
    const entry = {
      ...
      exams,
      // relative() は OS 依存の区切りを返す（Windows は円記号）。CI は Linux なので、
      // ローカルビルドの成果物を commit すると区切りだけが全行入れ替わって汚染される。
      // 生成物は OS 非依存にする（2026-08-18・report-monetization-coverage と同型）。
      source_path: relative(ROOT, filePath).split(sep).join('/'),
    };
