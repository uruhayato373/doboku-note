...
実行: 2026-09-04T11:21:15.614Z / branch `develop` / HEAD `c028086a9b`
...
| 指標 | 値 | 意味 |
| --- | --- | --- |
| ワークツリー | 5.46 GiB | 作業ディレクトリの実容量（追跡外含む・.git 除く） |
| HEAD 追跡 | 0.48 GiB / 10998 files | clone 直後に checkout される量。Phase 6 の削減目標対象 |
| Git 履歴 (pack) | 1.05 GiB | 過去 blob 込み。通常 commit では減らない（Phase 7 の別承認） |

garbage: 0 bytes / in-pack objects: 120,312

## 2. 分類サマリ

| 分類 | 容量 | 件数 |
| --- | ---: | ---: |
| **KEEP_GIT** | 392.6 MiB | 10426 |
| **REVIEW** | 96.8 MiB | 565 |
| **R2_PRIVATE** | 0.3 MiB | 7 |

### KEEP_GIT — 392.6 MiB / 10426 files

- **`text-ssot`** — 170.2 MiB / 8362 files
  - 理由: 原稿・設定・コードの SSOT
...
- **`obsidian-env`** — 5.9 MiB / 64 files
  - 理由: Obsidian のプラグイン・アイコン素材。制作物ではなくエディタ環境で、全 PC 共有が要る（git-binary-policy.json allowlist で KEEP_GIT 判定済み）
  - 参照コード: `Obsidian エディタ`
...
### REVIEW — 96.8 MiB / 565 files

- **`(unmatched)`** — 31.2 MiB / 550 files
  - 理由: どのルールにも当たらない。置き場の方針が未定義
- **`obsidian-env`** — 21.9 MiB / 4 files
  - 理由: Obsidian のプラグイン・アイコン素材。制作物ではなくエディタ環境で、全 PC 共有が要る（git-binary-policy.json allowlist で KEEP_GIT 判定済み）
  - 参照コード: `Obsidian エディタ`
- **`kindle-artifact`** — 20.9 MiB / 6 files
  - 理由: KDP 入稿成果物。scripts/kindle-dist/ 配下は R2 バックアップ済み（kindle-dist group・2026-08-29 決着）。kindle-published/・kindle-covers/ は cover-designs（P6 で repo-archive group へ退避済み）を除き未バックアップだが、いずれも git が正典で CI（check-kindle-format.mjs 等）が git 実体に依存するため untrack はしない
  - 再生成入力: content/kindle/books/**（要検証）
  - 生成器: `kindle-build`
  - 参照コード: `scripts/kdp-publish.mjs`
- **`text-ssot`** — 11 MiB / 2 files
  - 理由: 原稿・設定・コードの SSOT
- **`public-static`** — 6.7 MiB / 2 files
  - 理由: サイトが直接配信する静的アセットの原本。R2 ではなく Next.js のビルド入力（git-binary-policy.json allowlist で既に判定済み）
  - 参照コード: `Next.js static assets`
...
| 容量 | 分類 | パス |
| ---: | --- | --- |
| 7.8 MiB | REVIEW | `.claude/state/assets/manifest.json` |
...
| 4.6 MiB | REVIEW | `public/quiz/pe-first-stage.json` |
| 4.3 MiB | REVIEW | `scripts/kindle-dist/d-00.epub` |
