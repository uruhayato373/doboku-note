...
> **やる月はカードの `[時期:YYYY-MM]`（または `YYYY-MM..YYYY-MM`）で決める**。年間ロードマップ（管理画面 計画 ＞ 年間ロードマップ）と月間（`[時期:]` が今月を含むカード）はこの値から自動で決まり、`monthly.md` にタスク表は書かない。月初に `todo-planner` が今月やるカードへ `[時期:]` を付ける・直す。
...
### [DN-0356] プロフィールと権威性バナーの保有資格に、技術士（上下水道部門）と測量士を加える
タグ: [収益化] [領域:商品] [時期:2026-10] [種類:改善] [起票:2026-09-26]

**起点**: 2026-09-26 にユーザーから、上下水道・舗装・測量士を既に取得していると聞き、3 資格の展開を決めた。`content/note/プロフィール.md` には 1級舗装施工管理技術者はあるが、上下水道と測量士が無い。答案の信頼性を示す一番の根拠なので、展開した資格の商品より先に載せる。

**やること**: 正式名称（技術士の部門・選択科目、取得年）をユーザーに確認し、プロフィール（note・サイト・ココナラ）と権威性バナー（author-authority-banner.md の配布元）を直す。

**完了条件**: 各チャネルのプロフィールに載り、権威性バナーの再配布が要るかを決めたら、このカードを削除する。
...
### [DN-0354] 測量士の商品設計を決めて、午後記述（計算）の解き方をまとめた商品を作る（測量士補を加えるかも決める）
タグ: [収益化] [領域:商品] [時期:2026-11..2027-02] [種類:制作] [起票:2026-09-26] [期日:2027-03-15]

**起点**: 2026-09-26 にユーザーが測量士の展開を決め、資格台帳を active にした（ユーザーは取得済み）。試験は 5 月で、買われる時期（3〜5 月）が夏の山と重ならず平準化に効く。出題は午前 択一・午後 記述（計算を含む）で、経験記述や論文は無い。国土地理院が過去 5 か年の問題と解答例を公開しているので、価値は「解答例までの解き方」と計算の型にある。測量士補は同じ日の択一のみで受験者が多い（exam-stats.json）。

**やること**: 午後記述の分野（基準点・地形写真・地図編集・応用測量）ごとに、解き方の型と過去問の解説をまとめた商品の構成と価格を決めて作る。測量士補を同時に展開するか（資格台帳の portfolio）を決める。

**完了条件**: 公開して note-magazines.ts（id は `surveyor-` 始まり）・product-lineup.json・salesRules に載せ、測量士補の判断を 06_多資格展開戦略.md の判断の記録に書いたら、このカードを削除する。

### [DN-0355] サイトに舗装施工管理技術者と測量士の入口（出題傾向・勉強法・もくじ）を作る
タグ: [収益化] [領域:サイト] [時期:2027-01..2027-03] [種類:制作] [起票:2026-09-26]

**起点**: 舗装・測量士は資格台帳で active になったが、サイトに入口が無い（トップの資格カード・categories.json・/exam/ 配下）。買われる時期（測量士 3〜5 月・舗装 5〜6 月）より前に検索の入口と note への導線を用意する。
