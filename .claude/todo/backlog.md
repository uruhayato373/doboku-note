...
**完了条件**: `node scripts/check-note-republish.mjs --json` の `driftFiles` に上の 40 本が 1 本も無い。各記事の添付 PDF がライブで 1 つ残っている（スクリプトの `[5e] API 実体検証 OK` と添付確認のログで確かめる）。
...
### [DN-0294] note の PDF なし記事 169 本の本文を再公開し、旧 `/docs/` リンクと 404 リンク 2 本を note 上から消す
タグ: [SNS・マーケ] [種類:改善] [起票:2026-09-24]

**起点**: PR #598（2026-09-24 マージ）で note 原稿の旧 `/docs/` リンクを新 URL に張り替え、`コンクリート主任技士/配合計算-実戦演習`（n5a55ae6dc16b）の 404 リンク 2 本も直した。note.com 上は再公開するまで旧 URL のまま（`check-note-republish` の本文 drift 674 本のうち 667 本が #598 由来。9/23 の全文更新は #598 より前）。旧 `/docs/` は本番で 301・UTM 保持のまま転送されるので、実害は 404 の 2 本だけ。2026-09-24 に本数・順番をユーザーと決め、PDF 付きほか 505 本は DN-0295 に分けた。

**やること**: Windows PC から `node scripts/note-update-body.mjs --list <list> --commit` で流す（CI の ops-write は 1 回 1 記事で PDF も付け直せないため使わない。会社 PC は `DOBOKU_PW_MIN_FREE_MB=500`、画像の CDN 待ちで止まるなら `NOTE_IMG_SETTLE_*` を延ばす）。PV は `.claude/state/metrics/note/articles-pv-2026-08.json` と `-09.json` をタイトルで突き合わせて並べる。

1. 第 0 段: `配合計算-実戦演習` 1 本。dry-run → `--commit`。note 上で 404 の 2 本が消え、`verify-note-status` で有料境界が保たれていることを確かめる
2. 第 1 段: PV 上位の無料 2 本・有料 3 本で試し、有料エリア・画像・リンクカード・目次を note 上で見る
3. 第 2 段: PDF なしで PV 5 以上（第 0・1 段を含め 81 本・画像 78 枚）を PV 順に。1 日 40 本前後を 2 回に分ける（アップロードは 1 日 90 件まで）。第 0〜2 段は 2026-09-24 夜に実施する
4. 第 3 段: 残りの PDF なし 88 本（画像約 40 枚）を翌日以降に

**完了条件**: `node scripts/check-note-republish.mjs` の本文 drift に、PDF なしの通常記事 169 本が残っていない。再公開した本文から 20 本を抜き出し、note 上に旧 `/docs/` と 404 のリンクが無いことを確認済み。
...
### [DN-0295] note 本文 drift の残り 505 本（PDF 付き 491 本ほか）を再公開するか決める
タグ: [SNS・マーケ] [種類:意思決定] [起票:2026-09-24]

**起点**: DN-0294 で PDF なしの通常記事 169 本を再公開する。残りは PDF 付き 491 本（PDF 506 件・8〜9 月の PV 6,378・中断記録のある 2 本を含む）、会員限定 6 本、PDF なしで中断記録のある 1 本（総監 設問3 序章）、noteId の無い 7 本（2026-09-24 時点）。旧 `/docs/` リンクは 301・UTM 保持で転送されるので、再公開で変わるのは転送 1 回分だけ。一方で全文置換は PDF 添付を消し（2026-07-28・07-31 の事故）、付け直せるのはローカルの `--reattach-pdf` だけ。PDF 実ファイルは Windows PC に 4 / 506 件しか無く、アップロードは 1 日 90 件まで（PDF と画像で約 650 件＝8 日前後）。

**やること**: PDF 付きを (a) PV 上位 50 本だけ（PV 1,964・Drive vault から PDF を戻してローカル実行）(b) 保留して各記事の次の改稿時に反映 (c) 全件を約 8 日で、のどれにするか決める（推奨は b）。会員限定 6 本は記事ごとに `--keep-member-lock` か `--trial-line-bottom`、中断記録のある 3 本は `check-note-attachments --live --only` でライブを確かめてから `--force-retry`、noteId の無い 7 本は noteId の書き戻しから。

**完了条件**: 方針を決め、実行するなら実行カードを起票する。やらないと決めたら本カードを削除する。
