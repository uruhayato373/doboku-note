...
品質・安全の「書き方」2本が、検討項目を②側に置く2級式の説明のまま**公開中**。1級は検討項目が
(1)側なので1区画約200字に収まらない。送客先はどちらも1級商品。同じ誤りが C8 予想模試にもあり、
購入者から「文字数がオーバーする」と指摘されて発覚した（2026-08-30・DM 10051134）。

**工程管理は `status: draft` で未公開**だったので対象外（当初「3本」としていたのは誤り）。

**原稿側は 2026-08-31 に修正を完了している。**56c62e61 の修正は不完全で、その後に判明した
3 件も直した: hinshitsu の「設問2では検討」2箇所 / anzen 冒頭の「設問2の検討」/ 4記事の太字記法
`**`（ココナラのエディタは解釈せずアスタリスクがそのまま出る）。

- **貼り替えは docs/handoffs/2026-08-31-coconala-c8-moshi-correction.md §6**（現原稿から機械抽出）。
  品質=置換4+挿入1 / 安全=置換2+挿入1
- 更新導線は編集画面 →「公開設定」→「投稿する」（実機確認済み）。**「下書き保存」は押さない**
- **自動化は見送り**。`coconala-blog-publish` は二重公開ガードで公開済みに使えず、更新には
  本文を消して入れ直す処理が要る。同スクリプトに過去2回の破損（本文が500/1684字で切断・
  段落順の破壊）が実測記録されており、公開中の記事を壊すリスクに見合わない
- 再発は `npm run check-keiken-answer-split` と `check-coconala-blog`（太字記法）が止める

### [DN-0158] PSI の field(CrUX) が全 22 URL で null のまま — 供給停止か判定方法かを決める
タグ: [インフラ・計測] [種類:意思決定] [起票:2026-08-31] [検証:psi-audit:check]

`psi-config.json` は `primary_source: field` ＋ `min_field_coverage: 1` なので、field が 0 件だと
CI ゲートが恒久的に赤になる（「実害なし」でも「判定材料なし」でもない状態）。
2026-08-31 時点で **fetch は 22/22 成功・lab も正常・error 0 件**、それでも field_data は全 null。

**config に書かれている「2026-08-18 に CrUX 側の供給が停止した」という診断は、根拠が弱い。**
実データを見ると 08-18 のバッチは **全件 `PSI API 500: Lighthouse returned error`** で、
field どころか計測自体が失敗している。つまり「取得・パースは正常」という前提が
その日のバッチには当てはまらない。08-17 までは 21/22 で field あり（トップページ含む）。

やること: (1) 最新バッチで `loadingExperience` / `originLoadingExperience` が API 応答に
そもそも含まれているかを生レスポンスで確認する（バッチ JSON はパース後しか保存していないので
生を1件だけ保存する必要がある）。(2) origin レベルに data があって URL レベルに無いなら
サイトが CrUX の最小トラフィック閾値を割ったということなので、field 判定は諦めて
`primary_source` を lab 中央値ベースへ切り替える（config の `_min_field_coverage_note` が
その判断を保留している）。(3) origin にも無いなら CrUX 全体の供給問題として記録する。

**判断はユーザーに委ねる**（閾値を下げる／lab へ切り替えるのは計測方針の変更なので勝手にやらない）。
関連: 2026-08-30 に「22 URL 中 20 が `_redirects` の転送元」だった問題は修正済み（f8f6cab14）。
転送元では CrUX が構造的に存在しないので直す必要はあったが、**それは field 0 の原因ではなかった**
（修正後も 0/44）。

**完了条件**: field が戻る、または `psi-config.json` の判定方式が更新されて CI ゲートが
「判定材料なし」で恒久的に赤い状態が解消している。
