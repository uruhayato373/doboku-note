...
2. **最小リスト捕獲** — LINE公式（ノーコード）＋一次→二次ブリッジ磁石「一次おつかれ→二次の始め方」。
   ...
   **磁石記事は 2026-08-26 に note 無料記事として公開済み**
   （https://note.com/dobokunote/n/na31c6abae8f6 ／
   [content/note/1級・2級土木/一次二次ブリッジ磁石-LINE/](../../content/note/1級・2級土木/一次二次ブリッジ磁石-LINE/README.md)）。
   残るのは「器」＝ユーザー作業（LINE公式アカウント開設・友だち追加URLの発行・各面への実配置）のみ。
   アカウント開設を要するためエージェントでは実行不可。
