...
**ツール側は 2026-08-25 に解消した**。止まっていた原因は「1ページ目しか走査しない」ではなく、
**下書き行に `a[href]` が無い**ことだった——公開記事はタイトルが `note.com/dobokunote/n/<key>` への
リンクになるが、下書きはまだ URL を持たないのでリンク要素そのものが存在しない。
`--note <key>` は下書きに一生当たらない。`note-delete-note.mjs` に下記を入れた:

- `--list-drafts`… 公開ステータス→下書きで絞り、**宣言件数に届くまで**読み切って列挙（read-only）
- `--draft '<タイトルの一部>' [--date '…']`… タイトル＋日付で同定。2 件以上一致したら削除しない
- 削除後の検証を**一覧の再走査**に変更（note API は下書きも削除済みも 404 で、使うと偽成功になる）

**実測（2026-08-25・39/39 読み切り）**: 下書きは **39 件**あり、カードが把握していた 8 件は氷山の一角だった。
うち本カードの 3 件は実在を確認:

| 行 | 日時 | タイトル |
|--:|---|---|
| 2 | 2026年8月22日 19:24 | コンクリート主任技士 小論文 模範答案｜試験・検査機関｜環境配慮 |
| 3 | 2026年8月22日 19:20 | …｜耐久性 |
| 4 | 2026年8月22日 19:18 | …｜環境配慮 |

**削除実行を試行・3回連続ABORTで停止（2026-08-25）**: `--draft '環境配慮' --date '2026年8月22日 19:24' --commit`
を3回実行したが、fail-closed のまま毎回異なる箇所で止まった（1回目「確認ダイアログの削除ボタン未検出」・
2回目/3回目「メニューに「削除」未検出」）。**下書き3件は無傷**（保存前に止まる安全弁が効いている）。
同じ Playwright セッションで直後に実行した `note-update-body.mjs` の一括公開・全文置換は正常に動作した
（同日 24 記事 ok）ため、note 側の全面的な自動化ブロックではなく、ダッシュボード（note.com/notes）の
操作メニュー UI に特有のセレクタドリフトの疑い。**未解決**——次回は目視確認できる環境（`headless: false`
の画面を人が見ながら）で再試行するか、UI 変更を直接確認してセレクタを更新する。

**残**: ①削除の実行（セレクタ要修正 or 目視同席での再試行）②一括公開を **25本程度で区切る**（または失敗時に
自動クールダウンして再開する）——ただし `note-publish.mjs` は 1 本ずつ公開する作りで**ループを持つ
バッチ実体が無い**ので、反映先は呼び出し側（skill / 手順書）になる。今回はバッチの成否判定を
RESULT 行で見ていたため `[13] FAIL` の 2 本を成功と数えた＝**判定はスクリプトの検証結果で行う**。
...
### [DN-0011] IG 論点パック 残62件の波状予約（1セッション30件）
...
1日2件（civil-1・civil-2 交互）で埋まった。status.json 30 件を commit 済み。残 62 件を次セッション以降で継続。
...
**死守コア（時間が足りなければこれだけ）**:
1. **会員ローンチ** — **2026-08-26 実体確認で完了済みと判明**（DN-0019カードは古い前提のまま残っていたため削除）。
   添削実測ゲートは2026-07-18に通過済み（サンプル2本とも30分以内）、添削つきプランは定員20名・¥4,980で
   確定（2026-08-06）、公開API（`circle_plans`）で実際に募集中（`is_freeze_admission`なし）を確認。
   無料集客16本も全てnoteStatus:publishedを実査済み。死守コア1は解消済み。
2. **最小リスト捕獲** — LINE公式（ノーコード）＋一次→二次ブリッジ磁石「一次おつかれ→二次の始め方」。
   **中身（磁石PDF・配信台本3通・友だち追加CTA）は 2026-08-25 に作成済み**
   （[content/note/1級・2級土木/一次二次ブリッジ磁石-LINE/](../../content/note/1級・2級土木/一次二次ブリッジ磁石-LINE/README.md)）。
   残るのは「器」＝ユーザー作業（LINE公式アカウント開設・友だち追加URLの発行・各面への実配置・
   磁石PDFのnote公開）のみ。
...
**残る②**: 土木一般・共通工学編（土工・建設機械・コンクリート工・基礎工・測量・解体工事の6章。
`category-curriculum.json`に同型の`introGuides`が揃っており同じ手法で組成可能）／2級土木版は未着手。
**残る①**: 建設部門は手つかず（11専門分野の技術事実を扱うため WebSearch によるファクトチェックが
必須。`pe-secondary-exam-factcheck`エージェント相当の裏取りをしてから着手する）。
**価格・単品/バンドルの別・公開タイミングはユーザー判断待ち**（2巻揃うまで無料の単発記事として保留）。
