...
DN-0126 で判定基準を揃えた結果、`FREE_PREVIEW_COLLAPSE` 71 件は消えたが **`BOUNDARY_SHIFT` が 270 件**残った（有料 706 本の 38%・2026-08-25 実検査 810/810・取得失敗 0）。これは基準変更で増えたのではなく、以前から 199 件あったところへ 71 件が移ったもので、**元から 270 本がこの状態だった**。

判定内容は「境界直前の無料プローブ（正規化した末尾 30 字）が live の無料本文に見つからない」＝ソースが無料と宣言している範囲と、note 側で実際に無料になっている範囲がズレている。HIGH なので CI を止めず、レポートにしか出ないため誰も読んでいない（レビュー W35 の「読み手が無い赤」と同型）。

**切り分けるべき 2 択**: ①本当に境界がズレている（ソースを再公開すれば直る）②プローブの取り方が実態に合わず誤検出している（30 字・正規化・note 側の本文整形の差）。**270 という数の大きさ自体が ② を疑わせる**ので、数本を手で開いて live とソースを目視照合するところから始める。

**実行順**: ①代表 5 本（パック内 / 総監 / 建設部門から）で live 無料本文とソース境界前を目視照合する ②誤検出ならプローブ生成を直す。実ズレなら `note-update-body --commit` の対象リストへ入れる ③どちらでもない混在なら、判定を分けられる signal を足す

**完了条件**: 270 件が「実ズレ N 件（要再公開）」と「誤検出 M 件（判定を修正）」に分解され、`BOUNDARY_SHIFT` の残数に理由がある状態になる。
