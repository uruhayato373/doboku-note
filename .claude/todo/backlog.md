...
### [DN-0159] ココナラブログ3本のライブ本文が2級式のまま（ソースは修正済み）
タグ: [コンテンツ品質] [種類:不具合] [起票:2026-08-31] [検証:check-coconala-blog]

品質・安全・工程の「書き方」3本が「設問2では検討した項目を書きます」と**2級式**で説明して
いる。1級は検討項目が(1)側なので、1区画約200字に収まらない。3本とも `exam: civil`（1級・2級
共通）で送客先は1級商品（coconala-1kyu-full-pdf 等）。

同じ誤りが C8 予想模試にもあり、購入者から「文字数がオーバーする」と指摘されて発覚した
（2026-08-30・DM 10051134）。模試は修正・R2 反映済み。**ブログもソースは修正済み
（commit 56c62e61）で、ライブだけが古い。**

- 差分は「1段落の挿入」＋「1〜3文の置換」。`git show 56c62e61 -- content/coconala/blog/` で全文
- **自動化は見送った**。`coconala-blog-publish.mjs` は二重公開ガードで公開済み記事に使えず、
  更新には本文を消して入れ直す処理が要る。同スクリプトのコメントに過去2回の破損
  （本文が500/1684字で切断・段落順の破壊）が実測記録されており、公開中3本を壊すリスクに
  見合わない。ブラウザで直接編集するのが安全
- 級ごとの割り振りの真実源は `content/site/civil-construction-{1,2}/secondary-r0{6,7}/article.mdx`。
  混入の再発は `npm run check-keiken-answer-split`（quality:audit ci:true）が止める
