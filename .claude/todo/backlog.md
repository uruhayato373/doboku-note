...
### [DN-0066] ココナラブログ 連載の残り（第5回 執筆＋総監単発1本）
タグ: [収益化] [種類:制作] [実行:sweep] [検証:check-coconala-blog] [起票:2026-08-17]

第1〜4回は公開済み（`check-coconala-blog` で 5/5 検査・公開4/下書き1 を実測）。**公開は 1 日 1 本まで**・連載順は変えない。

| # | slug | 状態 |
|---|---|---|
| 5 | `sokan-shutsudai-theme-bunseki` | **draft**（総監・単発。公開判断が要る） |
| 6 | （未作成） | **第5回「安全管理」が未執筆**。第4回の本文末尾がライブで「次回は、5管理のうち安全管理を題材に…」と予告済み＝書かないと約束を破る |

ハードゲート: 外部リンク/連絡先ゼロ・送客先が listed（`npm run check-coconala-blog`）。執筆は `coconala-blog-writer` → 採点は `coconala-blog-qa`。
