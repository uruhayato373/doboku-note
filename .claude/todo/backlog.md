...
- 対象: `civil-construction-1-public-servant-merit`（土木公務員に1級土木施工管理技士は必要？）
- 症状: `tags: [career]` を持つが `HIGH_INTENT_CAREER_SLUGS`（`src/config/affiliate-creatives.ts:158`）に未収録。MDX にも `<CareerAffiliate>` が無く、コンポーネントは `SeeAlso` と `Callout` のみ
- 位置づけ: 2026-08-17 に 13_土木公務員SEO戦略の第1期で新設した**最新かつ最大（184行）のキャリアページ**。収益面だけが空

やること: 収録するか（＝アフィリ面として扱う）、career タグを外すか（＝学習ページとして扱う）を決めて WARN を消す。**判断の前に DN-0120 の計測結果を見る**（クラスタ全体が無流入なら、ここだけ埋めても効かない）。

### [DN-0118] note の下書き残骸3件を消す＋連続投稿スロットルを手順化する
タグ: [収益化] [種類:不具合] [実行:ユーザー] [起票:2026-08-22]

2026-08-22 に実務立場別小論文集33本を一括公開した際、**31本目以降で note の「投稿する」が完了しなくなった**（画面は公開設定のまま・記事は404）。20分のクールダウン後は同じ原稿がそのまま公開できたので、連続投稿のスロットルとみられる。

消せていない下書き（いずれも 404＝非公開・実害なし）:

| key | 記事 |
|---|---|
| `n8cac2ac51117` | 08-試験検査機関-環境配慮（1回目） |
| `n4142b5a6d17c` | 08-試験検査機関-耐久性（1回目） |
| `n237dcb291b70` | 08-試験検査機関-環境配慮（2回目） |

`note-delete-note.mjs` は `note.com/notes` の**1ページ目しか走査しない**ため「カードが見つからない」で止まる。手で消すか、同スクリプトにページ送り／下書きタブ対応を足す。

あわせて、一括公開は**25本程度で区切る**（または失敗時に自動クールダウンして再開する）ようバッチ側へ反映する。今回はバッチの成否判定を RESULT 行で見ていたため、スクリプトが `[13] FAIL` を出していた2本を成功と数えた＝**判定はスクリプトの検証結果で行う**。
...
**DN-0120 の決定待ち**（(b) を選んだ場合はこのカードごと破棄する）。

実行内容の仕様は [13_土木公務員SEO戦略2026-08.md](../../docs/strategy/13_土木公務員SEO戦略2026-08.md) の「キャリア記事ラインナップ（genba-career 勝ち型の再現）」節が真実源。要点だけ:

- **改稿4本**: `guide-quit-or-stay` / `guide-career-consultation-before-quit` / `guide-hatchu-shien` / `public-engineer-qualification-map`。6ステップ骨格（想定読者の明示→冒頭に一人称の経歴→判断軸の番号付け→3番目にCTA→反対側も書く→実名アンカーで相互リンク）へ寄せる
- **新設4本**: 自治体を辞めた当事者 / 公務員技術職試験を採用側から / RCCM を発注者側から / 年収（案A or 案B）
- **見送り2本**: 自己啓発型（作業員と技術者の分かれ目・成長できない焦り）は検索意図が資格・転職に繋がらないため作らない

Red Line: 未利用サービスの体験談は**埋めずに空ける**（相手はビルドジョブ面談の体験を書いているが、実利用が無い限り真似しない）。一人称の具体性そのものは Red Line ではないので、運営者の真正な座（元自治体土木＝発注者・技術士両部門含む10資格の取得当事者）の範囲で踏み込む。相手のアーク（コンサル→公務員→出戻り）は流用しない。
...
### [DN-0120] キャリアクラスタ39本が無流入 — 転職アフィリを「増やす／畳む」の判断
タグ: [収益化] [種類:意思決定] [実行:対話] [起票:2026-08-24]

競合 genba-career.com の全71記事調査（2026-08-24・[07_競合調査.md](../../docs/strategy/07_競合調査.md#現場技術者のキャリアノートgenba-careercom)）で、**転職アフィリの前提が崩れていることが分かった。**

**実測（GA4 `ga4-page-2026-08-21T08-33-35.json` / 2026-07-24〜08-20・日本・スパム除外・上位100ページ）**

- 全体 5,110 セッション。`tags: [career]` の記事は **39 本中 1 本しか上位100に入っていない**（`guide-grade-comparison` 18 セッション）
- 上位100の最小が 8 セッションなので、**残り 38 本は 28 日で 8 セッション以下**という上限が押さえられる
- これは新発見ではない。`affiliate-creatives.ts:211` が既に「HIGH_INTENT_CAREER_SLUGS の36件は流入上位100に1つも入っていない／実際の流入は学習系（`guide-strategy` 641・`secondary-experience-writing-guide` 359）に集中」と記録している

**競合状況**: genba-career は同一広告主（ビルドジョブ・A8・EPC 942円）を同一キーワードに置き、14本のキャリア記事すべてを**一人称＋自分の年収実額**で書いている。論点の網羅では負けていないが、読ませる力で負けている。ただし**相手の流入量は未確認**（外形からは測れない）。

**決めること** — 次のどれを採るか。

- **(a) 既存を一人称で強化して再計測**: 13 の「キャリア記事ラインナップ」の改稿4本のみ実施し、28日計測して流入が動くか見る。新設は結果を見てから
- **(b) 学習系に寄せる**: キャリアクラスタは現状維持（畳む）。実流入のある学習ページのアフィリ面の最適化に集中する。GA4 実測はこちらを支持している
- **(c) 企画リストを全部やる**: 新設4本＋改稿4本。相手と同じ土俵で殴り合う

**注意**: (c) を無検証で選ぶと、無流入のクラスタに4本足すだけになりうる。13 の「やらないこと」＝「類似記事の量産」に触れないかも合わせて判断する。

付随して決めるもの（13 に併記済み）: 年収記事を **案A（自治体の給料表＝公開情報で書く・推奨）** と **案B（運営者の実額を開示）** のどちらで作るか。
