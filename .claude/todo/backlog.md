...
**再開の前提**: ブランチ `codex/character-framing` の最新を取得する（復元・検査実装の基点は `fc641401`）。採用画像の復元と代表動画の生成は [動画生成手順](../../content/sns/youtube/VIDEO-RENDER.md) に従う。画像はGitに含まれないため、別端末ではDriveから復元する。音声生成にはVOICEVOXとffmpeg/ffprobeを用意する。生成・検証・Drive保存の進捗とYouTube側の再開条件は更新状態を参照する。再開時は新しい説明レイアウトと「要＝かなめ」の読み修正を含むブランチを使い、生成済み実体を確認して残対象から進める。画像採用と音声付き動画の確認・外部投稿の更新を分ける。

**別PCでの再開順**:

1. ブランチを同期し、Drive台帳から確認済みPNGを復元して `npm run check-youtube-cover-handoff -- --local` を通す。代表パックの通常動画とShortsを音声付きで生成して確認する。制作端末のフォントへ依存せず `approvedImage` のhashが一致する画像を使う。
...
6. YouTubeのサムネ更新は日次上限の再開条件を満たした後、未反映の公開済み動画を優先する。Instagramの本文／Reelsカバー／本体、Xの編集期限を項目別に確認する。動画本体の再アップロードや投稿の削除・再投稿が必要なものは、URL・反応履歴への影響を示してユーザーの依頼範囲内で扱う。保存後にID・予約日時・公開状態・実表示を再照合し、素材生成だけで外部反映済みとしない。
...
4. 確認がそろった旧版は日次CIの削除前実査→旧IDだけの削除→APIで不存在確認へ進める。不一致・認証切れ・応答不明の対象を解消し、確認待ちで恒久停止させない。削除の都度、承認や手書きの直前実査日時を求めない。
5. `npm run check-youtube-delivery -- --require-deleted` のexit 0を旧版撤去の完了条件とする。exit 1は残件あり、exit 2は検査不成立。R2資格情報がない端末ではprivate取得済みの `--state` と `--plan` を渡す。動画の生成チェックだけで撤去完了にしない。
...
### [DN-0187] Git LFSの未検証オブジェクトを保全・復元確認し、安全にローカル容量を回収する
タグ: [インフラ・計測] [種類:改善] [起票:2026-09-10]

**起点**: 2026-09-10の `.git/lfs` 実体は約2.20GiB。削除候補199件（約2.4GB）のリモート照合が成立せず、削除は0件で停止した。リモート不存在と通信・認証失敗を区別して再開する。通常Git履歴の切り詰めを扱うDN-0155とは別件。

1. 現在のLFSオブジェクトと参照を再計測し、接続可能な環境でリモート保管を再確認する。各オブジェクトのSHA-256をDrive・R2台帳と実体へ突合し、唯一のローカル原本・復元に必要なもの・安全な重複を分ける。
2. 未保全の必要資産は利用者別の保管方針に従って退避する（人が使う原本・制作物は既存Drive vault、CIが使うものはprivate R2）。元のパス・hash・復元方法を記録し、アップロード後の読み戻しと空の復元先への取り戻しを検証する。DN-0186の接続設定を必要に応じて先に解消する。
3. 保管・復元が実証できた対象だけを選択的に削除し、削除件数・回収容量・保持理由を記録する。未検証の強制prune、`.git/lfs` の一括削除、履歴書き換え・force-pushはこの作業に含めない。

**完了条件**: 再監査した全対象に保管先と復元証跡、またはローカル保持理由が揃い、安全に削除できる分を回収して実測値を残す。現行開発に必要な資産の取得・復元が成立し、唯一の原本を失っていないこと。

**参照**: [ローカル資源運用](../knowledge/reference/local-resource-policy.md)、[アセット保管方針](../knowledge/reference/asset-storage-policy.md)。当日の端末内証跡は `.local/resource-audit/lfs-latest.json`（Git追跡外）。
