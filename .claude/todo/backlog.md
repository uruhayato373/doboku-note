...
### [DN-0223] 全教材の論点と記事・図解・SNSを突合し未展開の学習課題を解消する
タグ: [コンテンツ品質] [種類:改善] [起票:2026-09-13] [進行中]

**対象**: reference-sources の commercial-book / operator-owned 全件。教材の章・論点から既存記事・図解・SNSへの対応と必要性を確認する。台帳の語数や図の存在だけで充足と判定しない。

**次**: 原本/OCRと既存本文を突合し、学習課題単位に充足・記事補強・図解追加・SNS展開・対象外理由・原典待ちを記録する。確認済みの制作は順次実装・検証し、残りを具体的な単位へ分割する。

**完了条件**: 対象教材の取りこぼし0、未確認を充足へ変換しない対応表、実装成果物へのリンク、残務の実行先が揃う。記事・図・投稿の数ではなく説明・演習・答案への役割を確認する。制作と公開・計測は別判定。既存 DN-0206/0207/0208/0209/0220 の範囲は重複起票しない。
