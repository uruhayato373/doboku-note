...
### [DN-0322] 4週でクリック0の記事サイドバー広告を外すか、A8ピクセルの置き場を決めて判断する
タグ: [収益化] [種類:意思決定] [起票:2026-09-26]

**起点**: `npm run report-career-funnel`（GA4 2026-08-20〜09-16）で、記事サイドバーの転職広告は表示 12,673・クリック 0、記事末も 3,413・0。クリックは本文中 7・本文中間 3・カテゴリ サイドバー 1 だけ。ただしサイドバーは A8 計測ピクセルの唯一の発火源（`affiliate-operations.md`「1 ページ 1 ピクセル」・`ArticleSidebar.tsx`）で、外すと成果が計測されない。サイドバーは PC（≥993px）だけの表示なので、表示数がモバイルの非表示分まで数えているかは未確認。

**やること**: (1) サイドバーの表示イベントが非表示のモバイルでも発火していないか確かめる (2) 外すなら、ピクセルを本文中の広告（クリックのある面）へ移し 1 ページ 1 発を保つ (3) 記事末は内部導線へ替えるか判断する。判断と結果は EXP-008 の actions に記録する（コード変更は feature ブランチ＋PR）。

**完了条件**: サイドバー広告の存廃が決まり、ピクセルの発火源が 1 ページ 1 箇所で残っていることを build 後 HTML で確認したら、このカードを削除する。
