...
### [DN-0211] 既存CIの管理画面依存誤検出と仕様書E2Eの旧UI期待値を修正する
タグ: [インフラ・計測] [種類:不具合] [起票:2026-09-13] [進行中]

**実体**: develop acfd2952 のCIでもhast-util-sanitizeがunused判定。PR #496で仕様書の古いselect・ラベル・重複目次・summary選択のE2E 11件が失敗。

**次**: [実装計画](../plans/DN-0211-ci-contracts.md) に従い、利用実体と現在のナビを確認して検査を一致させる。

**完了条件**: baselineを増やさずknipを通し、仕様書の機関選択・相互リンク・章の原典範囲・節アンカーをPC/モバイル両方で検証する。
