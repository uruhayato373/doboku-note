...
### [DN-0212] SEO Rank Watchを資格受験者優先の選定と継続記録へ改める
タグ: [SNS・マーケ] [種類:改善] [起票:2026-09-13] [進行中]

**目的**: 1級土木・技術士総監・建設部門の受験ニーズを順位より先に評価する。方針、監視語の登録根拠、選定・待機理由、改善結果を履歴で結ぶ。

**完了条件**: [実装計画](../plans/DN-0212-qualification-seo.md)に従い選定・発見・記録・管理画面・既存定期実行を同期し、専用テスト・型検査・文書照合・3資格の計測を通す。一般用語の近順位が受験課題を押しのけず、改善しない回にも記録が残ること。
