...
### [DN-0222] 図版検査のstaged対象漏れと旧画像パス参照を直す
タグ: [インフラ・計測] [種類:不具合] [起票:2026-09-13] [進行中]

**根拠**: 12枚のサイトSVGをstageしても `check-figure-canvas --staged` が0枚検査で成功した。staged抽出の正規表現が `content/site` に一致していない。MDX検査は旧 `public/posts` を見て、現存する `content/site` 画像にも欠落警告を出した。

**次**: stagedパスを正しく抽出し、画像実体の保存先を現行構成へ合わせる。隔離したGitリポジトリで正常1枚・不正寸法・不正ファイル名を検査し、画像の実体あり/なし/HTML偽装の回帰テストを追加する。

**完了条件**: 実際のstaged図12枚を検査し、異常ケースは失敗する。MDX画像警告が実体と一致する。`node --test tests/check-figure-canvas.test.mjs tests/check-mdx-images.test.mjs` と `node scripts/pre-commit-mdx.mjs` を通す。


### [DN-0221] 土工の計算表とコンクリート二次過去問の原典不整合を監査・修復する
タグ: [コンテンツ品質] [種類:不具合] [起票:2026-09-13] [進行中]

**根拠**: 図解の照合中、`textbook-earthwork-planning/article.mdx` の表1.32で「盛土量86・C=0.9→地山換算73」とする逆換算、測点11+10の累加値1248−106と1121の不一致を確認。`secondary-concrete-past-problems/article.mdx` のH30問題4は原PDFの4小問・5空欄に対し2小問だけで、選択問題の区分も一致しない。両記事は図解対象の修正箇所以外にも原典照合が必要。

**次**: 当該2記事を年度・問題番号・空欄・数値計算の単位で原資料へ照合する。土工表の原本を確定できない場合は数値を推測せず、条件を明示した独自の計算例へ置き換える。H30原問題は `https://dobokujira.com/wp-content/uploads/2021/05/h30_1doboku_jitti_doboku.pdf` の問題4を起点に同記事の残りを確認する。

**完了条件**: 対象範囲と未照合範囲を明示し、問題文と解答記号の対応、全計算の検算、出典、MDX検査を通す。同種欠陥の再生成を止める検証を追加する。公開・既存投稿の訂正はDN-0220へ具体的な対象を渡す。原本が読めない部分を正しいと断定しない。
...
### [DN-0219] 既存図3テーマをSNS・動画で再利用できる制作物にする
タグ: [SNS・マーケ] [種類:制作] [起票:2026-09-13] [進行中]

**根拠**: 骨材含水・水準測量・ネットワーク工程には利用可能な既存SVGがある。一方9月土木X計画90投稿はimage:null、動画155パックの明示figureシーンは3。既存77図追加候補は全対象ページに画像があるため一括新作しない。

**次**: 既存図を意味確認し、骨材含水4状態、水準の後視/前視、ネットワークの余裕時間を各1テーマの図解投稿下書きにする。各テーマの元記事・元図・次の学習先を保持し、適合する未公開動画のfigureシーンを整備する。資格固定修正後に制作する。

**完了条件**: 3テーマの再生成可能な入力と画像の目視確認、出典/リンク/資格の一致、投稿・動画の既存ゲートを通す。公開済み動画の再投稿や投稿履歴の書換えをしない。

**手順・停止条件**: [実装計画](../plans/DN-0219-figure-reuse.md)。


### [DN-0218] 総監のALARP・根本原因分析・5管理の学習関係を図解する
タグ: [コンテンツ品質] [種類:制作] [起票:2026-09-13] [進行中]

**根拠**: ALARP・root-cause-analysis・総監キーワード2026は本文画像がない。図は増やす枚数より意思決定や因果の読み取りを優先する。

**次**: ALARPの3領域と合理的に実行可能な低減、RCAの事実→直接原因→仕組み→対策、5管理を相互に行き来する学習関係を3図で整理する。既存ネットワーク図の重複制作はしない。

**完了条件**: ALARPを単純な費用便益一致や無条件の追加対策不要と説明しない。RCAは事実と仮説を分け、5管理の正式名を保持。一次根拠・SVG/MDX・375px確認・refresh-indexesを通す。

**手順・停止条件**: [実装計画](../plans/DN-0218-cem-diagrams.md)。


### [DN-0217] 技術士建設部門の設問分解・答案活用・劣化対策を図解する
タグ: [コンテンツ品質] [種類:制作] [起票:2026-09-13] [進行中]

**根拠**: 建設部門の公開135記事のうち本文画像ありは5記事、SVGは0。setsumon-bunkai・hissu-kamoku-kaitourei・steel-concrete-exam-themesは本文画像がない。

**次**: 設問の条件→求める答案、模範解答→骨子→自分の答案、劣化事象→原因→調査→対策という異なる3つの関係を図示する。設問分解ではR7必須Iの設問(4)も落とさない。年度別分析のR8追加はDN-0206に委ねる。

**完了条件**: 3図が別々の学習課題を解決し、専門事実の一次資料照合・SVG/MDX/出典検査・375px視認性・refresh-indexesを満たす。予想を実際の試験と混同しない。

**手順・停止条件**: [実装計画](../plans/DN-0217-pe-diagrams.md)。


### [DN-0216] 1級土木の経験記述と土量変化を図解する
タグ: [コンテンツ品質] [種類:制作] [起票:2026-09-13] [進行中]

**根拠**: 経験記述ガイドの答案欄対応は文章中心、発注者の関係図は390pxで文字が小さい。土工計画記事は本文約1.5万字に本文画像がない。

**次**: 経験記述の(1)現場状況・課題・検討項目→(2)対応処置・評価を図示する。発注者と受注者の一般的な役割を独自図へ置換し、同図を使う未公開動画の参照を更新する。土量は地山・ほぐし・締固めの状態遷移とL/Cの方向を1図にする。

**完了条件**: 3図のArticleImage結線、1級/2級の答案欄混同0、図の意味・375px視認性・SVG/MDX/出典検査・refresh-indexes。顧客の工事事実を創作しない。

**手順・停止条件**: [実装計画](../plans/DN-0216-civil-diagrams.md)。


### [DN-0215] 資格別の図解SNS書き出し・制作手順を統一する
タグ: [SNS・マーケ] [種類:不具合] [起票:2026-09-13] [進行中]

**根拠**: render-figure-sns はカテゴリ指定を受けるがYouTube用画像に「技術士 総合技術監理」を固定表示する。ig-figure-packにも総監固定の文言と廃止パスが残る。

**次**: 既存レンダラーに資格名の自動解決・未知資格の拒否・再現可能な出力先を実装し、既存図を使う制作手順を3重点資格に対応させる。既存のキャラクター意匠改修はDN-0184の守備範囲を維持する。

**完了条件**: 3重点資格の代表図について誤った資格名が出ず、未指定互換と未知カテゴリ拒否をテストする。スキル・参照規約・生成互換・doc-syncを通す。

**手順・停止条件**: [実装計画](../plans/DN-0215-figure-pipeline.md)。


### [DN-0214] 側圧の誤説明とR1・H30コンクリート過去問の空欄対応を修復する
タグ: [コンテンツ品質] [種類:不具合] [起票:2026-09-13] [進行中]

**根拠**: 調査で secondary-concrete-past-problems に「スランプが小さく打上がり速度が遅いほど側圧は大きい」という逆転と、R1/H30の問題文・解答記号の不整合を確認した。Xの2026-09-03投稿（ID 2095356106256105570）にも同じ説明がある。

**次**: 原問題の年度・問題番号・空欄を照合し、記事の問題文と独自解説を修正する。誤説明の再利用箇所を検索し、公開済み投稿の記録を保存したまま訂正文案と原投稿IDを用意する。

**完了条件**: 当該2問の原問題との対応確認、誤説明の再利用防止、MDX検査・refresh-indexes。外部投稿と本番反映は公開・実測カードで扱う。

**手順・停止条件**: [実装計画](../plans/DN-0214-pressure-correction.md)。
