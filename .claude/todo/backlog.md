...
### [DN-0210] SEO Rank Watch を既存の計測・実験・管理画面へ統合する
タグ: [インフラ・計測] [種類:改善] [Codex候補] [起票:2026-09-13] [進行中]

**実体**: GSC・実験台帳・週次改善は存在するが、クエリ単位の選定と本番反映起点の観察・再判定が結線されていない。

**次**: [実装計画](../plans/DN-0210-seo-rank-watch.md)に従い、期間計算・監視設定・7日比較・ページ排他・管理画面・CI/スキルを実装する。

**完了条件**: 境界日付・欠測・状態遷移・履歴不変・観察中の変更拒否をテストし、管理画面と初回GSC計測を確認する。本番公開前を observing としない。
