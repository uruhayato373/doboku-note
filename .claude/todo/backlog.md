...
### [DN-0281] ココナラに4テーマ分の経験記述添削を出すか、価格とあわせて決める
タグ: [収益化] [種類:意思決定] [起票:2026-09-23] [期日:2026-09-30]

**起点**: ちゃんさとは質問4つ分（自社の4テーマに相当）の添削を ¥24,000 で155件、作成代行を ¥32,000 で168件売っている（2026-09-23 のプロフィール表示・[07b_販売動線分析_ちゃんさと_2026-09.md](../../docs/marketing/07b_販売動線分析_ちゃんさと_2026-09.md)）。自社には4テーマの構成（`coconala-sakusei-4theme`）はあるが、4テーマの添削が無い。

**決めること**: 出すか、価格（2テーマ ¥6,000 の2倍前後か）、受付の上限（運営者は在職中）。出すなら2級二次（10/25）の前に出品する（新規出品は1日4件まで）。

**完了条件**: 判断と理由を `ココナラ展開キット.md` §2 の決定ログに記録する。出す場合は出品して `check-coconala-live` が通るまで。
