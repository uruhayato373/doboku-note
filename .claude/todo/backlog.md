...
### [DN-0135] ユーザー手作業でしか閉じない残務（外部依存 9 件を統合）
...
| 4 | ココナラ S3 2テーマ版のライブ反映 | SoT（`src/lib/coconala-services.ts` の `coconala-sakusei`）の是正は 2026-08-18 完了。4テーマ版で審査を通った表現へ揃える | `/coconala-publish` でサービス 4317796 の説明文を更新（ブラウザ実機）。title は変更しない |
...
**完了条件**: 各行の実体が解消したら行ごと消す。9 行すべて消えたらカードを削除する。**部分的に片付いたら行を消して残数を書き直す**（「残 N 件」を本文に持たない＝表の行数が真実源）。
