...
**起点**: 計画 T1。civil-1-keiken-complete-pack（¥9,800）は直近 2 件実売、note-funnel の `topCtaOverrides` は 1級 `m150c9db08902`（完成答案集 ¥2,480）を指している。

**やること**: `.claude/config/note-funnel.json` の `1級土木/` override を `m8290970a7f05`（完全攻略パック）へ、`2級土木/` を `m8554e87ca6ec`（想定工事バンク）へ切替 → `wire-note-funnel-cta --exam civil --apply` → `note-update-body --list --commit`（既存の civil ドリフト約 250 本は本タスクの対象外。切替対象 4〜6 本だけ）。10/5 と 10/26 に元へ戻す。試験後に無料「R8 二次 自己採点の目安と合格発表までにやること」を各級 1 本（会員ラボ・次年度導線）。
