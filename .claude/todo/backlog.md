...
### [DN-0357] PR #655 を develop へマージして本番へ反映し、本番の SSR を確かめる
タグ: [インフラ・計測] [領域:管理] [時期:2026-09..2026-10] [種類:改善] [起票:2026-09-26]

**起点**: 2026-09-26 のセッションの変更（計画の層の配線・展開の判断の正本と管理画面・資格台帳の active 追加・保有資格 `src/config/author.ts`・バックログの [時期:] 必須化と翌月繰り越し・週次/月次レビューの配線）は feature/product-lineup-view の PR #655 に積んだまま。サイトに出るのは保有資格（/about・記事末尾のプロフィール・Article schema）だけで、ほかは運用側の変更。

**やること**: ユーザーが PR #655 を develop へマージし、`/deploy` で本番へ反映する。反映後に `npm run check-production-ssr` を実行する。

**完了条件**: `check-production-ssr` が exit 0 で、/about に技術士（上下水道部門）と測量士が出ていたら、このカードを削除する。

### [DN-0358] note の売上記録を 9/12 以降の分まで追いつかせる
タグ: [収益化] [領域:管理] [時期:2026-09..2026-10] [種類:改善] [起票:2026-09-26]

**起点**: `.claude/state/sales/sales-log.json` の最終記録が 2026-09-12 で止まっている。資格別の売上（`npm run qualification-market`・展開の判断）と月次レビューの判断が、9 月後半の分だけ欠けたまま読まれる（欠測を 0 と読まない）。

**やること**: note の販売履歴を `/record-sales`（`npm run note-sales-fetch -- --month 2026-09 --commit` が使えればそれ）で 9/13 以降の分まで取り込む。新しい productId が資格へ分類できなければ `npm run check-qualification-market` が止めるので、product-lineup.json の salesRules に足す。

**完了条件**: 売上記録の最終日が取り込み時点の直近になり、`npm run check-qualification-market` が通ったら、このカードを削除する。
...
**起点**: 2026-09-26 の市場スキャンで、ココナラの資格別の検索語 26 語は Playwright のメモリガード（空き 1,175MB < 閾値 1,200MB）で取得できなかった。管理画面 戦略 ＞ 展開の判断のココナラ列は、汎用の検索結果から数えた下限（*）のまま。
...
タグ: [収益化] [領域:商品] [時期:2026-10..2026-12] [種類:改善] [検証:check-coconala-live] [起票:2026-09-25] [期日:2026-10-03]
...
タグ: [収益化] [領域:商品] [時期:2026-09..2026-10] [種類:改善] [検証:check-coconala-orders] [起票:2026-09-25] [期日:2026-10-03]
...
タグ: [SNS・マーケ] [領域:SNS] [時期:2026-10..2026-12] [種類:改善] [検証:check-video-publication] [起票:2026-09-23]
...
タグ: [SNS・マーケ] [領域:SNS] [時期:2026-10..2026-12] [種類:改善] [検証:check-x-campaign-plan] [起票:2026-09-23] [期日:2026-10-10]
...
タグ: [収益化] [領域:商品] [時期:2026-10..2026-11] [種類:不具合] [起票:2026-09-23]
...
タグ: [コンテンツ品質] [領域:サイト] [時期:2026-10..2026-12] [種類:制作] [起票:2026-09-16] [期日:2026-10-10]
...
タグ: [収益化] [領域:商品] [時期:2026-09..2026-10] [種類:定期] [起票:2026-09-17] [期日:2026-09-29]
...
タグ: [SNS・マーケ] [領域:SNS] [時期:2026-09..2026-10] [種類:改善] [Codex候補] [検証:check-video-content] [起票:2026-09-08]
...
### [DN-0359] ココナラ・X のプロフィールと公開中の自己紹介に、技術士（上下水道部門）と測量士を加える
タグ: [収益化] [領域:商品] [時期:2026-10..2026-11] [種類:改善] [起票:2026-09-26]

**起点**: 2026-09-26 に保有資格の正本（`src/config/author.ts`）と note プロフィールの管理ファイル（`content/note/プロフィール.md`）へ技術士（上下水道部門・2024）と測量士（2020）を加えた。ココナラと X のプロフィールはリポジトリに管理ファイルが無く、公開中の文面は未更新。総監の note 記事（`content/note/技術士総監/総監R08合格ロードマップ/article.md`）は冒頭の資格紹介を「6 資格」として参照している。上下水道・舗装・測量士の商品を出す前に、信頼性の根拠を各チャネルに揃える。

**やること**: ココナラのプロフィールと X のビオに加える（公開中の文面の変更なので、文面を見せてから運営者が反映する）。総監の note 記事の自己紹介を直すかは、有料境界を壊さない反映方法（`note-update-body.mjs` の注意点）と合わせて決める。

**完了条件**: ココナラ・X に反映し、総監の記事を直すか直さないかを決めたら、このカードを削除する。


### [DN-0322] 合格発表後に候補資格の昇格・見送りを決める（建築の有料化・舗装・技術士の他部門ほか）
タグ: [収益化] [領域:戦略] [時期:2027-01..2027-03] [種類:意思決定] [起票:2026-09-26]

**起点**: 2026-09-26 に施工管理系は判断済み（土木二次を先に立て直す・建築は無料記事で需要を測る・電気/管/造園/建設機械は見送り。06_多資格展開戦略.md の判断の記録）。残る候補は、建築（有料で 1 商品の実験をするか）・技術士の他部門（上下水道は 2026-09-26 に展開を決定済み＝DN-0348〜0351。舗装・測量士も同日に決定済み＝DN-0352〜0355。他の部門は部門別の模範解答では広げない）・公務員・測量ほか。材料は管理画面 戦略 ＞ 展開の判断（`npm run qualification-market`）で、市場スキャンが 90 日を超えていたら先に `npm run scan-qualification-market -- --coconala` で取り直す。建築は DN-0342 の無料記事の検索表示とクリックも見る。
...
### [DN-0352] 舗装施工管理技術者（1級・2級）の商品設計を決める
タグ: [収益化] [領域:商品] [時期:2026-11..2026-12] [種類:意思決定] [起票:2026-09-26]

**起点**: 2026-09-26 にユーザーが舗装の展開を決め、資格台帳を active にした（ユーザーは 1級を取得済み）。応用試験（記述式）に「施工現場で経験したこと」を基に課題・対策・結果を書く設問があり、土木の経験記述と同じ売れる型（exam-formats.json）。試験は 6 月で、買われる時期は 5〜6 月。公式は直近 5 年度の問題だけを掲載し解答は出していない。YouTube・note・ココナラの強い売り手は少ない（`npm run qualification-market`）。

**やること**: 直近 5 年度の応用試験の設問を並べ、1級土木の経験記述の資産（想定工事バンク・工種別完成答案）のうち舗装工事で流用できるものを洗い出して、商品の順番と価格を決める。一般試験（択一）は note では売れにくいので、サイトの無料記事か Kindle で扱うかを決める。

**完了条件**: 商品の順番・価格を決めて DN-0353 の範囲を確定させたら、このカードを削除する。

### [DN-0353] 舗装施工管理技術者 応用試験の経験記述 模範答案集を作る（1級・2級）
タグ: [収益化] [領域:商品] [時期:2027-01..2027-04] [種類:制作] [起票:2026-09-26] [期日:2027-04-15]

**起点**: 舗装の展開（DN-0352 で範囲を確定）。買われる時期（5〜6 月）の前に公開する。

**やること**: DN-0352 で決めた商品を、civil-keiken-essay-writer／qa の型（重複・捏造・形式の検査）で書く。舗装に広げるならエージェント定義と registry を同じ commit で直す。
...
**やること**: 午後記述の分野（基準点・地形写真・地図編集・応用測量）ごとに、解き方の型と過去問の解説をまとめた商品の構成と価格を決めて作る。測量士補は択一なので、note の有料より無料記事・Kindle・過去問アプリで扱うかを含めて決め、測量士の商品へつなぐ入口にする。
...
**やること**: (1) 先に建築の出題形式（令和6年度改正後の経験記述）を建設業振興基金の公式原文で照合し、exam-formats.json の building-construction-1/2 を checkedBy: self にする（記事に書く事実の正本）。(2) 経験記述の書き方と過去問の傾向を中心に、検索意図の違う無料記事を 3〜5 本公開する。資格台帳では candidate のままとし、サイトに資格の入口を作るかは記事の反応を見て決める。

**完了条件**: 記事を公開して refresh-indexes まで済ませ、検索表示とクリックを見る URL を DN-0322 に書いたら、このカードを削除する。
...
タグ: [SNS・マーケ] [領域:SNS] [時期:2027-04..2027-07] [種類:制作] [検証:check-video-content] [起票:2026-09-23]
...
タグ: [インフラ・計測] [領域:管理] [時期:2026-10..2026-12] [種類:改善] [起票:2026-09-23]
...
タグ: [コンテンツ品質] [領域:商品] [時期:2026-09..2026-10] [種類:定期] [起票:2026-09-19] [期日:2026-10-05]
...
タグ: [コンテンツ品質] [領域:サイト] [時期:2026-11..2026-12] [種類:不具合] [検証:test:e2e:a11y] [起票:2026-09-17]
...
タグ: [コンテンツ品質] [領域:サイト] [時期:2026-11..2027-01] [種類:改善] [起票:2026-09-17]
...
タグ: [インフラ・計測] [領域:サイト] [時期:2026-11..2026-12] [種類:改善] [Codex候補] [起票:2026-09-14]
...
タグ: [コンテンツ品質] [領域:サイト] [時期:2026-10..2026-12] [種類:制作] [Codex候補] [起票:2026-09-13]
...
タグ: [収益化] [領域:アフィリエイト] [時期:2026-10..2026-12] [種類:改善] [起票:2026-09-22] [期日:2026-10-20] [検証:report-career-funnel]
...
タグ: [SNS・マーケ] [領域:サイト] [時期:2026-09..2026-10] [種類:改善] [期日:2026-09-28]
...
タグ: [コンテンツ品質] [領域:商品] [時期:2026-10] [種類:不具合] [起票:2026-09-25]
...
タグ: [収益化] [領域:商品] [時期:2026-10] [種類:不具合] [起票:2026-09-25]
...
タグ: [インフラ・計測] [領域:計画] [種類:改善] [起票:2026-09-26]

**起点**: 2026-09-26 の展開の議論で「年間の売上を平準化する」視点を判断の軸に入れた。資格別・月別の売上は `npm run qualification-market -- --json` の salesByMonth で出るが、管理画面では試験日と買われる時期（年間ロードマップ）と並んでいない。

**やること**: 年間ロードマップ（`/plan/roadmap`）の月の行に、前年同月の資格別売上を `scripts/lib/qualification-market.mjs` の salesByQualification から出す（値を写さない）。

**完了条件**: 画面で月ごとの売上と買われる時期が並び、テストを足したら、このカードを削除する。

### [DN-0323] 総監・建設部門の直前期（試験前）の価格と品揃えを確定する
...
### [DN-0324] 次の年間ロードマップ（2027-10〜）を作る
...
タグ: [収益化] [領域:商品] [時期:2026-09..2026-10] [種類:意思決定] [起票:2026-09-25] [期日:2026-10-09]
...
タグ: [SNS・マーケ] [領域:SNS] [時期:2026-09..2026-10] [種類:不具合] [検証:check-x-queue-health] [起票:2026-09-19] [期日:2026-09-28]
