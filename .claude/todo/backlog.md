...
### [DN-0162] 建設部門の過去問↔科目キーワード導線を設計し、設問単位へ拡張する
タグ: [コンテンツ品質] [種類:改善] [起票:2026-09-03]

`PastExamBacklinks` の設問単位逆引きは総監（`exam-keyword-map.json`＝人手キュレーション）と1級・2級土木（本文照合の自動索引）にしかない。pe-construction は past-exam 84本・keyword 35本。2026-09-04に道路・河川・都市計画の3組を生成HTMLで確認したところ、対応する過去問↔科目キーワードの直リンクは6方向すべて無かった。まず科目単位の確実な双方向導線を定義し、35本の広い科目記事を各設問へ機械的に貼るのではなく、設問の中心概念へ対応するslug体系を決めてからmapとauditorを拡張する。完了条件は、代表3科目の生成HTMLで記事単位の双方向リンクが確認でき、設問単位mapの正源・対象範囲・品質監査が一意になっていること。コンクリート3資格はprimary-X ↔ textbook-Xが1対1で対応するため対象外。
...
### [DN-0153] kindle-dist EPUBの再生成内容一致を検証してgit追跡解除を判断する
タグ: [インフラ・計測] [種類:改善] [起票:2026-08-29]

`scripts/kindle-dist/`はEPUB 38冊＋表紙38枚をR2 privateへバックアップ済みだが、Git追跡を意図的に維持している。catalogは現在live 45冊。残る条件は、全冊を既存distへ上書きせず別ディレクトリへ再ビルドし、ZIPタイムスタンプを除いた内容一致を比較する安全なラッパーの実装だけ。`scripts/check-kindle-format.mjs`の既定走査先は`kindle-published`であり、dist追跡解除のCIブロッカーではない。内容一致を実証できるまでは`asset-storage-policy.md`どおりGitを正本、R2をバックアップとして維持する。
