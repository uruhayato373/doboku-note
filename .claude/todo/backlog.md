...
**測り直しの結果（2026-08-25 実施・実検査 810/810・取得失敗 0）**:

| | CRITICAL | HIGH (BOUNDARY_SHIFT) |
|---|--:|--:|
| 固定 600（CI 2026-08-23 の実行ログ） | **71** | 199 |
| 記事別下限＋`textLen`（本修正後） | **0** | 270 |

**本当に下限割れしている記事は 0 本**。71 件はすべて記事別下限を上回っており、固定 600 との差だけで
CRITICAL になっていた（BOUNDARY_SHIFT へ移っただけで総数 270 は不変）。有料 706 本中 434 本が
記事別下限（<600）で判定される＝固定 600 は全体の 6 割に対して過大な基準だった。
...
確認した人間の担当・CLAUDE.md §8）。BOUNDARY_SHIFT は DN-0132 へ切り出し済み（270 → 194 へ減少）。
...
やること: 収録するか（＝アフィリ面として扱う）、career タグを外すか（＝学習ページとして扱う）を決めて WARN を消す。

**収益の期待値はほぼ無い**（2026-08-24 実測: このページを含む career 39 本は 28 日で上位100 に 1 本しか入らない）。これは「WARN を出しっぱなしにしない」衛生の話であって、増収施策ではない。DN-0120 の (3) 撤退を選ぶなら`HIGH_INTENT_CAREER_SLUGS` 側ではなく career タグを外す方向で揃える。
...
### [DN-0132] 有料 194 本で有料境界がソースの想定と違う — 再公開で消えることが実証済み
タグ: [コンテンツ品質] [種類:不具合] [実行:sweep] [検証:check-note-structure] [起票:2026-08-25]

DN-0126 で判定基準を揃えた結果、`FREE_PREVIEW_COLLAPSE` 71 件は消えたが `BOUNDARY_SHIFT` が残った。判定内容は「境界直前の無料プローブ（正規化した末尾 30 字）が live の無料本文に見つからない」＝ソースが無料と宣言している範囲と、note 側で実際に無料になっている範囲がズレている。

**起票時の 2 択は決着した（2026-08-25 実証）**。起票時は「270 という数の大きさ自体が②誤検出を疑わせる」と書いたが、**①実ズレだった**。

DN-0003 のバッチで本文 76 本をライブ反映したところ:

| | 件数 |
|---|--:|
| 反映前 BOUNDARY_SHIFT | 270 |
| 反映後 BOUNDARY_SHIFT | **194** |
| 差 | 76 |
| **反映した 76 本のうち、まだ BOUNDARY_SHIFT に残っているもの** | **0** |

反映した本数と減少数が一致し、かつ**反映済みの残留がゼロ**。つまり `note-update-body --commit` を通せば確実に消える。目視照合による切り分けは不要になった。

**やること**: 残 194 本を DN-0003 のバッチ対象に含めて消化する。専用の作業は要らない（DN-0003 の消化に従属する）。プローブ生成の修正も不要。

**完了条件**: `check-note-structure` の `BOUNDARY_SHIFT` が 0。ただし**実検査数を必ず読む** — 2026-08-25 の実行は 809/810（取得失敗 1）で、810 に届いていない。
