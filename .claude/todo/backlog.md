...
### [DN-0148] コンクリート技士 四肢択一パイロットの着手判断
タグ: [収益化] [種類:意思決定] [起票:2026-08-27]

旧 DN-0095（想定工事拡張＋コンクリート資格の商品展開）は主要3本柱（1級150工事・2級60工事・
コンクリート主任技士32小論文）が完了・公開済みのため撤収した（詳細: 
[1級・2級土木/noteコンテンツ計画.md §8.2](../../content/note/1級・2級土木/noteコンテンツ計画.md)）。
**唯一の未着手部分がこのコンクリート技士パイロット**。

コンクリート技士は四肢択一のみ（記述式なし・日本コンクリート工学会試験案内で確認済み）のため、
主任技士と同じ「小論文」商品は作れない。代替仮説: ①四肢択一・頻出論点完成パック
（材料/配合/フレッシュ性状/硬化コンクリート/製造品質管理/施工/構造/耐久性の論点別）
②現場判断ケース演習 ③主任技士へのステップアップ導線（無料ガイド）。

**着手する場合の手順**: (1) 公式受験案内・出題範囲を一次資料で棚卸し (2) 既存サイトの
コンクリート技士コンテンツ数・検索需要・競合・主任技士との重複を調査 (3) 2論点×各20問=40問を
パイロット生成 (4) 正答・全選択肢正誤理由・規格名数値・著作権・モバイル可読性を独立QA
(5) 無料サンプル10問＋有料30問で需要観測（価格はこの時点で競合と原価から決定）
(6) 4週間で無料閲覧・有料転換・問い合わせ・誤問率を確認し全論点展開の可否を決める。
全論点展開はパイロット品質が合格し販売導線・更新方法が定義できた場合だけ行う。

**完了条件**: 着手する/しないを決める。着手する場合はパイロット40問の公開まで。
