...
### [DN-0148] civil-practice 完遂後の残課題（note導線・数値出所の機械可読化・タグ語彙）
タグ: [コンテンツ品質] [種類:制作] [起票:2026-08-27]

civil-practice（土木施工の実務）は 2026-08-27 に44記事・図版17点・計算ツール7ページ
（原本HTML 10本を全移植）・現場管理値の早見表まで完遂し `visible:true` でナビ公開済み。
残るのは以下3点。

1. **note CTA・収益導線が今もゼロ**。`magazine-placement.ts` に civil-practice の配線が無く、
   資格カテゴリと違って記事末尾に商品への導線が出ない。実務者向けの導線（ココナラ添削・
   note マガジン等）を設計するか、意図的にゼロのままにするかを決める
2. **数値の出所が frontmatter に機械可読で無い**。各記事は本文で出典を明示しているが、
   示方書・JIS・労安則等の改訂時に影響記事を grep で洗い出す仕組みが無い。44記事分の
   出典を構造化するかは工数対効果で判断
3. **タグ語彙が `[guide]` 単独か `[guide, 分野]` かで記事によって揺れている**
   （`concrete` `earthwork` `rebar` `pavement`(未使用) 等）。10語以内に確定して
   `src/config/tags.json` を整理する
