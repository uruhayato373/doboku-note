...
領域 × 月の重点は `.claude/config/annual-roadmap.json` が正本で、管理画面 **計画 ＞ 年間ロードマップ**（`/plan/roadmap`）が試験カレンダー（`exam-calendar.json`）と重ねて描く。上段は資格ごとの申込・試験・合格発表と買い場（試験前 `buyWindowWeeks` 週）、下段は領域ごとの重点の帯。翌年の試験期で日付が未公表のものは、昨年度の同じ行事を 1 年ずらした推定として破線で出る。
