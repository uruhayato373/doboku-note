...
- **`--base`**: PR の base ブランチ（省略時は `develop` — AGENTS.md「ブランチ運用ルール」に準拠）
...
- **`git add .` / `git add -A` を使わない** — 明示的にファイル指定。`.env` などの混入防止（AGENTS.md「Git Safety Protocol」）
...
- **force push しない** — 特に base が main のとき（AGENTS.md 明示）
...
- `AGENTS.md` ── 「Creating pull requests」「Git Safety Protocol」セクション
- `.Codex/skills/dev/deploy/SKILL.md` ── develop→main 運用フロー（こちらは push 後のデプロイ確認まで）
