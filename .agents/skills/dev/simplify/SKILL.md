...
- プロジェクト内に既存のユーティリティがあるのに新規実装している
  - 例: `.Codex/scripts/lib/mdx-io.mjs` の `readMdxFile` / `writeMdxFile` を使わず、直接 `readFileSync` / `writeFileSync` を呼んでいる → **HIGH**（CRLF 事故の再発リスク）
...
<ファイルパス1>
  [HIGH] L42: readFileSync を直接呼んでいる
    → .Codex/scripts/lib/mdx-io.mjs の writeMdxFile を使うべき
...
3. **src/lib や .Codex/scripts/lib の変更があるなら `npm test`** を走らせる
...
**重要**: hook ではなくスキル内でチェーン化しているため、ユーザが `/simplify` を明示的に叩いた時のみ Step 5-6 が走る。自動発火はしない（AGENTS.md「実装時の行動原則」の確認原則を守るため）。
...
scripts/backfill-category.mjs
  [HIGH] L24: writeFileSync を直接呼んでいる
    → .Codex/scripts/lib/mdx-io.mjs の transformMdxFile を使う
    （AGENTS.md の MDX 書き込み規約、CRLF 保持）
```
...
- **重大度を過大評価しない** ── HIGH は「事故歴あり」「AGENTS.md 明示違反」に限定
- **既存のユーティリティ有無を Grep で確認せずに「既存あり」と主張しない** ── 確認手順を省略するとハルシネーションになる

## 参照

- `AGENTS.md` ── 「実装時の行動原則」（Karpathy 4 原則）とハーネス設計原則
- `.Codex/knowledge/reference/content-authoring.md` ── MDX コンポーネント・frontmatter 規約
- `.Codex/skills/dev/code-review/SKILL.md` ── コード品質レビュー（観点が重複するため、/simplify は「修正提案型」、/code-review は「批判型」と使い分け）
- `.Codex/skills/dev/pr-create/SKILL.md` ── Step 6 で呼び出す PR 作成スキル
