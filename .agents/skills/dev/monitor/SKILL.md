...
```
Monitor:
  command: bash .Codex/skills/dev/monitor/scripts/check-mojibake.sh 15
...
```
Monitor:
  command: cd "$CLAUDE_PROJECT_DIR" && node .Codex/scripts/upload-images-to-r2.mjs 2>&1 | grep --line-buffered -iE "(Found|Progress|FAILED|Done|Error|Uploading|Skipped)"
...
```
Monitor:
  command: node .Codex/skills/dev/monitor/scripts/check-frontmatter.mjs 60
...
```
Monitor:
  command: bash .Codex/skills/dev/monitor/scripts/health-check.sh 30
  ...
  command: |
    ...
    while true; do
      ...
      if [ -n "$changed" ]; then
        ...
        for f in $changed; do
          result=$(node .Codex/scripts/validate-mdx.mjs "$f" 2>&1)
