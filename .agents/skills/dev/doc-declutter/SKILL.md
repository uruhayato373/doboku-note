...
> **handoff の既定処分は extract→削除（2026-07-11 改定・archive 廃止）**: handoff は溜めない。生きたタスクは `.Codex/todo/backlog.md` へ、手順・runbook は `.Codex/knowledge/reference/` の該当 SSOT へ、個人知見は memory へ抽出し、本体は**残作業の有無に関わらず** `git rm`（**記録は git 履歴が持つ**＝復元可能）。**残作業がある＝KEEP ではない**（タスクは backlog が持つ）。KEEP は「当該セッションで今まさに編集中・未だ backlog 未抽出」のごく直近に限る。真実源 → `.Codex/knowledge/reference/information-architecture.md`「handoff のライフサイクル」。reference/project doc は従来どおり完了判定で KEEP/TRIM/DELETE。
...
4. **`git add` はしない。`git commit -- <pathspec>` で対象だけ**コミット（並行セッションの未 staged 変更を巻き込まない／AGENTS.md §3・§10）。
5. **memory も同期**。`~/.Codex/.../memory/` がその doc を参照していたら、本体 `.md` と `MEMORY.md` の索引行を実態へ更新（memory は repo 外＝commit 不要）。
...
grep -rl '<doc-basename>' docs .Codex --include='*.md' | grep -v '<self>'

# memory 参照
grep -rl '<doc-basename>' ~/.Codex/projects/*/memory/ 2>/dev/null
```
...
- **DELETE**（handoff の既定）: **生きたタスクを `.Codex/todo/backlog.md` へ・手順を reference へ・知見を memory へ抽出**してから `git rm <doc>`。handoff への出典引用は張り替え不要（対象外）。reference/project doc の場合は全参照を SSOT へ張り替え。
...
git commit -m "docs(handoff): <doc> を抽出→削除（…完了検証済・記録は git 履歴）" -- \
  docs/handoffs/<doc>.md .Codex/todo/backlog.md <抽出先ファイル...>
```
...
文字化け（U+FFFD）混入・CRLF 混在もチェック。完了後、何を削除/trim し・何を未確認で残したかを 1 行で報告（AGENTS.md §12）。
...
- **check-doc-refs が落ちる**: 削除した reference/project doc への参照が残っている。`grep -rl '<basename>' docs .Codex` で残存を洗い、SSOT へ全部張り替える（handoffs への参照は対象外なので落ちない）。
...
- ルール真実源: AGENTS.md §8 / `.Codex/knowledge/reference/information-architecture.md`「SSOT と参照規律」「handoff のライフサイクル」
