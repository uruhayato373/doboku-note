...
4. **一時ファイルは後処理で削除**: AGENTS.md の作業規約に従う
...
新しいサブエージェント（`.Codex/agents/*.md`）を作成・改修するときは、frontmatter の `model:` を必ず決める。既定値は **`sonnet`**。
...
> **モデル方針**: このエージェントは `model: {sonnet|inherit}` で動作します。{理由を1-2文で}。詳細は AGENTS.md「ハーネス設計原則」参照。
```

### 既存エージェントの model 指定一覧

AGENTS.md「ハーネス設計原則」のクイックリファレンス表を参照。エージェントを追加したら同表も更新すること。

### なぜこうするか

親エージェント（Codex 本体）が Opus で計画・判断・統合を担い、サブエージェントは Sonnet で高速・低コストに実行する。これによりコスト効率と判断の質を両立できる。Opus を固定指定するのはサブエージェント側で深い推論が不可欠なケースのみで、原則は親の Opus がレビューする前提で Sonnet に任せる。詳細は AGENTS.md「ハーネス設計原則」§5。
