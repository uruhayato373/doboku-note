...
- MCP: 主軸は **aidesigner**（`mcp__aidesigner__*`・`.mcp.json` に登録済み）。**Canva**（`mcp__claude_ai_Canva__*`）は Codex.ai コネクタとして接続されている場合のみ利用可（現状は未接続の想定・使う前に接続確認）。いずれも Codex.ai OAuth 経由のため**会社 PC のプロキシ（外部 API 遮断）でも到達する**（`curl`/node 直叩きとは別経路）。
...
2. デザイン SSOT と現状を読む:
   - `.Codex/knowledge/reference/ogp-prompts.md`（レイアウト・配色・テーマ色・変更履歴の真実源）
   - `.Codex/knowledge/design-system/note-cover-tokens.json` の `exams[].base`（資格別テーマ色の真実源）
...
1. レンダラを編集: `.Codex/skills/conversion/ogp-create/scripts/lib/ogp-templates.mjs`（mono-tag 等の satori レンダ関数）。新テンプレを足す場合は `/ogp-create` の「テンプレート追加手順」に従う（`templates.json` / `rules.json` / `ogp-prompts.md` も更新）。
2. デザイン SSOT を更新: `.Codex/knowledge/reference/ogp-prompts.md`（レイアウト・配色・**変更履歴**）。テーマ色を変えるなら `.Codex/knowledge/design-system/note-cover-tokens.json` の `exams[].base`。
3. 量産＆QA:
   ```bash
...
- 量産・本番生成（決定論テンプレ）: `/ogp-create`（`.Codex/skills/conversion/ogp-create/SKILL.md`）
- 資格別 AI 背景（背景だけを Gemini で生成・全記事共有）: `npm run ogp-backgrounds`（`scripts/generate-ogp-backgrounds.mjs`）。mono-tag の最背面に敷く下地用。**課金が発生するため実行前にユーザー確認**（`--dry-run` は無料）。意匠の全面試作は本スキル、背景下地だけならこちらが軽量。※**2026-07-02〜、現行の資格別背景は Codex 生成のブランド写真プール**（`.Codex/config/ogp/backgrounds/<exam-key>.png`・真実源 [`.Codex/knowledge/reference/brand-image-system.md`](../../../../.Codex/knowledge/reference/brand-image-system.md)）。この Gemini 生成はレガシー/代替扱い
- デザイン SSOT: `.Codex/knowledge/reference/ogp-prompts.md`（レイアウト・配色・テーマ色・変更履歴）
- テーマ色トークン（真実源）: `.Codex/knowledge/design-system/note-cover-tokens.json`
- レンダラ実装: `.Codex/skills/conversion/ogp-create/scripts/lib/ogp-templates.mjs`
- 一括目視 QA: `npm run ogp-gallery`
- AI デザイン MCP: `aidesigner`（スキル `/aidesigner` でも起動可）/ Canva（`mcp__claude_ai_Canva__*`）
- スキル設計規約: `.Codex/knowledge/reference/skills-design-guide.md`
