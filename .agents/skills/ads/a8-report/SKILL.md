...
> **正典は `.Codex/knowledge/reference/a8-affiliate-pipeline.md`**（レポート節）。本 skill は手順のみ。
> ラベル・URL・列名・プログラム写像は `.Codex/config/a8-report-automation.json` が SSOT、
...
- `.Codex/config/a8-report-automation.json` の `mediaId` / `targetSite` / `reports[].siteScope` を Read
- 前回実行: `.Codex/state/metrics/affiliate/a8-ui/last-run.json`
...
出力を見て `.Codex/config/a8-report-automation.json` の `a8.periodForm` を人間が確定する。
...
- `.Codex/state/metrics/affiliate/a8-report-log.json` — `siteSummary`（doboku 分離済み＝真実源）/
  `programPeriod`（allowlist 抽出）/ `monthly`・`daily`（**口座横断**）/ `crossCheck` を upsert
- `.Codex/state/metrics/affiliate/a8-results.json` — 既存スキーマの records へ rollup。
...
- 大きな変化があれば `.Codex/todo/weekly.md` の週次レビューに 1 行残す
...
- `.Codex/knowledge/reference/a8-affiliate-pipeline.md` — A8 運用 SSOT
- `.Codex/config/a8-report-automation.json` — ラベル/URL/列名/写像
- `scripts/lib/a8-report-csv.mjs` — 解析コア（`tests/a8-report-csv.test.mjs`・`npm test` に含まれる）
- `.Codex/scripts/report-buildjob-affiliate.mjs` — EPC 消費側
