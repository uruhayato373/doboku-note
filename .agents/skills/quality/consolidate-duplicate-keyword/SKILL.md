...
- `pe-chapters.json` の duplicate 検出（`.Codex/skills/quality/consolidate-duplicate-keyword/scripts/find-duplicate-keywords.mjs`）
- 7 フェーズの標準作業手順
- 過去の統合履歴と経験則警告
...
```bash
node .Codex/skills/quality/consolidate-duplicate-keyword/scripts/find-duplicate-keywords.mjs
```
...
- 長い表はモバイル視認性のため箇条書きに変換（`.Codex/knowledge/reference/content-authoring.md` の「モバイル視認性（詳細ルール）」参照）
...
```bash
# MDX コンパイル
node .Codex/scripts/validate-mdx.mjs .local/r2/posts/pe-comprehensive-management/A/article.mdx
...
| 連携先 | 役割 |
|---|---|
| **`.Codex/skills/quality/consolidate-duplicate-keyword/scripts/find-duplicate-keywords.mjs`** | Step 0 の候補検出 |
| **`.Codex/skills/quality/exam-backlinks/scripts/build-exam-backlinks.mjs`** | Phase 5 の自動生成 |
...
- `.Codex/skills/quality/consolidate-duplicate-keyword/scripts/find-duplicate-keywords.mjs` — 重複検出（本スキルの入口）
- `.Codex/skills/quality/exam-backlinks/scripts/build-exam-backlinks.mjs` — 自動生成スクリプト
- `public/_redirects` — Cloudflare Pages リダイレクト定義
- `.Codex/knowledge/reference/content-authoring.md` — モバイル視認性ルール（表 → 箇条書きの判断基準）
