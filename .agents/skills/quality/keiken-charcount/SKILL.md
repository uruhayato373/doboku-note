...
description: >
  ...
  .Codex/config/keiken-answer-sheet-limits.json のしきい値で OVER を surface する。決定論的処理（AGENTS.md 原則5）。
...
`.Codex/config/keiken-answer-sheet-limits.json`。設問区分（`current2_q1/q2`・`legacy3_q1/q2/q3`・`yosou`）ごとの `maxChars`。
