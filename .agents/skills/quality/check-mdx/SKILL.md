...
| Rule | 重大度 | 対象 | 検査内容 | 実装 |
|---|---|---|---|---|
| `syntax` | ERROR/WARN | MDX 全般 | MDX 構文（`{}` エスケープ、`<>` タグ、見出し階層、テーブル、数式、未対応記法の混入） | Codex 読解 |
| `frontmatter` | HIGH/MEDIUM/LOW | frontmatter | zod スキーマ + 内容（description 長・publishedAt・tags allowlist） | `.Codex/scripts/lint-frontmatter.mjs` |
...
| `staging` | — | Obsidian ステージング | 公開準備度（5 軸 15 点評価） | Codex 読解 |
...
| `related-keyword` | MEDIUM | キーワードページ | 末尾「関連キーワード: [A]、[B]」列挙パターン（ルール 8-1） | `.Codex/scripts/lint-mdx-mobile.mjs` |
| `legal-citations` | LOW | 法令引用 | e-Gov 法令検索リンク化されているか（ルール 8-2） | `scripts/rules/legal-citations/` |
| `note-link` | MEDIUM | note リンク | note 記事リンク（`note.com/dobokunote/n/`）が `<NoteLink>` 外（生 markdown・`<Callout>`・`<LinkCard>`）で書かれていないか（ルール 8-3） | `.Codex/scripts/lint-mdx-mobile.mjs` |
...
**出力**: ERROR / WARN / INFO。Codex が MDX を読んで判定。

### frontmatter — zod + lint（`.Codex/scripts/lint-frontmatter.mjs`）
...
**実行**: `node .Codex/scripts/lint-frontmatter.mjs <target>`
...
**実行**:
- 外部: `node .Codex/skills/quality/check-mdx/scripts/rules/links/check-external-links.mjs`
- 内部: `npm run check-links`（= `check-links.mjs`。`-- --scope note` 等を付与可）

#### 定期監査（`link-audit.yml`）

`.github/workflows/link-audit.yml` が毎週土曜 07:00 JST に `npm run check-links-audit`（`--scope all --report`）を実行し、`.Codex/state/link-audit/` に結果を蓄積して develop へ `[skip ci]` commit する。Issue は起票せず、`latest-report.md` 自体が記録。
...
**出力**: `.Codex/state/svg-audit.json`

**ギャラリー生成（目視 QA）**:
```bash
npm run svg-gallery               # site/note 図版を 1 枚 HTML で（site/note タブ＋資格別フィルタ・site は svg-audit.json 重大度バッジ）
node .Codex/skills/quality/check-mdx/scripts/rules/svg/build-gallery-comment.mjs  # 旧: GitHub コメント用 Markdown
```
...
**実行**: `node .Codex/skills/quality/check-mdx/scripts/rules/svg/audit.mjs`

### staging — Obsidian 公開準備度（Codex 読解）
...
**出力**: promote 候補リスト（推奨順）。Codex が Read で読んで判定。
...
**出力**: `.Codex/state/broken-explanations.json`

**引数**: `--category civil-construction-1` / `--topic 港則法` で絞り込み

**実行**: `node .Codex/skills/quality/check-mdx/scripts/rules/explanations/audit.mjs`
...
**実行**: `node .Codex/scripts/lint-mdx-mobile.mjs <target> 2>&1 | grep "8-1"`
...
```bash
# dry-run
node .Codex/skills/quality/check-mdx/scripts/rules/legal-citations/fix-legal-citations.mjs --dry-run

# 適用（バックアップは /tmp/fix-legal-citations-backup/ に保存）
node .Codex/skills/quality/check-mdx/scripts/rules/legal-citations/fix-legal-citations.mjs --apply
```
...
note.com 記事へのリンク（`note.com/dobokunote/n/`）は `<NoteLink>` コンポーネントに統一する規約（→ `.Codex/knowledge/reference/content-authoring.md`「リンク系コンポーネントの使い分け」）。生 markdown リンク・`<Callout type="reference">` 内・`<LinkCard>` で note 記事リンクを書いている箇所を MEDIUM で検出する。
...
**実行**: `node .Codex/scripts/lint-mdx-mobile.mjs <target> 2>&1 | grep "8-3"`
...
- `.Codex/scripts/lint-frontmatter.mjs` — frontmatter rule 実装
- `.Codex/scripts/lint-mdx-mobile.mjs` — related-keyword (8-1) / legal-citations (8-2) 検出ロジック
- `.Codex/scripts/lib/mdx-io.mjs` — CRLF 保持 I/O（`--fix` 系で使用）
...
- `.Codex/skills/authoring/keyword-page/SKILL.md` — 関連 Generator（作成規約）
- `.Codex/skills/authoring/improve-article/SKILL.md` — 関連 Generator（対話型修復）
