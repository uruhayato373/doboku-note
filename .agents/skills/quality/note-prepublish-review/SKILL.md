...
/note-prepublish-review {NN-...}
  ...
  │   ├ 売れる構成（観点喚起・report-only・自動判定/BLOCK しない）: 無料部分が `.Codex/knowledge/reference/note-selling-structures.md` の5ステップ骨格（悩み→原因→解決→読後の変化→次の行動）を満たし、記事タイプに合う型（悩み直撃/Before→After 等）が機能しているか目視確認を喚起
...
echo "MAGAZINE_CTA:"
node "$ROOT/.Codex/scripts/check-note-magazine-cta.mjs" "$F"
...
echo "THREE_SET:"
node "$ROOT/.Codex/scripts/check-note-3set.mjs" --require "$F"
...
case "$F" in
  */docs/note/magazines/*)
    ...
    node "$ROOT/.Codex/scripts/note-essay-charcount.mjs" "$F"
...
```bash
node .Codex/scripts/build-note-published-index.mjs
```

`.Codex/state/note-published.json` に集計が出力される。`pricing: paid` を含む将来の有料記事や series まとめページの参照源として利用。
...
| `.Codex/scripts/build-note-published-index.mjs` | 公開済み記事インデックス生成 |
| `note-link-injector` agent | リンク注入の Generator |
| `svg-figure-auditor` agent | 図版品質の Evaluator |
| `note-fact-checker` agent | 事実性の Evaluator |
| `.Codex/knowledge/reference/note-svg-policy.md` | 図版品質ルールの真実源 |
| `.Codex/skills/social/social-post/SKILL.md` | リンク注入ルールの真実源 |
...
- **Opus で考え、Sonnet で実行**: 3 エージェントは全て sonnet。最終判断（公開可否）は親 Codex（Opus）が行う
