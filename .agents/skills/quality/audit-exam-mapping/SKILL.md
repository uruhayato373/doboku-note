...
`.Codex/state/exam-keyword-map.json` の紐づけ精度を体系的に監査し、追加/削除候補を confidence 付きで surface する。`auto_apply` 階層は機械的に反映、`needs_review` 階層は NotebookLM サンプリング監査用シートに切り出す。
...
| ファイル | 役割 |
|---|---|
| `.Codex/agents/exam-keyword-mapping-auditor.md` | 過去問 1 問の紐づけを評価する Evaluator |
| `.Codex/scripts/build-keyword-summaries.mjs` | auditor Stage 2 用の事前資産（`keyword-summaries.json`）生成 |
| `.Codex/scripts/apply-audit-result.mjs` | 監査差分 JSON を機械的に exam-keyword-map.json に反映 |
| `.Codex/state/exam-keyword-map.json` | 単一正源（読み書き対象） |
| `.Codex/state/keyword-summaries.json` | 658 件のキーワード要約（事前生成） |
| `.Codex/state/improvements/pe-priority-2026-05-11.json` | リライト優先度（A/B/C/C'/D）注釈源 |
| `.Codex/state/exam-keyword-audits/{year}/{date}.json` | 監査差分の保存先 |
...
```bash
node .Codex/scripts/build-keyword-summaries.mjs
```

**処理フロー**:

1. **対象 anchor 列挙**: `.Codex/state/exam-keyword-map.json` の `pe-comprehensive-management.{year}-primary` から全 anchor を取得
...
3. **集約**: 全 anchor の JSON を `.Codex/state/exam-keyword-audits/{year}/{YYYY-MM-DD}.json` に統合
...
```
/audit-exam-mapping apply .Codex/state/exam-keyword-audits/r07/2026-05-11.json
```
...
git diff .Codex/state/exam-keyword-map.json
```
...
```
/audit-exam-mapping export-notebooklm-sheet .Codex/state/exam-keyword-audits/r07/2026-05-11.json
```
...
2. ソースとしてアップロード:
   - 該当年度の過去問 PDF（`.Codex/pdfs/exam/pe-comprehensive-management/{year}/`）
   - 上記シート markdown
...
```
/audit-exam-mapping verify-needs-review .Codex/state/exam-keyword-audits/r07/2026-05-11.json
```
...
`.Codex/state/exam-keyword-map.backup-{date}.json` を `exam-keyword-map.json` に上書きコピーし、`npm run refresh-indexes` を実行。
