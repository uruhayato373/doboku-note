...
- ブランチが `main` または `develop`（feature ブランチ不要、AGENTS.md §2 準拠）
...
```bash
node .Codex/skills/quality/civil-figure-rework/scripts/run-crop.mjs \
...
```bash
node .Codex/skills/quality/civil-figure-rework/scripts/inject-img-mdx.mjs \
...
# 整合性監査
node .Codex/scripts/audit-exam-figures.mjs

# バックリンク再生成（AGENTS.md §4 必須）
...
```bash
# 該当ページのファイルを明示指定（AGENTS.md §10、git add -A 禁止）
...
- `.Codex/agents/civil-exam-figure-extractor.md` — Generator agent
- `.Codex/agents/civil-exam-figure-auditor.md` — Evaluator agent
- `.Codex/scripts/audit-exam-figures.mjs` — 既存整合監査スクリプト
- `.Codex/scripts/lib/mdx-io.mjs` — writeMdxFile（CRLF 保持必須）
- `.Codex/knowledge/reference/image-policy.md` L165-177 — 過去問図 caption/alt 厳格ルール
- `.Codex/knowledge/reference/agents-registry.md` L71-96 — Generator/Evaluator 分業原則
- AGENTS.md §3, §4, §8, §10 — MDX 書き込み・refresh-indexes・レジストリ更新・並行エージェント commit ルール
