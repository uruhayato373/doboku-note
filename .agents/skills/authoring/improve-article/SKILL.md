...
```bash
node .Codex/scripts/notebooklm-cross-query.mjs --notebooks "総監標準テキスト" \
...
- **過去問解説の破損検出** — primary / secondary の MDX を扱う場合は `node .Codex/skills/quality/check-mdx/scripts/rules/explanations/audit.mjs --topic=<テーマ>` を走らせ、`」と規定されている` 文頭欠落や `summary="」..."` ExamPoint 欠落がないか確認。検出されたら `/improve-article` の修正案に必ず組み込む。意味矛盾（設問肢と同じ内容なのに ❌）は読者視点で拾う
- **SVG 品質の静的検査** — 記事に SVG を含む場合は `node .Codex/skills/quality/check-mdx/scripts/rules/svg/audit.mjs --path=<img dir>/*.svg` を走らせ、文字クリップ（P1）・必須属性欠落（P3）・テキスト重なり（P2）等を検出。HIGH は必ず修正、MEDIUM は視覚検証（Playwright スクリーンショット）と合わせて判断
...
【構造】
  ...
  [HIGH] 表4.5 ほか 3 つが 4 列超（AGENTS.md 違反）
...
| 修正タイプ | 使うツール |
|---|---|
| 構造（見出し・リスト・表） | Edit / Write（AGENTS.md「MDX ファイル書き込みの規約」遵守：CRLF 維持） |
| コンテンツ追記 | Edit / Write |
| SVG 新規作成 | `/create-svg` スキル（または直接 SVG を Write） |
| MDX 互換性 | `/check-mdx` スキル |
| モバイル視認性 | `/review-mobile` スキル |

1 記事編集が一区切りついたら AGENTS.md「コンテンツ編集時のコミット運用」に従って**即コミットを提案**（ただし実行は人間承認を得てから）。

### Step 5: 完了判定

**cem-qa の再呼び出しはしない**（初回評価の指摘と機械ゲートで判定できる。参考値のためだけに Evaluator を再起動すると 1 イテレーションあたり数分〜十数分のコストが乗る）。以下で判定する:

- NLM照合 HIGH がすべて解消 → Step 6 へ
- 初回 cem-qa が挙げた構造 HIGH（0点軸）が未解消 → 優先して修正、Step 3 に戻る
- MED 以下のみ残存 → Step 6 へ（残課題を記録）

> 構造を大きく作り替えた場合など、初回の指摘では判定できないときだけ cem-qa を再度呼ぶ。
...
3. **図版の扱い**: 視覚 HIGH/MED で SVG 化候補が出たら、**原 PNG を Read で目視確認**してから SVG 化すべきかを判断する。写真は SVG 化禁止（`.Codex/knowledge/reference/image-policy.md`）
...
```bash
node .Codex/skills/conversion/pdf-to-mdx/scripts/verify-pdf-mdx.mjs <mdx-path>
```
...
- `--pdf`: PDF 原本を明示指定（省略時は `.Codex/skills/conversion/pdf-to-mdx/scripts/verify-pdf-mdx.mjs` の `SLUG_PDF_HINTS` + title glob で自動発見）
...
- [.Codex/agents/civil-construction-qa.md](../../../agents/civil-construction-qa.md) — civil 記事の評価基準
- [.Codex/agents/cem-qa.md](../../../agents/cem-qa.md) — CEM 記事の評価基準
- [.Codex/agents/content-qa.md](../../../agents/content-qa.md) — 汎用（過去問）の評価基準
- [.Codex/skills/conversion/pdf-to-mdx/scripts/verify-pdf-mdx.mjs](../../conversion/pdf-to-mdx/scripts/verify-pdf-mdx.mjs) — verify mode の決定論的前処理スクリプト
- [.Codex/skills/authoring/create-svg/SKILL.md](../create-svg/SKILL.md) — SVG 図版作成
- [.Codex/skills/quality/quality-cycle/SKILL.md](../quality-cycle/SKILL.md) — CEM バルク処理（本スキルと補完関係）
- [.Codex/knowledge/reference/image-policy.md](../../../reference/image-policy.md) — 図/写真の判定フロー
- [AGENTS.md](../../../../AGENTS.md) — MDX 書き込み規約・コミット運用
