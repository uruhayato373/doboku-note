...
**対象スコープ**: `group: keyword`（または group 未設定）のキーワードページのみ。`group: guide` の **ガイド記事は対象外**（受験戦略・学習計画・俯瞰ハブはコンバージョン地点として別構造）。ガイド記事の末尾構成・禁止セクションは [content-principles.md §20](../../../../.Codex/knowledge/reference/content-principles.md) を参照。ガイド記事の生成・改善は `group: guide` 品質サイクル（`guide-rewriter`／`guide-qa`／`guide-fact-checker`）経由。
...
| コンポーネント | 用途 | 使用基準 |
|---|---|---|
| `<ExamPoint>` | 試験対策ポイント | **詳細ルールは `.Codex/knowledge/reference/content-principles.md` §5 を参照**（個数は原則1個・最大2個、3個以上禁止、総括位置必須、「誤り選択肢パターン」など過去問解説に属する内容は禁止） |
...
**詳細ルールは `.Codex/knowledge/reference/content-principles.md` §9「参考資料の構成」を参照**（公的資料＋民間記事 各最低1件、WebSearch→WebFetch検証必須、e-Gov法令リンク形式）。
...
4. **lint-mdx-mobile 実行**: `node .Codex/scripts/lint-mdx-mobile.mjs <file>` を実行
...
5. **`/review-mobile` プロンプトチェック**: 機械判定できないカテゴリ 2（解説の簡潔性）・4（段落構成）・5（コンポーネント）・**10（SVG 追加候補）** を Codex の目視判定で実施
   - HIGH/MEDIUM ゼロまでループ
...
4. **加重スコア ≥ 2.0 で完了**
5. いずれかの軸が 0 点なら即不合格
6. 参考資料軸の合格には `.Codex/knowledge/reference/content-principles.md` §9 準拠（公的＋民間 各1件以上）が必要
...
| モバイル視認性 | `/review-mobile` で HIGH/MEDIUM 違反ゼロ（`.Codex/scripts/lint-mdx-mobile.mjs` が裏で走る） |

## コンテンツ変更後のインデックス再生成

開発中にキーワードページを作成・改訂した後は `npm run refresh-indexes` で静的インデックスを更新すること（本番 `npm run build` では自動）。詳細は `.Codex/knowledge/reference/workflows.md`「コンテンツ変更後の静的インデックス再生成」を参照。

## 参照

- `.Codex/knowledge/reference/content-principles.md` — ペルソナ定義、コンテンツ原則（7項目）
- `.Codex/knowledge/reference/content-authoring.md` — MDX 作成詳細ルール、MDXコンポーネント一覧
- `src/config/pe-chapters.json` — 章・節構造
- `.Codex/skills/conversion/pdf-to-mdx/templates/cem.md` — 過去問MDXの構造ルール（RelatedKeywords の仕様）
- `.Codex/skills/quality/check-mdx/SKILL.md` — MDX 検査統合スキル（`--rules syntax` で作成後に実行推奨）
