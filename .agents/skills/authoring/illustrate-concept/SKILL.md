...
Codex が独力で作図すると「ありきたりの2カラム比較」に陥りやすく、かつユーザーが事前に指定した概念が実は図示に不向き（標準パターン未確立）だった場合に品質が出にくい問題を、Discovery First 方式で解決する。
...
**重要**: `caption` 属性は絶対に使わない（`.Codex/knowledge/reference/content-principles.md` 141 行目「`<ArticleImage>` の caption は使わない」ルール、過去に指摘済み）。図の説明は本文で行う。
...
- **トレース・再現は禁止**。形状・配色・レイアウトは `.Codex/knowledge/design-system/design-system.md` のデザイントークンに基づき独自作図
- 出典 URL は 2 箇所に必ず記録:
...
- `.Codex/knowledge/design-system/design-system.md` — コントラスト比 4.5:1・禁止パターン
- `.Codex/knowledge/reference/content-principles.md` — `<ArticleImage>` の caption 禁止ルールなど真実源
- `.Codex/knowledge/reference/content-authoring.md` — MDX コンポーネント・画像配信規約
- `AGENTS.md` § 「コンテンツ編集時のコミット運用」 — 1 記事 = 1 コミット原則
