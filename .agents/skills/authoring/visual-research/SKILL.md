...
NotebookLM（`.Codex/scripts/notebooklm-cross-query.mjs`、旧 `nlm cross query` 後継）で概念の内部構造を抽出し、参照ページの視覚パターンを着想源として、
...
`.Codex/skills/authoring/create-svg/SKILL.md` のルールを完全適用:
...
```bash
# 保存先
.local/r2/posts/pe-comprehensive-management/<slug>/img/figure-{N}.svg
...
node .Codex/skills/quality/check-mdx/scripts/rules/svg/audit.mjs \
  --file=.local/r2/posts/pe-comprehensive-management/<slug>/img/figure-{N}.svg \
  --fail-on=HIGH
```

```bash
git add .local/r2/posts/pe-comprehensive-management/<slug>/
...
- `.Codex/skills/authoring/illustrate-concept/SKILL.md` — Discovery First 方式（--ref 未指定時の自動Web検索手順）
- `.Codex/skills/authoring/create-svg/SKILL.md` — SVG 作図ルール・デザイントークン
- `.Codex/skills/authoring/notebooklm-research/SKILL.md` — notebooklm-cross-query の詳細
- `.Codex/knowledge/reference/content-principles.md` — `<ArticleImage>` caption 禁止ルール
