...
1. 変換した MDX を `.local/r2/posts/{category}/{slug}/article.mdx`（Convention B）として `writeMdxFile`（`.Codex/scripts/lib/mdx-io.mjs`）経由で書き込む（直接 `writeFileSync` は CRLF 混在を招き pre-commit で reject される）
...
2. **存在する場合**:
   ...
   - 手動同期する場合は `npm run upload-images-r2`（= `node .Codex/scripts/upload-images-to-r2.mjs --prefix {category}/{slug}`）を実行
3. **存在しない場合**:
...
本番 `npm run build` では自動実行されるが、開発中は手動で実行すること。詳細は `.Codex/knowledge/reference/workflows.md` を参照。
