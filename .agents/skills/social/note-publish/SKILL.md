...
note の埋め込み検出は **`keyboard.type`（実入力）で起動し、synthetic な `ClipboardEvent` paste では起動しない**（v1〜v5 の paste/Enter 系は全失敗、v6/v7 の type で確定。真実源 `.Codex/knowledge/reference/note-api-verification.md` L101「URL単独行で入力→Enterでリンクカード化」）。
...
- `.Codex/skills/social/publish-note/` — browser-use 版（Mac・手順の元）。本スキルはその Windows Playwright 版
- `.Codex/skills/social/note-magazine-add/` — 公開後のマガジン収録（別操作）
- `.Codex/knowledge/reference/note-api-verification.md` — 公開状態の実体検証
