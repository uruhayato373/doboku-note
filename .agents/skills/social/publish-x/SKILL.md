...
> **アカウントSSOT**: `.Codex/config/x-account.json`。旧アカウント @dobokunotecom が「platform manipulation and spam」で凍結（異議却下）。実因は**同一/類似文面の連続予約というスケジューリング失敗**。運用は新アカウント **@doboku373** で継続中（`disable-model-invocation: true` は維持＝Skill 自動起動はせず、人／明示コマンドで実行する）。
>
...
```bash
npx tsx .Codex/skills/social/publish-x/publish-x.ts 004 --tweet 1 2026-05-09T08:00 --dry-run
```
...
> [!warning] dry-run のログで「予約モード」か「即時投稿モード」かを必ず読む（2026-08-12 追加）
> `--dry-run` が緑（`19/19 件完了`）でも、それは**即時投稿モードのまま完走した緑**でありうる。
> 見るべきは件数ではなく次の 1 行:
>
> - `📅 予約モード確認OK` / `dry-run: 予約モードまで到達` → **正**
> - `即時投稿` / `dry-run: 即時投稿モード、投稿はスキップ` → **異常**。`--dry-run` を外すと全件がその場で連投される
>
> **zsh の罠**: 日時をシェル変数でまとめて渡すとき、zsh は展開時に単語分割をしないため
> `... 083 $DATES` は**日時 19 個が引数 1 個**になり、スクリプトは「日時指定なし＝即時投稿」と解釈する。
> 引数の数が合わないだけなのでエラーにならず、静かに即時モードへ落ちる。
>
> ```bash
> npx tsx ... 083 ${=DATES}   # zsh は ${=VAR} で明示分割。$DATES のままだと即時投稿になる
> ```
>
> 2026-08-12 に 19 本の一括予約で実際に踏み、dry-run のこの 1 行だけが 19 連投を止めた。
...
> **2026-06-12 凍結事故**: 運用 X アカウントが「platform manipulation and spam」で凍結された。Playwright 自動投稿は本質的に bot 的で、一括予約・同時刻・連投・重複文面・同一リンク反復は spam/自動化として検知されやすい。真実源は [`x-post-policy.md` §11](../../../../.Codex/knowledge/reference/x-post-policy.md)。本スキルは予約**補助**であり、全量を機械投稿に寄せない。
...
`x-post-qa`（人/エージェント採点）に加え、**予約実行の直前後に機械ゲートを必ず通す**（真実源 [`x-post-policy.md` §11.5](../../../../.Codex/knowledge/reference/x-post-policy.md)）:
...
# 3a. 即時投稿（特定ツイート）
npx tsx .Codex/skills/social/publish-x/publish-x.ts 004 --tweet 1 --immediate

# 3b. 予約投稿（全ツイート、日時をツイート数分並べる）
npx tsx .Codex/skills/social/publish-x/publish-x.ts 004 \
  2026-05-09T08:00 \
  2026-05-10T08:00 \
  2026-05-11T08:00 \
  2026-05-12T08:00 \
...
`.Codex/skills/social/publish-x/publish-x.ts`
