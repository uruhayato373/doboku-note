...
**Phase 0 ガード**: 本文に未反映プレースホルダー（`{{MAGAZINE_URL}}` または `※note 公開後…予定` 系）が残っていたら、その記事は**公開せず中断**。先に `.Codex/scripts/note/inject-magazine-url.cjs <persona> <マガジンURL>` で実 URL を注入しておく。
...
- **メンバーシップ連携記事（price=0 だが会員限定）**: `note-update-body` は3段の試し読みフロー（`公開に進む → 試し読みエリアを設定 → 更新する`）に対応。入口LP（まるごとパック等）の無料プレビュー復旧は `--trial-line-bottom`（試し読みラインを末尾-1に設置。絶対末尾は全ロックに戻る罠あり）。詳細 SSOT: `.Codex/knowledge/reference/note-api-verification.md`「メンバーシップ試し読みフロー」
...
- **中断した記事をそのまま再実行しない**。`note-update-body` は中断を `.Codex/state/note-update-aborted.json` に記録し、次回は SKIP する（`--force-retry` で解除）。解除前に必ず live を実査する
...
- `.Codex/skills/social/publish-note/references/` — note.com エディタ操作ノウハウ（stats47 由来・差分マップで読み替え）
- `.Codex/scripts/note/inject-magazine-url.cjs` — マガジンURLのプレースホルダ一括注入（doboku-note 版）
- `.Codex/knowledge/reference/note-essay-review-checklist.md` Step 10 — 公開後 URL 反映フロー
- `.Codex/skills/social/publish-x/` — 同系統のブラウザ自動投稿（persistentContext・偽成功検証の設計元）
