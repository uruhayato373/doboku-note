...
| シリーズ | 用途 | デザイン | パイプライン |
|---|---|---|---|
| **A. 択一クイズパック** | 運営者作問のシンプル知識クイズ | 既存 SVG ベース（明色） | `.Codex/scripts/sns/render-quiz-pack.mjs` |
| **B. 過去問パック** | 公式試験問題 H21-R8 全 720 問 | AIDesigner 新意匠（単一 brand + semantic、Manrope+NotoSansJP） | `ig-post-create.mjs --exam` |
...
| 役割 | ファイル |
|---|---|
| 過去問パック生成 (B) | `.Codex/skills/social/ig-post-create/scripts/ig-post-create.mjs` (`--exam`) |
...
| 過去問 SoT | `src/config/exam-questions.json` (720 問) |
| 過去問 slide-data SoT | `docs/sns/instagram/{exam}/exam-packs/<year>/pack-<NN>/slide-data.json` |
| 過去問スライドビルダー | `.Codex/scripts/lib/sns-common/quiz-slides.mjs`（tokens.json 参照） |
| 過去問デザイン真実源 | `.Codex/knowledge/design-system/instagram-carousel-tokens.json` + `.Codex/knowledge/design-system/instagram-carousel.md` |
| 択一クイズパック生成 (A) | `.Codex/scripts/sns/render-quiz-pack.mjs` |
| 択一クイズ source パーサ | `.Codex/scripts/sns/lib/quiz-parser.mjs` |
| 択一クイズテンプレ | `.Codex/scripts/sns/templates/quiz-ig.mjs` |
| 単独 KW (notebook) ビルダー | `.Codex/scripts/lib/sns-common/notebook-slides.mjs` |
| キャプション生成 | `.Codex/scripts/instagram/generate-caption.cjs` |

---

## B. 過去問パック（AIDesigner 新意匠・現行主軸）

> **デザイン真実源**: [`.Codex/knowledge/design-system/instagram-carousel-tokens.json`](../../../../.Codex/knowledge/design-system/instagram-carousel-tokens.json) と [`.Codex/knowledge/design-system/instagram-carousel.md`](../../../../.Codex/knowledge/design-system/instagram-carousel.md)。色・フォント・余白・スライド種別仕様はすべて tokens.json が真実源。`quiz-slides.mjs` はここから値を import する。
>
...
| 10 | 09-cta.png | 「もっと解きたい人は doboku-note で全問解説をチェック」+ stats（720問 PRACTICE / 5管理 SCOPE）+ 保存ボタン（brand 塗りつぶし）|
...
```bash
# 1. 過去問 MDX をパース（H21-R8 全 720 問抽出）
...
node .Codex/skills/social/ig-post-create/scripts/ig-post-create.mjs --exam r07-pack-01 --size carousel
...
# 2. レンダリング
node .Codex/scripts/sns/render-quiz-pack.mjs docs/sns/<channel>/draft/<NNN>-...
...
```bash
node .Codex/skills/social/ig-post-create/scripts/ig-post-create.mjs --slug heinrich-law --date 2026-05-09 --size carousel
```
...
`.Codex/skills/conversion/ogp-create/assets/fonts/`
