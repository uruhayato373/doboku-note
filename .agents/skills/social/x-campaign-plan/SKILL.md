...
description: >
  X の月次投稿計画（.Codex/config/x-campaigns/YYYY-MM-*.json）を 1 日 3 本体制（schemaVersion 2）で
...
| 何を決めるか | 読むファイル |
|---|---|
| 本数体制・スロット・凍結ガード | `.Codex/knowledge/reference/x-post-policy.md`（§11.2.1 スロット / §11.6 段階復帰 / §5.3 実績訴求） |
| 資格ごとの季節性（申込・試験日） | `.Codex/config/exam-calendar.json` |
| 何が実際に反応したか | `.Codex/state/x-metrics/own-posts.json`（`npm run x-own-metrics -- --report`） |
| 競合の動き | `.Codex/state/x-competitors/snapshot.json`（`npm run scout-x-competitors`・四半期） |
