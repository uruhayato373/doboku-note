...
1. `.Codex/knowledge/design-system/design-system.md` — デザイン単一 SSOT（トークン体系・レイアウト体系・記事 prose・5 原則・§8 禁止パターン）
...
UI コンポーネントを書く Generator（Codex 本体）と視覚回帰を検出する Evaluator を分離する。Generator は作った直後に自分の変更を「綺麗だ」と判断するバイアスを持つため、別プロセスで検証する。
...
- `AGENTS.md` — UI コンポーネント必須ルール（デザイントークン・ダークモード）
