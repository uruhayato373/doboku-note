...
> その場合は CI がコミットした `.Codex/state/metrics/{ga4,gsc}/` のスナップショットを読んで baseline/current を比較する（既定経路）。計測は CI/CD 供給が正で、ローカル creds 未設定は「計測基盤未整備」ではない。恒久ルール: `.Codex/knowledge/reference/measurement-incidents.md`（2026-06-05）。
...
本スキルは Anthropic "Building Skills for Codex" ガイドの **Pattern 3: Iterative refinement**（page 23）を NSM 改善に適用し、以下のサイクルを仕組み化する:
...
詳細は `.Codex/skills/management/nsm-experiment/references/definition.md` と `.Codex/pdfs/guide.pdf`（Chapter 3）を参照。

## GSC インデックス登録リクエストの実行と記録（2026-07-30 改訂）

本スキルの Use-when に「GSC インデックスリクエスト」が入っているが、実行手段は長らく**手作業**で、
記録も手書きの `.Codex/state/metrics/notes/gsc-indexing-requests-YYYY-MM-DD.md` だった
（`.Codex/state/*.md` 新規作成禁止＝AGENTS.md §8 に反する旧パターン）。現在は自動化してある:
...
- **記録の SSOT は `.Codex/state/metrics/gsc-indexing/{requests-latest,history}.json`**（追跡）。
...
1. `.Codex/state/experiments.json` を読み、`experiments[]` から `status` が `running` と `measuring` の実験を全件抽出（兄弟スキル weekly-review/weekly-plan/weekly-improve と同じく JSON 直読み。ヘルパーモジュールは介さない）
...
🔴 EXP-001 統合ハウスキーピング (running, 経過 X 日)
   ...
   理由: GSC 1 日クォータ上限到達
   参照: .Codex/state/metrics/gsc-indexing/history.json（機械記録・SSOT）
```
...
1. `node -e "import('./.Codex/scripts/lib/metrics-reader.mjs').then(async m => console.log(JSON.stringify(await m.fetchWeeklyNsmMetrics(), null, 2)))"` で現状取得
...
7. **roadmap フィードバック提案**: 成功パターンなら `.Codex/todo/backlog.md`（タスクマスタ）への追記提案を出力
...
| 連携先 | 役割 |
|---|---|
| **`.Codex/state/experiments.json`** | 実験 state 本体（JSON 直読み書き。専用ヘルパーモジュールは無い） |
...
| **`.Codex/skills/management/weekly-plan/SKILL.md`** | Phase 1 Agent C で実験提案を自動化 |
| **`.Codex/skills/management/weekly-review/SKILL.md`** | 実験進捗セクションで running を自動表示 |
| **`.Codex/skills/management/nsm-experiment/references/playbook.md`** | 実験パターンカタログ |
...
| **`.Codex/todo/backlog.md`** | close 時の learnings フィードバック先（タスクマスタ。旧 05_コンテンツロードマップ.md はアーカイブ済み）|
...
- `.Codex/pdfs/guide.pdf` Chapter 3 (Testing and iteration) ── Pattern 3 Iterative refinement の出典
- `.Codex/skills/management/nsm-experiment/references/definition.md` ── NSM 定義と目標値
- `.Codex/state/experiments.json` ── 実験 state 本体（JSON 直読み書き）
