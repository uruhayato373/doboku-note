...
> AGENTS.md §8 準拠で、レビューは `docs/reviews/weekly/YYYY-Www-review.md` に保存する（旧「Issue 一本化」方式は廃止）。
> なお、サブエージェントは Bash 不可（記憶 `feedback_agent_bash.md`）なため、データ収集は親が Bash で実行し、抽出済みテキストを分析に使う。
>
> **保持方針**: `docs/reviews/weekly/` は最新レビュー＋次週計画だけを保持する。新しい週次レビューを確定したら、旧週ファイルの未完タスクを `.Codex/todo/backlog.md`、恒久知見を `.Codex/knowledge/` へ抽出済みか確認して旧週ファイルを削除する。履歴目的では残さない（git履歴が記録を持つ）。
...
- IG 公開状態ドリフト: `npm run verify-ig-status` を実行（posted.json/status.json ↔ ライブ
  ...
  `/ig-reconcile` を実行して posted.json backfill / 未公開を予約（真実源 `.Codex/knowledge/reference/ig-publish-reconcile.md`）。
...
- **内部リンク健全性**: `npm run check-internal-links-vs-gsc -- --json`（オフライン・GSC UI SSOT 参照）。
  ...
  ※Playwright + Google ログイン必須＝ローカル実行限定。クラウド週次では実行不可なのでサーフェスのみ（真実源 `.Codex/knowledge/reference/gsc-management.md`）
- A8 成果取込期限（月次）: `npm run check-a8-report-due -- --json` を実行（30日。committed `affiliate/a8-ui/last-run.json` 参照・creds不要）。
  ...
  （真実源 `.Codex/knowledge/reference/a8-affiliate-pipeline.md`）
...
> GA4/GSC を取得し `.Codex/state/metrics/{ga4,gsc}/` に commit、`psi-audit.yml` が PSI を
...
> 詳細・恒久ルール: `.Codex/knowledge/reference/measurement-incidents.md`（2026-06-05 エントリ）

A. NSM 指標取得（既定 = スナップショット読み）:
- まず後述「スナップショット読み」でコミット済み JSON から WoW を算出するのが既定。
- creds + 外部到達性が両方ある環境（例: creds 入り macOS）に限り、任意で
  `node .Codex/scripts/lib/metrics-reader.mjs`（markdown 出力）でライブ取得してもよい
...
- **期限判定はまず `npm run check-experiments-due -- --json` を実行し、その結果を転記する**
  （LLM が経過日数を数え直さない。期限計算は決定的に決まる＝AGENTS.md §5）。
...
- 続けて `.Codex/state/experiments.json` を読み、status 別にグループ化:
...
補助コマンド:
- `node .Codex/scripts/lib/metrics-reader.mjs --json` で生データ
- 追加のディメンション別データ:
...
- CI（`fetch-metrics.yml` が毎週金曜 06:00 JST に commit）が残した
  ...
  - GA4 NSM（推奨）: `.Codex/state/metrics/ga4/ga4-channel-organic-*.json`（7日窓・JP・Organic Search のみ）の
  ...
  - GSC（推奨）: `.Codex/state/metrics/gsc/gsc-date-*.json`（7日窓・日次）の最新2ファイルで
    日次 clicks/impressions を合計して前週比。無ければ `gsc-query-*.json`（28日窓）にフォールバックし
    「28日ローリング」と明記。GSC は3日遅延があるため直近数日は未確定（両週同条件なので方向は有効）。
  - PSI: `.Codex/state/metrics/psi/psi-batch-*.json`（Agent C2 と同じ）を使う（ライブ PSI 呼び出し不要）
...
```
調査項目:
- .Codex/state/metrics/psi/psi-batch-*.json の直近 7 日分（GitHub Actions psi-audit.yml が develop に毎日 [skip ci] で commit）
- .Codex/config/psi-config.json のしきい値
- （廃止: `gh issue list --label performance,weekly-pdca` は GitHub Issue 廃止〔AGENTS.md §8〕で無効。違反の追跡は上記 psi-batch JSON の時系列＋しきい値比較のみで行う）
...
> 真実源: `.Codex/knowledge/reference/measurement-incidents.md`「2026-07-27: lab と field の判定原則」。
> 機械可読は `.Codex/config/psi-config.json` の `judgment`。
...
- `npm run report-monetization-coverage` を実行（tsx, 外部 API 不要）。
  - 入力: 最新 `.Codex/state/metrics/ga4/ga4-page-*.json`（流入）+ `ga4-cta-clicks-*.json`（クリック, あれば）
  - 配置の真実源: `src/lib/magazine-placement.ts`（note）/ `src/app/docs/[...slug]/page.tsx`（アフィリ）
  - 出力: `.Codex/state/metrics/monetization/coverage-latest.md`（+ coverage-*.json）
...
- SNS 流入: `.Codex/state/metrics/ga4/ga4-sourceMedium-sns-*.json` の最新 2 ファイルで
...
- 週次スナップショット: `.Codex/state/weekly-metrics/` の最新 YYYY-Www.json の `sns` セクション
  （source 別 WoW・合計）も併記できる（上と同じ CI 由来）。
- YT 公開照合: `.Codex/state/yt-verify/latest.json`（verify-yt-status.yml が週次で commit）の
...
```
調査項目:
- .Codex/state/weekly-reports/ の当週計画ファイルを読み込み
- 計画タスクの完了/未達を判定
...
- /distill-proofread-learnings --since "7d" を呼び出す
  （スキル実体: .Codex/skills/management/distill-proofread-learnings/SKILL.md）
- 分析対象: git log で直近 7 日の `.local/r2/posts/` 配下の MDX/SVG コミット
- 出力: .Codex/state/proofread-learnings/YYYY-MM-DD.md
...
### 学習ログ
- `.Codex/state/proofread-learnings/YYYY-MM-DD.md` に詳細記録
...
> report のまま放置すると誰も読まない——`knip` は `batch-approve.mjs` の壊れ import を
> 報告し続けたまま 4 か月放置され、`note-meta-lint` は 3 週間ゼロ件検査のまま FAIL を
> 出し続けていた（2026-08-16 発覚）。
...
node -e "const d=require('./.Codex/state/dispatch/dispatch-log.json');const w=d.entries.filter(e=>e.date>='<今週月曜>');const by=k=>w.filter(e=>e.outcome===k).length;console.log(JSON.stringify({total:w.length,done:by('done'),swept:by('swept'),blocked:by('blocked'),fail:by('fail'),byExecutor:w.reduce((a,e)=>((a[e.executor]=(a[e.executor]||0)+1),a),{})}))"
```
...
- **台帳の健全性**: `check-backlog-health` の S2（🟢/🟣 に沈んだ不具合）・S4（`種類:定期`＝backlog の役割違反）・S9（`.Codex/todo` の 4 層以外）。**この 3 つが 0 でない週は放置しない**（S2 は選定順で先頭に出るのに tier が嘘をついている状態、S4/S9 は置き場違い）。しきい値を超えたら次セッションで `/backlog-sweep --audit`
...
> **サマリーは 5 行以内**（成果・要注意・数値の要点のみ）。**各セクションは箇条書き 3〜5 点**。
> **表は上位 5 行＋「他 N 件」**。エージェントの出力をそのまま貼らず、要点に落として書く。
> 該当ゼロのセクションは「なし」の 1 行で閉じる（空の見出しを並べない）。
> **検出と表示は別**: 違反・ドリフト・候補は全件把握したうえで、載せきれない分は件数と参照先（JSON パス等）を必ず書く。黙って落とさない。
...
<!-- Agent C が .Codex/state/experiments.json から running/measuring/今週 close を自動生成。
...
<!-- Agent C2 が .Codex/state/metrics/psi/ と open/closed Issues から自動生成。
...
<!-- Agent F が `.Codex/state/metrics/ga4/ga4-sourceMedium-sns-*.json`（最新2件）で source 別 WoW、
     `.Codex/state/weekly-metrics/` 最新の sns セクション、`.Codex/state/yt-verify/latest.json` の
...
- `.Codex/skills/management/nsm-experiment/SKILL.md` — 実験ライフサイクル管理
- `.Codex/scripts/lib/metrics-reader.mjs` — NSM 週次メトリクス取得（本スキル Agent C の中核）
- `scripts/x-queue-surfacer.mjs` — X 予約キューの未投入下書き surfacer（本スキル Agent I の中核。`npm run x-queue-surfacer`）
- `.Codex/skills/analytics/fetch-gsc-data/scripts/fetch-gsc-data.mjs` — GSC 個別取得（ページ別・フィルタ付き）
- `.Codex/scripts/fetch-ga4-data.mjs` — GA4 個別取得（ディメンション・メトリクス指定）
...
- `.Codex/skills/management/nsm-experiment/references/definition.md` — NSM 定義の真実源
