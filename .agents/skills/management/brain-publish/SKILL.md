...
name: brain-publish
description: >
  Brain（brain-market.com）へ Codex キット商品を Playwright で出品するスキル。カタログ
  （src/lib/brain-products.ts＝価格/状態/URL）と listings（.Codex/config/brain-listings.json＝
...
[.Codex/knowledge/reference/brain-operations.md](../../../../.Codex/knowledge/reference/brain-operations.md)。

## 手順

1. **整合ゲート**: `npm run check-brain-wiring` が green であること（catalog↔listings↔dist ZIP↔配布URL位置）。
   赤ならエラー行の指示どおり SoT を直してから進む。
2. **配布物の確認**: `.Codex/config/brain/dist/<distFile>` が main に載り、
...
販売設定ステップ 5b でカテゴリ v-select を選択（**仮想リスト**＝候補は wheel スクロールで DOM 化してから click→assert）。全候補・注意点は [brain-operations §4/§6](../../../../.Codex/knowledge/reference/brain-operations.md)。

## 本文（LP）差し替え（再審査を伴う）

既定は既存本文を保持（>50字なら挿入スキップ）。listings の bodyText を書き直してライブへ反映するには **`--replace-body`**（既存を全消去→再挿入）。本文変更も再申請＝再審査。**書き直す際は構成の型＝[note-selling-structures.md](../../../../.Codex/knowledge/reference/note-selling-structures.md)「強化コンポーネント」を参照**（誠実表現は brain-operations §5-6）。

```
node scripts/brain-publish.mjs --service <id> --edit-url <editUrl> --commit --force-resubmit --replace-body [--set-category <label>]
```

本文に**図版を挿入**するなら `--insert-figures <json>`（`[{after,image}]`）を併用。**重複防止のため必ず `--replace-body` と併用**（Brain は保存前でも画像を保持し、やり直すと重複する）。挿入ロジックは `scripts/lib/brain-figures.mjs`、図版は `.Codex/config/brain/assets/figures/`。
