...
SEO を4面（技術健全性 / index coverage / performance / 検索意図）で束ねるオーケストレータ。各面は既存の分業体制（`.Codex/knowledge/reference/gsc-management.md` の分業表）に乗り、**決定的な検出は機械スクリプト、意味的な評価は audit-only Evaluator、修正はしない**という原則を通す。
...
1. **機械スクリプトを先に走らせる（決定的検出）**
   ...
   - 計測: `.Codex/state/metrics/{gsc,ga4}/` の最新スナップショット日付を確認（無ければ CI 実行を案内）
...
4. **打ち手はユーザー判断へ**
   ...
   - hygiene（404/redirect）: `.Codex/todo/` へ起票を提案
...
- `.Codex/knowledge/reference/gsc-management.md` — GSC 管理 SSOT（分業表・閾値・判断マトリクス）
- `.Codex/agents/technical-seo-auditor.md` / `search-intent-auditor.md` — 本スキルが新設した Evaluator
- `.Codex/agents/{gsc-index-auditor,metrics-analyzer,performance-auditor}.md` — 既存 Evaluator（本スキルが束ねる）
- `scripts/check-seo-build.mjs` / `.Codex/skills/quality/check-seo-meta/` — 決定的検出（機械）
- `.Codex/skills/management/{gsc-review,weekly-improve,nsm-experiment}/` — 各面の既存スキル（重複させない）
