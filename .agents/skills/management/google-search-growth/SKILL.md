...
オーケストレータ。**副作用（ブラウザ操作）があるため Codex からの自動呼び出しは禁止**（ユーザーが
明示起動する）。実装・設計の真実源は
...
1. **preflight**
   ...
   - `.Codex/config/google-console-automation.json` の property を確認
   - `.Codex/state/metrics/{gsc,ga4,url-inspection}/` の最新スナップショット日付を確認
...
5. **join**（決定的スクリプト）
   ...
   - 生成: `.Codex/state/improvements/search-growth-<run>.json` と `search-growth-latest.md`
...
10. **finalize（継続運用）**
    - **観測・判断ログ追記**: `.Codex/knowledge/reference/gsc-management.md` の「観測・判断ログ」へ、当月の理由別件数・
...
`.Codex/skills/management/google-search-growth/references/recovery.md` を参照。
...
- GSC 管理 SSOT: `.Codex/knowledge/reference/gsc-management.md`
- Playwright 認証: `.Codex/knowledge/reference/playwright-auth-profiles.md`
- 計測の原則: `.Codex/knowledge/reference/measurement-incidents.md`（CI 供給が正・外部検証の罠）
