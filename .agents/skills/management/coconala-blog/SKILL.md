...
`.Codex/state/coconala/blog-competitors.json` を読み、競合が書いていない／自分に一次資産がある論点を選ぶ。
...
- 同名記事がココナラ側に**まだ無い**（前回中断のオートセーブ残骸→二重投稿の防止）
- 本文の反映が元テキストの 90% 以上（途中で切れていない）
- **見出しが実際に適用されている**（`h2.c-blogBody_h2` の DOM 実測。クリック成否では判定しない）
- **ブロックの順序が原稿と一致**（字数とカード枚数が正常でも順序だけ壊れる事故があった）
