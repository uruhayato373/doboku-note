...
description: >
  ...
  （src/lib/coconala-services.ts＝価格/状態/URL）と listings（.Codex/config/coconala-listings.json
...
- 判定ロジックは `scripts/lib/coconala-guards.mjs`（`tests/coconala-guards.test.mjs` で固定）。詳細 → [coconala-operations.md §8](../../../../.Codex/knowledge/reference/coconala-operations.md)
...
- **代筆禁止・外部誘導禁止**: 出品文面の原則は `.Codex/knowledge/reference/coconala-operations.md` §5・展開キット §2-3。
- **構成の型**: 文面を書き直すときは `.Codex/knowledge/reference/note-selling-structures.md`「強化コンポーネント」を参照（C系PDF=直適用／S系人力=翻案・body 1000字上限）。
...
```
npm run gen-image-gemini -- --out .Codex/config/coconala/assets/bg-civil.png --prompt "..."  # 背景（Gemini API課金）
...
- `--image` は **bare 名でよい**（`.Codex/config/coconala/assets/` に自動解決）。存在は**ブラウザ操作前に検査**（fail-fast）＝不正パスで orphan draft を残さない。
...
素材は `.Codex/config/coconala/assets/`（`bg-civil.png`/`bg-docs.png`／`thumb-<key>.png`）。詳細は [coconala-operations.md §8](../../../.Codex/knowledge/reference/coconala-operations.md)。
...
- 投入 SoT: `.Codex/config/coconala-listings.json`（本文/カテゴリ/納期/genreFacets）
- 価格/状態 SoT: `src/lib/coconala-services.ts` ／ アカウント: `.Codex/config/coconala-account.json`
- 運用 SSOT: `.Codex/knowledge/reference/coconala-operations.md` ／ 戦略・文面: `docs/note/1級・2級土木/ココナラ展開キット.md`
- エージェント: `.Codex/agents/coconala-operator.md` ／ KPI 照合: `/coconala-status` ／ 受注: `/coconala-order`
