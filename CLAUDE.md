...
- **画像追加**: `generate-webp` → webp 参照で commit → R2 は `main` push 時に CI（`r2-sync.yml`）が自動同期（対象 path = `**/img/**`）。`ogp.png` は git 追跡せず `asset-offload`（次項参照）で直接 R2 へ、`ogp.webp` は未使用のため作らない
