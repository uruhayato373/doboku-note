...
- **ドキュメント ライフサイクル（肥大化の棚卸し）**: doc が「完了済みなのに active」「他 SSOT と重複」「完了行が混じって肥大」していくのを定期的に棚卸しする。`npm run check-doc-lifecycle`（機械 surfacer＝完了 handoff 候補を鮮度/orphan で列挙）→ **`/doc-declutter`**（`doc-curator` Evaluator が KEEP/TRIM/ARCHIVE/DELETE/CONSOLIDATE を判定→退避/trim/参照更新/memory 同期まで適用）。pre-commit hook が active handoff 蓄積時に nudge する。**鉄則＝外部実体（PR merged・published:true・deploy・ファイル実在）を検証してから処分、未確認なら DELETE せず ARCHIVE・退避≠削除・参照は同一 commit で張り替え**。`/doc-sync`（コード変更起点の prose 陳腐化）とはトリガーが直交（2026-06-17 新設）
