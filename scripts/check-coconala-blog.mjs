 ...
 *
 * 2モード:
 *   --staged  pre-commit。staged の docs/coconala-blog/**\/article.md だけを検査
...
function collect() {
  if (STAGED) {
    ...
    return out.split('\n').map((s) => s.trim()).filter((p) => /^docs\/coconala-blog\/.+\/article\.md$/.test(p))
...
if (AS_JSON) {
  console.log(JSON.stringify({ ...out, inconclusive }, null, 2));
  process.exit(inconclusive ? 2 : out.due ? 1 : 0);
}

if (out.target === 0) {
  console.log(`${TAG} 対象ゼロ（${STAGED ? 'staged に該当記事なし' : 'docs/coconala-blog/ に記事がまだありません'}）— 異常なしではなく未着手`);
  process.exit(0);
}
