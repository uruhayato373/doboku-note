...
function parseTweetsFile(content, folderExam = "pe-comprehensive") {
  ...
  return blocks
    .map((rawBlock) => {
      // 制作メモ（HTML コメント）を落とす。除去規則は scripts/lib/x-tweets-md.mjs が唯一の実装
      // ——ここにローカル実装を置いていたため、同じ tweets.md を読む検査側が持っていない
      // 非対称が生まれた（2026-08-18 に共有化）。
      const block = stripTweetMemos(rawBlock);
