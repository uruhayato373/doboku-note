...
function findTxtFiles() {
  let entries = [];
  try { entries = readdirSync(join(ROOT, 'content/note'), { recursive: true, encoding: 'utf-8' }); } catch { return []; }
  return entries.filter((f) => f.endsWith('note掲載文.txt')).map((f) => join(ROOT, 'content/note', f));
}
