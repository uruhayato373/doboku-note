...
const GATES = [
  {
    name: 'check-note-cover-tokens',
    cmd: ['node', 'scripts/check-note-cover-tokens.mjs'],
    re: /記事 dir\s*(\d+)\s*件/,
    // 走査は content/note 全体の再帰。現状 590 件。article*.md の正規表現が壊れると急減する
    // （2026-07-28 の `article.md` 固定で型別ファイルを落とした事故と同型）。
    min: 300,
  },
  {
  ...
  // ── SNS・画像系（2026-08-18 追加）──
  // それまで GATES は note/図版だけで、SNS 系は 1 本も見張られていなかった。
  // check-x-utm / check-ig-cta / check-sns-urls 等が走査崩れで `✓ 0` を返しても誰も気づけない。
  {
    name: 'check-sns-urls',
    cmd: ['node', 'scripts/check-sns-urls.mjs'],
    re: /✓\s*(\d+)\s*ファイル/,
    aliveRe: /本番に存在しない \/docs\/ リンク\s*\d+\s*件/,
    // content/sns 配下の全チャネル。実測 886 ファイル。走査が壊れると 0 に落ちる
    min: 400,
    note: 'SNS 素材のファイル数。/docs/ リンクの実在検査はここが 0 になると全部素通りする',
  },
  {
    name: 'check-x-utm',
    cmd: ['node', 'scripts/check-x-utm.mjs'],
    re: /✓\s*(\d+)\s*ファイル/,
    aliveRe: /UTM 規約違反の X 送客リンク\s*\d+\s*件/,
    // 2026-08-18 時点は違反 82 件（すべて _archive-old-account の退役ドラフト）で赤。
    // aliveRe があるので coverage としては合格＝「走査は生きている」ことだけ見る。
    //
    // 2026-08-20: min 100 は**この検査より前の実態**（旧アカウント分を含む数）に基づいていた。
    // check-x-utm は `_` 接頭辞ディレクトリを除外するので、tweets.md 65 件のうち
    // `_archive-old-account` 32 + `_archive-2026-pe-countdown` 4 = 36 件は対象外で、
    // **正しい実検査数は 29**。100 のままだと「常に下限割れ」で警告が鳴り続け、
    // 本物の走査崩れ（0 件）と区別がつかなくなる（狼少年）。20 に下げて 0 検知は保つ。
    min: 20,
    note: 'X 下書きのファイル数。パス判定が \ 区切りで壊れると 0 になる（note 側で実際に起きた型）',
  },
  {
    name: 'check-ig-cta',
    cmd: ['node', 'scripts/check-ig-cta.mjs'],
    re: /✓\s*(\d+)\s*パック/,
    aliveRe: /件のテンプレ逸脱/,
    min: 20,
    note: 'IG パック数。パック探索が壊れると 0 になり CTA 逸脱を全て見逃す',
  },
  {
    name: 'check-ig-cover',
    cmd: ['node', 'scripts/check-ig-cover.mjs'],
    re: /✓\s*(\d+)\s*パック/,
    aliveRe: /件のテンプレ逸脱/,
    min: 20,
    note: 'IG パック数（表紙・白背景）。上と同じ探索を通るので同時に 0 化しうる',
  },
  {
...
if (JSON_OUT) {
  writeSync(1, JSON.stringify({ results }, null, 2) + '\n');
