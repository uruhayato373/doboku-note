 ...
 *
 * 判定:
 *   - 対象 = .local/r2/posts/**\/*.mdx|*.md 内の <ArticleImage src="/posts/....svg">
...
function listStagedArticles() {
  ...
  return out
    ...
    .filter((l) => l.startsWith(".local/r2/posts/") && !l.includes("/img/") && isArticleFile(l))
    .map((rel) => join(ROOT, rel).replace(/\\/g, "/"))
    .filter((full) => existsSync(full));
}

/** src="/posts/..." → .local/r2/posts/... の絶対パス */
...
if (candidates > 0 && resolved === 0) {
  ...
  console.error(`  想定原因: .local/r2/posts のパス解決の破損 / src の接頭辞規約変更。`);
  process.exit(1);
}
