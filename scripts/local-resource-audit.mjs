...
export async function run({ argv = [], quiet = false } = {}) {
  ...
  try {
    ...
    if (args.includes('--json')) out.log(JSON.stringify(snapshot, null, 2));
    else {
      out.log(`[resources] free disk ${(snapshot.machine.freeDiskBytes / GiB).toFixed(1)} GiB; free memory ${(snapshot.machine.freeMemoryBytes / GiB).toFixed(1)} GiB`);
      for (const row of snapshot.directories || []) out.log(`  ${row.path}: ${(row.bytes / GiB).toFixed(2)} GiB / ${row.files} files / links skipped ${row.links}`);
      for (const group of snapshot.processGroups || []) out.log(`  ${group.kind}: ${(group.bytes / GiB).toFixed(2)} GiB working set / ${group.count} processes (shared pages included)`);
      for (const warning of snapshot.warnings) out.log(`  WARN ${warning}`);
      for (const error of snapshot.errors) out.error(`  INCOMPLETE ${error}`);
    }
    return out.result(snapshot.errors.length ? 2 : 0);
  } finally { release(); }
}
