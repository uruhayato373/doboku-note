...
function stagedNameStatus() {
  const out = execFileSync(
    'git',
    // 日本語パスは既定でエスケープされるので quotepath を切る（パス比較が全部外れる）
    ['-c', 'core.quotepath=false', 'diff', '--cached', '--no-renames', '--name-status', '--diff-filter=ADM'],
    // 既定 1MB では大規模な移行コミットで ENOBUFS になり、**ゲートが例外で落ちて
    // commit ごと止まる**（2026-08-18 に 4,000 ファイルの移動で実際に発生）。
    // 検査が「落ちた」のか「合格した」のか区別できない状態を作らないために広く取る
