 ...
 *   4. dispatch-log へ ID・outcome・note を記録する
 *   5. monthly.md / weekly.md から当該 ID の行を削除する（あれば）
 *   6. backlog カードを削除する
 *   7. claims.json から記録を消す
 *   8. schema・task-plan-links の検査を再実行する
 *
 * plan ディレクトリの削除（処方箋4条件7の後半）は99-finalize-and-delete.mdの
 * 受入条件確認が要るため自動化しない。plan が残っていれば出力で明示する。
 *
 * Usage:
 *   node scripts/todo-complete.mjs DN-####                          # dry-run（チェックリスト表示のみ）
 *   node scripts/todo-complete.mjs DN-#### --confirm-conditions --commit --note "..."
 *
...
if (!id || !/^DN-\d{4}$/.test(id)) {
  console.error('使い方: node scripts/todo-complete.mjs <DN-####> [--confirm-conditions --commit --note "..."]');
  process.exit(2);
}
