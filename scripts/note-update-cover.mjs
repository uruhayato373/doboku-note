...
async function doOne(page, { abs, noteId, pricing, cover }) {
  ...
  for (let i = 0; i < 10; i++) {
    await sleep(1800);
    const paidArea = await page.getByRole('button', { name: '有料エリア設定' }).count();
    const trialArea = await page.getByRole('button', { name: '試し読みエリアを設定', exact: true }).count();
    const update = await page.getByRole('button', { name: /^(更新する|更新)$/ }).count();
    if (paidArea || trialArea || update) { onSet = true; break; }
  }
  if (!onSet) {
  ...
  const area = pricing === 'free' ? { count: async () => 0 } : page.getByRole('button', { name: '有料エリア設定' });
  if (await area.count()) {
    ...
    // 2026-08 UI: 無料索引でも有料マガジン収録時はこの入口が表示される。
    // ラインを動かさず設定画面を1段進め、既存の公開範囲を保持する。
    await page.getByRole('button', { name: '試し読みエリアを設定', exact: true }).first().click();
    await sleep(3500);
    console.log('[free] 試し読み設定を保持して更新へ進む');
  } else {
    console.log('[free] 有料エリア設定なし=無料記事');
  }
  // 4c. 更新する → 通知いいえ
  let upd = page.getByRole('button', { name: '更新する', exact: true });
  for (let i = 0; i < 6 && !(await upd.count()); i++) {
    await sleep(1000);
    if (!(await upd.count())) upd = page.getByRole('button', { name: '更新', exact: true });
  }
  if (!(await upd.count())) { console.error('[FAIL] 更新する未検出'); return false; }
  await upd.first().click(); console.log('[save] 更新');
  await sleep(2500);
  ...
  try {
    const rel = String(abs).replace(/\\/g, '/').replace(/^.*?(content\/note\/)/, '$1');
