...
// - フレーミング厳守: 総監=分析力 / 元発注者=採点者視点 / 施工管理技士=当事者（「予想的中」とは呼ばない）
...
const BASE_DIR = join(ROOT, 'docs', 'note', '1級・2級土木');
const BANNER_SRC = join(ROOT, 'docs', 'note', '共通', '著者オーソリティ', 'img', 'figure-author-authority.png');
const BANNER_NAME = 'figure-author-authority.png';
const MARKER = BANNER_NAME; // 冪等判定は画像ファイル名の存在で（note に残る HTML コメントは使わない）
const ALT =
  '技術士（総合技術監理部門）を持つ元発注者が、施工管理技士の記述を分析して作成。総監＝上位資格の分析力、元発注者＝採点者の視点、施工管理技士＝合格した当事者';
const BRIDGE =
