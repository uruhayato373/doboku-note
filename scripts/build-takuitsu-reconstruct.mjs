...
const THEMES = {
  ...
  sekokeikaku: {
    ...
    include:
      /施工計画(の作成|の立案|作成|立案|における建設機械)|事前調査|仮設工事(計画)?|建設機械(の(最近の動向|経費|組合せ|貸与|選定)|に関する|用エンジン)|工事の原価管理|工事原価管理|原価管理|土木工事の施工に際し行う届出や許可/,
    ...
    subtopics: [
      ...
      { key: 'cost', label: '原価管理・経済性', re: /原価管理/ },
      // 関係機関への届出・許可（労働安全衛生法以外）は出題数が少なく
      // 単独章に満たないため、施工計画の作成・立案の受け皿に統合する。
      { key: 'planning_other', label: '施工計画の作成・立案（届出・許可ほか）', re: /.*/ }, // 受け皿
    ],
  },
  kankyo: {
    key: 'kankyo',
    ...
    // 含み既に収録済み）。
    include:
      /環境保全|騒音|振動|建設リサイクル|建設工事に係る資材の再資源化|建設副産物|建設発生土|産業廃棄物|マニフェスト|水質汚濁|土壌汚染|汚染土壌|地盤沈下/,
    exclude:
    ...
    subtopics: [
      ...
      { key: 'water_soil', label: '水質汚濁・土壌汚染対策', re: /水質汚濁|土壌汚染|汚染土壌/ },
  ...
  hinshitsu: {
    ...
    // 組合せ形式で図は不要）。
    include:
      /品質管理|品質マネジメント|品質特性|ヒストグラム|管理図|レディーミクストコンクリート|受入れ検査|非破壊検査|非破壊試験|微破壊/,
    ...
    subtopics: [
      ...
      { key: 'soil', label: '盛土（土工）の品質管理', re: /盛土/ },
      ...
      // 鉄筋の検査・非破壊検査（コンクリート構造物の強度・鉄筋位置推定）は
      // 出題数が少なく単独章に満たないため受け皿章として統合する。
      { key: 'ndt', label: '鉄筋の検査・非破壊検査ほか', re: /.*/ }, // 受け皿
    ],
  },
}
