 ...
 * （2026-07-28 実測＝添付済み記事2本とも未ログイン HTML で出現0）。ただし**本数**は公開 API で取れる
 * （2026-09-23 実測: 有料 614 本は remained_file_num＝有料エリアの添付数が原稿の本数と全件一致、無料 1 本は
 * 公開本文にダウンロードリンクとして出る）。本数の欠落は CI の週次 check-note-public-view が見る。
 * どのファイルが付いているか（名前・中身）は引き続きこの --live（要ログイン）でしか確かめられない。
 *
 * 使い方:
...
if (!LIVE) {
  ...
  if (JSON_OUT) {
    console.log(JSON.stringify({ mode: 'source', inspected: targets.length, withPdf: withPdf.length, violations, silent: silent.map((t) => ({ noteId: t.noteId, title: t.title, file: t.file, pdfs: t.expected })) }, null, 2));
  } else {
