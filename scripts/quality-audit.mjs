...
const CHECKS = [
  ...
  { id: 'coconala-analytics', npm: 'check-coconala-analytics', timeout: 60_000, ci: false, ops: true, note: 'ココナラ分析の鮮度・listed全件取得・kpi-log整合を検査。取得は認証済みローカルで行い、読み手＝ops-audit.yml（日次Issue）で、停止は同ワークフローへ集約する。' },
  ...
  { id: 'ig-insights-freshness', npm: 'check-ig-insights-freshness', timeout: 30_000, ci: false, ops: true, note: 'Instagram Graph API 週次取得（fetch-ig-insights.yml）の停止と、トークン失効 7 日前を検知する。取得は CI（Secrets のトークン）。読み手＝ops-audit.yml（日次 --ops → automation-failure Issue channel ops・復旧で自動クローズ）' },
