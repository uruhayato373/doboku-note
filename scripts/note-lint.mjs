 ...
 *
 * 使い方:
 *   node scripts/note-lint.mjs                       # staged の content/note 配下の article.md を検査（pre-commit 用）
...
function stagedNoteArticles() {
  try {
    // -c core.quotepath=false: 日本語パスを引用符+8進エスケープ("content/note/1\347...")でなく
    // 生UTF-8で出力させる。既定(quotepath=true)だと下の /^content\/note\// 正規表現に不一致で
    ...
    return execSync('git -c core.quotepath=false diff --cached --name-only --diff-filter=ACM', { encoding: 'utf8', cwd: ROOT })
      .split('\n').filter((f) => /^content\/note\/.*\/article\.md$/.test(f))
      .map((f) => join(ROOT, f))
...
const PERSONA_RE = /content\/note\/技術士総監\/magazines\/(総監模範論文-[^/]+)\/[^/]+\/article\.md$/;
...
// 10. 同じ画像の重複。全件検査は check-note-duplicate-images（CI）が同じ判定で行う。
function checkDuplicateImages(content) {
  return findDuplicateImages(content).map((d) => ({
    line: d.line,
    msg: `同じ画像を重複使用（初出 ${d.firstLine} 行目）: ${d.path}。2 枚目は note の CDN 確定に至らず全文更新が中断する。この行を削除`,
  }));
}
// 11. インラインのバッククォート。全件検査は check-note-inline-code（CI）が同じ判定で行う。
function checkInlineCode(content) {
  return findInlineCode(content).map((h) => ({
    line: h.line,
...
if (args.length === 0) {
  ...
  files = args.flatMap((a) => {
    const p = existsSync(a) ? a : join(ROOT, 'content/note', a);
