...
//
// 使い方:
//   node scripts/check-note-hashtags.mjs            # docs/note 配下の全 hashtags を検査（CI 用・exit 1）
...
if (STAGED) {
  let staged = [];
  try {
    // 日本語パス（docs/note/技術士総監/...）は既定の core.quotepath で "\346..." に
    // エスケープされ startsWith('docs/note/') が外れる＝staged 素通りになる。必ず無効化する。
