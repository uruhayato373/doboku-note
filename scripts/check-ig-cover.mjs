#!/usr/bin/env node
// IG figure-pack の表紙（docs/sns/instagram/**/carousel/img/00-cover.svg）が
...
//   6. パスが docs/sns/instagram/{exam}/... 配下（exam 試験dir直下に置かない）
//
// 使い方:
...
for (const f of files) {
  ...
  if (NON_EXAM.has(examDir) || !/^[a-z0-9-]+$/.test(examDir) || segs.length < 4 || segs[1] === 'carousel') {
    problems.push(`${f}: パスが試験dir配下でない（docs/sns/instagram/{exam}/{slug}/carousel/img/ に置く）`);
  }
