#!/usr/bin/env node
// docs/note/5管理-情報管理/img/ に本文用 PNG/SVG 図版 10 枚を生成する。
//
// 設計ルール（.claude/reference/note-svg-policy.md 準拠）:
//   - キャンバス幅 1200（同一記事内で統一）
//   - フォント ≥ 22px（補足キャプションを含めて統一）
//   - 色トークン（src/styles/globals.css と整合）のみ使用
//   - ウォーターマーク不要（brandFrame は空文字）
//
// 使い方:
...
const OUT_DIR = join(ROOT, 'docs/note/5管理-情報管理/img');
...
function svgSeciModel() {
  ...
  quads.forEach(q => {
    ...
    <text x="${q.x + qW / 2}" y="${q.y + 150}" text-anchor="middle" font-size="17" fill="${INK_MUTED}">例: ${xml(q.example)}</text>`;
  });
  ...
  <!-- 螺旋の矢印（簡略化：循環を示す） -->
  <path d="M ${cx} ${cy - 16} A 16 16 0 1 1 ${cx + 1} ${cy - 16}" fill="none" stroke="${INK_MUTED}" stroke-width="2" marker-end="url(#arr)"/>
  <defs>
...
function svgIpRightsDuration() {
  ...
  const rights = [
    ...
    { name: '意匠権', period: '登録から25年', note: '秘密意匠制度あり', bars: 25, color: WARN },
    { name: '商標権', period: '登録から10年', note: '更新登録で実質無期限', bars: 10, color: DANGER },
    { name: '著作権', period: '死後70年', note: '創作時に自動発生（登録不要）', bars: 70, color: '#6a1b9a' },
  ];
  ...
  rights.forEach((r, i) => {
    ...
    <text x="${barAreaX}" y="${y + barH + 18}" font-size="17" fill="${INK_MUTED}">${xml(r.note)}</text>`;
  });
  ...
  ${rows}
  <text x="40" y="${rowStart + rights.length * rowH + 30}" font-size="18" fill="${INK_BODY}">${xml(note1)}</text>
  <text x="40" y="${rowStart + rights.length * rowH + 58}" font-size="18" fill="${DANGER}">${xml(note2)}</text>
  ${brandFrame()}
</svg>`;
}
...
function svgThreeAverages() {
  ...
  ${cards}
  <text x="40" y="${H - 20}" font-size="18" fill="${INK_MUTED}">覚え方: 比率・成長 → 幾何平均（√の形）、速度・密度 → 調和平均（逆数）、それ以外 → 算術平均</text>
  ${brandFrame()}
</svg>`;
}
...
function svgDataAnalysisFlow() {
  ...
  steps.forEach((s, i) => {
    ...
    flow += `<text x="${x + stepW / 2}" y="${cy + 30}" text-anchor="middle" font-size="17" fill="${INK_BODY}">${xml(s.sub)}</text>
    <text x="${x + stepW / 2}" y="${cy + 56}" text-anchor="middle" font-size="17" fill="${INK_MUTED}">Step ${i + 1}</text>`;
    if (i < steps.length - 1) {
      const ax = x + stepW;
      flow += `
  ...
  const ml = `
  <text x="40" y="390" font-size="18" fill="${INK_BODY}">機械学習は「仮説なし」でデータからパターンを自動発見。統計分析は「仮説あり」の確認的アプローチ。</text>`;
...
function svgCiaIsms() {
  const H = 620;
  const cia = [
    { name: '機密性', en: 'Confidentiality', desc: '許可された者のみが情報にアクセスできる', icon: '🔒', color: BRAND, fill: BRAND_FILL },
    { name: '完全性', en: 'Integrity', desc: '情報が正確・完全で不正改ざんなし', icon: '✅', color: POSITIVE, fill: POSITIVE_FILL },
    { name: '可用性', en: 'Availability', desc: '必要な時に許可された者がアクセスできる', icon: '⚡', color: WARN, fill: WARN_FILL },
  ];
...
function svgCommunicationTechniques() {
  ...
  const techs = [
    ...
    { num: '⑤', name: '合意形成', desc: 'ステークホルダーの意見一致を図る', color: '#6a1b9a' },
  ...
  ${rows}
  <text x="40" y="${H - 20}" font-size="18" fill="${DANGER}">${xml(note)}</text>
  ${brandFrame()}
</svg>`;
}

// ------------------------------------------------------------------
// 図 8: 特許出願から取得までのフロー（簡略版）
// ------------------------------------------------------------------
function svgPatentFlow() {
  const H = 560;
  const mainSteps = [
    { label: '① 特許出願', note: '特許庁に出願書類を提出', color: BRAND, fill: BRAND_FILL },
    { label: '② 方式審査', note: '書類上の形式確認', color: BRAND, fill: BRAND_FILL },
    { label: '③ 出願公開', note: '出願日から1年6ヶ月後', color: WARN, fill: WARN_FILL },
    { label: '④ 審査請求', note: '出願日から3年以内（必須）', color: WARN, fill: WARN_FILL },
    { label: '⑤ 実体審査', note: '特許要件の詳細審査', color: '#6a1b9a', fill: '#f3e5f5' },
  ];
  ...
  mainSteps.forEach((s, i) => {
    const x = startX + i * (stepW + arrowLen);
    flow += `
    <rect x="${x}" y="${mainY}" width="${stepW}" height="${stepH}" rx="12" fill="${s.fill}" stroke="${s.color}" stroke-width="2"/>
    <text x="${x + stepW / 2}" y="${mainY + 44}" text-anchor="middle" font-size="20" font-weight="700" fill="${s.color}">${xml(s.label)}</text>
    <text x="${x + stepW / 2}" y="${mainY + 72}" text-anchor="middle" font-size="16" fill="${INK_BODY}">${xml(s.note)}</text>`;
    if (i < mainSteps.length - 1) {
      const ax = x + stepW;
      flow += `
    <line x1="${ax}" y1="${mainY + stepH / 2}" x2="${ax + arrowLen - 10}" y2="${mainY + stepH / 2}" stroke="${INK_MUTED}" stroke-width="2.5"/>
    <polygon points="${ax + arrowLen - 2},${mainY + stepH / 2} ${ax + arrowLen - 12},${mainY + stepH / 2 - 7} ${ax + arrowLen - 12},${mainY + stepH / 2 + 7}" fill="${INK_MUTED}"/>`;
    }
  });

  // 分岐：特許査定 or 拒絶査定
  const branchX = startX + 4 * (stepW + arrowLen) + stepW;
  flow += `
  <!-- 分岐 -->
  <line x1="${branchX}" y1="${mainY + stepH / 2}" x2="${branchX + 40}" y2="${mainY + stepH / 2}" stroke="${INK_MUTED}" stroke-width="2.5"/>
  <line x1="${branchX + 40}" y1="${mainY + stepH / 2}" x2="${branchX + 40}" y2="${mainY + stepH / 2 - 90}" stroke="${POSITIVE}" stroke-width="2"/>
  <line x1="${branchX + 40}" y1="${mainY + stepH / 2 - 90}" x2="${branchX + 220}" y2="${mainY + stepH / 2 - 90}" stroke="${POSITIVE}" stroke-width="2"/>
  <rect x="${branchX + 220}" y="${mainY - stepH + 24}" width="160" height="${stepH - 24}" rx="10" fill="${POSITIVE_FILL}" stroke="${POSITIVE}" stroke-width="2"/>
  <text x="${branchX + 300}" y="${mainY - 20}" text-anchor="middle" font-size="18" font-weight="700" fill="${POSITIVE}">特許査定</text>
  <text x="${branchX + 300}" y="${mainY + 4}" text-anchor="middle" font-size="15" fill="${INK_BODY}">→ 特許料納付 → 登録</text>
  <line x1="${branchX + 40}" y1="${mainY + stepH / 2}" x2="${branchX + 40}" y2="${mainY + stepH / 2 + 80}" stroke="${DANGER}" stroke-width="2"/>
  <line x1="${branchX + 40}" y1="${mainY + stepH / 2 + 80}" x2="${branchX + 220}" y2="${mainY + stepH / 2 + 80}" stroke="${DANGER}" stroke-width="2"/>
  <rect x="${branchX + 220}" y="${mainY + stepH / 2 + 40}" width="160" height="80" rx="10" fill="${DANGER_FILL}" stroke="${DANGER}" stroke-width="2"/>
  <text x="${branchX + 300}" y="${mainY + stepH / 2 + 86}" text-anchor="middle" font-size="18" font-weight="700" fill="${DANGER}">拒絶査定</text>
  <text x="${branchX + 300}" y="${mainY + stepH / 2 + 108}" text-anchor="middle" font-size="15" fill="${INK_BODY}">→ 不服審判可</text>`;

  const key1 = '★ 先願主義：最初に出願した者が権利を取得（創作時期は無関係）';
  const key2 = '★ 審査請求なし = 取下扱い。出願日から3年以内の請求が必須。';

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" font-family="${FONT}">
  ${header('特許取得フロー', '出願から登録まで。審査請求は出願日から3年以内')}
  ${flow}
  <text x="40" y="${H - 50}" font-size="19" font-weight="600" fill="${BRAND}">${xml(key1)}</text>
  <text x="40" y="${H - 22}" font-size="19" font-weight="600" fill="${DANGER}">${xml(key2)}</text>
  ${brandFrame()}
</svg>`;
}
...
function svgDataScale() {
  ...
  scales.forEach((s, i) => {
    ...
    <text x="72" y="${y + 62}" font-size="17" fill="${INK_MUTED}">${xml(s.en)}</text>
    <text x="310" y="${y + 38}" font-size="18" fill="${INK_BODY}">${xml(s.example)}</text>
    <text x="310" y="${y + 64}" font-size="17" fill="${INK_MUTED}">${xml(s.ops)}</text>
    <rect x="${W - 220}" y="${y + 12}" width="160" height="52" rx="8" fill="white" stroke="${s.color}" stroke-width="1.5"/>
    <text x="${W - 140}" y="${y + 32}" text-anchor="middle" font-size="16" fill="${INK_MUTED}">代表統計量</text>
    <text x="${W - 140}" y="${y + 54}" text-anchor="middle" font-size="20" font-weight="700" fill="${s.color}">${xml(s.stat)}</text>`;
  });
  ...
  ${rows}
  <text x="40" y="${H - 20}" font-size="17" fill="${INK_BODY}">${xml(note)}</text>
  ${brandFrame()}
</svg>`;
}

// ------------------------------------------------------------------
// 図 10: ゼロトラストvs境界型セキュリティ
// ------------------------------------------------------------------
function svgZeroTrustVsBoundary() {
  ...
  leftItems.forEach((t, i) => {
    items += `<text x="${lx + 20}" y="${boxY + 76 + i * 52}" font-size="18" fill="${INK_BODY}">・${xml(t)}</text>`;
  });
  rightItems.forEach((t, i) => {
    const color = t.includes('両者') ? POSITIVE : INK_BODY;
    items += `<text x="${rx + 20}" y="${boxY + 76 + i * 52}" font-size="18" fill="${color}">・${xml(t)}</text>`;
  });
  ...
  <text x="40" y="${H - 20}" font-size="19" font-weight="700" fill="${DANGER}">${xml(warn)}</text>
  ${brandFrame()}
</svg>`;
}
