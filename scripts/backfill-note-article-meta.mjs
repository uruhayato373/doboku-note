#!/usr/bin/env node
/**
 * note 公開マガジンの収録記事 URL を、docs/note/**\/magazines/ 配下の article.md
...
for (const f of files) {
  ...
  if (!hit) {
    unmatched.push(`${f.replace("docs/note/", "")}  «${title}»`);
    continue;
  }
...
for (const p of plan.slice(0, 60)) {
  console.log(
    `  ${p.f.replace("docs/note/", "")}${p.via === "loose" ? "  [loose]" : ""}${p.ambiguous ? "  [曖昧]" : ""}\n      → ${p.hit.raw}\n      → ${p.hit.url}  (${p.hit.date}, ¥${p.hit.price})`,
  );
}
