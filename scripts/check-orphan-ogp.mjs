 ...
 *
 * `.local/r2/posts/**` にある OGP 画像（`ogp.png` / `ogp.webp`）のうち、対応する記事 MDX が
...
const POSTS_DIR = path.join(root, '.local', 'r2', 'posts');
...
if (doFix && orphans.length) {
  ...
  if (!asJson) {
    ...
    console.log(JSON.stringify({ fixed: orphans.map(rel), removedDirs: [...removedDirs].map(rel) }, null, 2));
  }
  process.exit(0);
}
