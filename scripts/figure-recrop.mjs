...
if (dryRun) {
  ...
  writeSync(1, JSON.stringify({ ...result, mode: "dry-run" }, null, 2) + '\n');
  process.exit(0);
}
...
if (!noMdx) {
  const parts = path.relative(path.join(ROOT, "content/site"), dir).split(path.sep); // [cat, localSlug, img]
  const slug = parts.slice(0, 2).join("/");
  const cands = [
    path.join(ROOT, "content/site", slug, "article.mdx"),
    path.join(ROOT, "content/site", slug + ".mdx"),
    path.join(ROOT, "content/site", slug.replace(/\//g, "-"), "article.mdx"),
  ];
