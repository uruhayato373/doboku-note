...
if (staged) {
  ...
  const relevant = changed.split('\n').some((p) =>
    /^docs\/note\/.*\/article(-[^/]+)?\.md$/.test(p.trim())
...
if (!existsSync(absBase)) {
  console.log('[check-note-price-consistency] docs/note が無いためスキップ');
  process.exit(0);
}
...
for (const file of walk(absBase)) {
  ...
  // シリーズ = magazines/ の1つ上（例: docs/note/技術士建設部門）。無ければ記事の親。
  const m = rel.match(/^(docs\/note\/[^/]+(?:\/[^/]+)?)\/magazines\//);
...
if (asJson) {
  console.log(JSON.stringify({ magazines: magCount, violations }, null, 2));
