...
for (const file of walk(BASE)) {
  ...
  const pricing = fmv(fm, 'notePricing');
  // 会員限定記事は未ログイン本文が0字で、無料記事の導入や有料記事の無料プレビューを持たない。
  // notePricing 未対応のまま free 扱いすると、公開後の週次お題を誤ってブロックする。
  if (pricing === 'membership') { membership++; continue; }
  const isPaid = pricing === 'paid';
  if (isPaid) paid++; else free++;
  if (allowSet.has(noteId)) { exempt++; continue; }
...
console.log(`[check-note-intro-benefit${STAGED ? ' --staged' : ''}] 実検査 有料 ${paid} 件 / 無料 ${free} 件 / 会員限定skip ${membership} 件 / allowlist適用 ${exempt} 件`);
...
if (!bad.length) console.log(`\n✓ 公開 ${paid + free - exempt} 件すべてが benefit 節を持つ（有料は無料プレビュー内）`);
