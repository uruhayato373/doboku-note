...
async function main() {
  ...
  // 見積り（カスタム提案）の取引名は出品サービス名と一致しないため、title 前方一致では
  // **構造的に必ず** serviceId が引けない。これは故障ではないので毎回の警告から外す。
  // ただし黙って捨てると本物の title 変更も一緒に消えるので、**運用台帳に quote として
  ...
  const quotedRooms = (() => {
    try {
      ...
      return new Map(); // 台帳が読めないなら例外を作らない＝全部警告する側へ倒す
    }
  })();
